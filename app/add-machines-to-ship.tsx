...
import { View, Text, StyleSheet, FlatList, ScrollView, TouchableOpacity, TextInput, Platform, ActivityIndicator, } from 'react-native';
...
const MachineRow = React.memo(function MachineRow({ machine, isSelected, onToggle }: MachineRowProps) {
    return (<TouchableOpacity style={[styles.machineRow, isSelected && styles.machineRowSelected]} onPress={() => onToggle(machine.id)} activeOpacity={0.7}>
...
export default function AddMachinesToShipScreen() {
    ...
    const renderMachineRow = useCallback(({ item }: ListRenderItemInfo<MachineEncyclopediaEntry>) => (<MachineRow machine={item} isSelected={selectedMachines.has(item.id)} onToggle={toggleMachine}/>), [selectedMachines, toggleMachine]);
    ...
    const handleAddMachinesToShip = async () => {
        if (!selectedShip || selectedMachines.size === 0)
            return;
        try {
            setIsAdding(true);
    ...
    return (<>
      ...
      <SafeAreaView style={styles.container} edges={['top']}>
        ...
        <View style={styles.shipSelectorContainer}>
          ...
          <TouchableOpacity style={styles.shipSelector} onPress={() => setFilterModal('ship')} activeOpacity={0.7}>
        ...
        <View style={styles.searchContainer}>
          <View style={styles.searchBar}>
            ...
            <TextInput style={styles.searchInput} placeholder="Search machines..." placeholderTextColor={COLORS.textDarkGrey} value={searchQuery} onChangeText={setSearchQuery}/>
            
          </View>
        </View>

        <View style={styles.filtersRow}>
          <TouchableOpacity style={[styles.filterButton, filterManufacturers.length > 0 && styles.filterButtonActive]} onPress={() => setFilterModal('manufacturer')} activeOpacity={0.7}>
          ...
          <TouchableOpacity style={[styles.filterButton, sortBy !== 'name' && styles.filterButtonActive]} onPress={() => {
        ...
        <FlatList data={displayedMachines} renderItem={renderMachineRow} keyExtractor={keyExtractor} style={styles.scrollView} contentContainerStyle={styles.machinesList} showsVerticalScrollIndicator={false} ListEmptyComponent={ListEmpty} extraData={selectedMachines} initialNumToRender={15} maxToRenderPerBatch={15} windowSize={5} removeClippedSubviews={Platform.OS !== 'web'}/>
