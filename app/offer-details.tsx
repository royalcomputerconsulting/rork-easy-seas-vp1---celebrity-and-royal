...
import { Ship, Clock, ChevronRight, CheckCircle, X, Dice5, Star, DollarSign, Ban, Archive, BedDouble, Users, ChevronDown, Filter, } from 'lucide-react-native';
...
export default function OfferDetailsScreen() {
    ...
    const { offerCode } = useLocalSearchParams<{
        offerCode: string;
    }>();
    ...
    const offerData = useMemo(() => {
        ...
        const uniqueCruises = allCruises.filter((cruise, index, self) => index === self.findIndex(c => c.id === cruise.id));
        // Find cruises matching this offer code
        let matchingCruises = uniqueCruises.filter((c: Cruise) => c.offerCode === offerCode);
        const offer = allOffers.find((o: CasinoOffer) => o.offerCode === offerCode);
        ...
        const filteredCruises = enrichedCruises.filter(cruise => {
            if (filterShip && cruise.shipName !== filterShip)
                return false;
            if (filterGuests) {
                const cruiseGuests = cruise.guests || 2;
                if (cruiseGuests !== filterGuests)
                    return false;
            }
            if (filterRoomType) {
                const cabinType = (cruise.cabinType || '').toLowerCase();
                if (!cabinType.includes(filterRoomType.toLowerCase()))
                    return false;
            }
            return true;
        });
        const cruises = [...filteredCruises].sort((a, b) => {
            switch (sortBy) {
                ...
                case 'highest-value': {
                    const valueA = Math.max(a.suitePrice || 0, a.balconyPrice || 0, a.oceanviewPrice || 0, a.interiorPrice || 0);
                    const valueB = Math.max(b.suitePrice || 0, b.balconyPrice || 0, b.oceanviewPrice || 0, b.interiorPrice || 0);
    ...
    const filterOptions = useMemo(() => {
        const allCruises = [...(storeCruises || []), ...(localData.cruises || [])].filter((c: Cruise) => c.offerCode === offerCode);
        const uniqueCruises = allCruises.filter((cruise, index, self) => index === self.findIndex(c => c.id === cruise.id));
        ...
        const roomTypes = [...new Set(uniqueCruises
                .map(c => c.cabinType)
    ...
    const offerInfo = useMemo(() => {
        ...
        cruises.forEach(cruise => {
            ...
            if (interiorPrice > 0) {
                ...
                if (minVal < minRetailValue)
                    minRetailValue = minVal;
            }
            if (suitePrice > 0) {
                const maxVal = (suitePrice * guestCount) + taxes;
                if (maxVal > maxRetailValue)
                    maxRetailValue = maxVal;
            }
    ...
    const renderCruiseCard = useCallback(({ item }: {
        ...
        return (<TouchableOpacity style={[styles.cruiseCard, isBooked && styles.bookedCard]} onPress={() => handleCruisePress(item.id)} activeOpacity={0.85}>
        <LinearGradient colors={isBooked
                ? ['#34D399', '#10B981', '#059669']
                : ['#0EA5E9', '#6366F1', '#8B5CF6']} start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }} style={StyleSheet.absoluteFill}/>
    ...
    return (<View style={styles.container}>
      ...
      <SafeAreaView style={styles.safeArea} edges={['top']}>
        ...
        <LinearGradient colors={['#E0F2FE', '#DBEAFE', '#E0F7FA']} start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }} style={styles.mergedHeader}>
          {/* Close Button */}
          ...
          <View style={styles.featuredOfferSection}>
            <Image source={{ uri: IMAGES.logo }} style={styles.offerLogo} resizeMode="contain"/>
        ...
        <View style={styles.sortSection}>
          ...
          <View style={styles.sortRowCentered}>
            <TouchableOpacity style={[styles.sortPillMain, sortBy === 'soonest' && styles.sortPillMainActive]} onPress={() => setSortBy('soonest')} activeOpacity={0.7} testID="sort-soonest">
              <Text style={[styles.sortPillMainText, sortBy === 'soonest' && styles.sortPillMainTextActive]}>Soonest Expiring</Text>
            </TouchableOpacity>
            <TouchableOpacity style={[styles.sortPillMain, sortBy === 'highest-value' && styles.sortPillMainActive]} onPress={() => setSortBy('highest-value')} activeOpacity={0.7} testID="sort-highest-value">
        ...
        <FlatList data={offerData.cruises} renderItem={renderCruiseCard} keyExtractor={(item) => item.id} extraData={`${sortBy}-${filterShip}-${filterGuests}-${filterRoomType}`} contentContainerStyle={styles.listContent} showsVerticalScrollIndicator={false} ListEmptyComponent={<View style={styles.emptyState}>
