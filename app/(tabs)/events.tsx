...
export default function EventsScreen() {
    ...
    const renderEventDots = useCallback((events: {
        cruise: number;
        travel: number;
        personal: number;
    }) => {
    ...
    const renderDayCell = useCallback((day: DayData) => {
        ...
        return (<TouchableOpacity key={day.date.toISOString()} style={[
                styles.dayCell,
                { backgroundColor: bgColor },
                day.isToday && styles.todayCell,
                !day.isCurrentMonth && styles.otherMonthCell,
            ]} activeOpacity={0.7} onPress={() => handleDayPress(day)}>
        <Text style={[
                styles.dayNumber,
    ...
    const upcomingEvents = useMemo(() => {
        ...
        today.setHours(0, 0, 0, 0);
        const allEvents: {
            event: CalendarEvent | BookedCruise;
            type: 'calendar' | 'cruise';
            date: Date;
        }[] = [];
        calendarEvents.forEach(event => {
    ...
    const renderEventCard = useCallback((item: {
        ...
        if (item.type === 'cruise') {
            ...
            return (<TouchableOpacity key={`cruise-${cruise.id}-${index}`} style={styles.eventCard} activeOpacity={0.85} onPress={() => router.push({
        ...
        const EventIcon = event.type === 'cruise' ? Ship
            : (event.type === 'travel' || event.type === 'flight') ? Plane
                : User;
        return (<TouchableOpacity key={`event-${event.id}-${index}`} style={styles.eventCard} activeOpacity={0.85}>
        ...
        <View style={styles.eventCardContent}>
          ...
          {event.location ? (<Text style={styles.eventCardSubtitle}>{event.location}</Text>) : null}
          <Text style={styles.eventCardDate}>
            {item.date.toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric', timeZone: 'UTC' })}
          </Text>
        </View>
      ...
      <SafeAreaView style={styles.safeArea} edges={['top']}>
        <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.scrollContent}>
          <View style={styles.heroHeader}>
            <View style={styles.heroContent}>
              <View style={styles.logoSection}>
                <Image source={{ uri: 'https://pub-e001eb4506b145aa938b5d3badbff6a5.r2.dev/attachments/m9jyogxgz1j0xb91psbkq' }} style={styles.logoImage} resizeMode="cover"/>
              </View>
          ...
          <View style={styles.viewToggleContainer}>
            {(['events', 'week', 'month', '90days'] as ViewMode[]).map((mode) => (<TouchableOpacity key={mode} style={[
