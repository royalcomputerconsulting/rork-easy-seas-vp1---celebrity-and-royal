...
import { View, Text, StyleSheet, ScrollView, Dimensions, TouchableOpacity, RefreshControl, Image, Platform, ActivityIndicator, } from 'react-native';
import * as FileSystem from 'expo-file-system';
import * as Sharing from 'expo-sharing';
import { LinearGradient } from 'expo-linear-gradient';
import { Stack, useRouter } from 'expo-router';
import { SafeAreaView } from 'react-native-safe-area-context';
import { BarChart3, TrendingUp, Ship, DollarSign, Award, MapPin, Zap, PieChart, Coins, Target, ChevronDown, Brain, LineChart, Receipt, Calendar, Dices, Calculator, Download, } from 'lucide-react-native';
...
import { CLUB_ROYALE_TIERS, getTierProgress } from '@/constants/clubRoyaleTiers';
import { getLevelProgress } from '@/constants/crownAnchor';
...
import { PlayerContext, runSimulation } from '@/lib/whatIfSimulator';
...
function calculateCruiseROI(cruise: BookedCruise): {
    roi: number;
...
export default function AnalyticsScreen() {
    ...
    const { activeAlerts, insights, dismissAlert, snoozeAlert, clearAllAlerts, runDetection, } = useAlerts();
    const { clubRoyaleProfile, localData } = useAppState();
    const { bookedCruises: storedBookedCruises, isLoading: storeLoading } = useCoreData();
    const { clubRoyalePoints: loyaltyClubRoyalePoints, clubRoyaleTier: loyaltyClubRoyaleTier, crownAnchorPoints: loyaltyCrownAnchorPoints, crownAnchorLevel: loyaltyCrownAnchorLevel, } = useLoyalty();
    ...
    const { sessions, addSession, removeSession, getSessionsForDate, getDailySummary, getSessionAnalytics, generateHistoricalSessions, } = useCasinoSessions();
    const { streak, updateStreakFromSession, updateWeeklyGoalProgress, checkAndUnlockAchievements, } = useGamification();
    ...
    const { w2gRecords, addW2GRecord, removeW2GRecord, compItems, } = useTax();
    ...
    const handleExportCruisePortfolio = useCallback(async () => {
        try {
            ...
            const completedCruises = bookedCruises.filter((c) => {
                if (c.completionState === 'completed' || c.status === 'completed')
                    return true;
                if (c.returnDate) {
    ...
    const renderROIFilterTabs = () => (<View style={styles.filterTabsRow}>
      <View style={styles.filterTabs}>
        {(['all', 'high', 'medium', 'low'] as ROIFilter[]).map((filter) => {
            ...
            return (<TouchableOpacity key={filter} style={[styles.filterTab, isActive && styles.filterTabActive]} onPress={() => setRoiFilter(filter)} activeOpacity={0.7} testID={`casino-portfolio-filter-${filter}`}>
      ...
      <TouchableOpacity style={styles.exportButton} activeOpacity={0.8} onPress={handleExportCruisePortfolio} testID="casino-portfolio-export">
    ...
    const renderPortfolioCard = (cruise: typeof cruisesWithROI[0]) => {
        ...
        return (<TouchableOpacity key={cruise.id} style={styles.portfolioCard} onPress={() => handleCruisePress(cruise.id)} activeOpacity={0.85}>
        <View style={styles.portfolioImageContainer}>
          <Image source={{ uri: cruiseImage }} style={styles.portfolioCardImage} resizeMode="cover" defaultSource={{ uri: DEFAULT_CRUISE_IMAGE }}/>
    ...
    const renderIntelligenceTab = () => (<View style={styles.tabContent}>
      <View style={styles.quickStatsRow}>
        {stats.map((stat, index) => (<View key={index} style={styles.quickStatItem}>
      ...
      <View style={styles.section} testID="casino-player-loyalty-section">
        <CompactDashboardHeader hideLogo={true} memberName={clubRoyaleProfile?.memberName || 'Player'} crownAnchorNumber={(clubRoyaleProfile as any)?.crownAnchorNumber} crewMemberCount={crewStats?.crewMemberCount || 0}/>
      </View>
      ...
      <View style={styles.section}>
        <CasinoMetricsCard completedCruises={bookedCruises.filter(c => {
            if (c.completionState === 'completed' || c.status === 'completed')
                return true;
            if (c.returnDate) {
            ...
            return false;
        })} currentPoints={currentPoints} currentTier={clubRoyaleTier} alwaysExpanded={true}/>
      </View>
      ...
      <View style={styles.section}>
        ...
        {filteredCruises.length > 0 ? (<View style={styles.portfolioList}>
            {(showAllCruises ? filteredCruises.slice(0, 25) : filteredCruises.slice(0, 5)).map(renderPortfolioCard)}
            
            
          </View>) : (<View style={styles.emptyPortfolio}>
      ...
      <View style={styles.section}>
        <AlertsCard alerts={activeAlerts} insights={insights} onDismiss={dismissAlert} onSnooze={snoozeAlert} onClearAll={clearAllAlerts} maxAlerts={5} showInsights={true} title="Pattern Recognition & Alerts"/>
      </View>

      <View style={styles.section}>
        <W2GTracker records={w2gRecords} onAddRecord={addW2GRecord} onRemoveRecord={removeW2GRecord}/>
      </View>

      <View style={styles.section}>
        <CompValueCalculator initialItems={compItems} onCompValueChange={(totalValue) => {
    ...
    const renderSessionTab = () => (<View style={styles.tabContent}>
      <View style={styles.section}>
        <SessionsSummaryCard analytics={sessionAnalytics} sessions={sessions} targetPPH={targetPPH}/>
      </View>
      ...
      <View style={styles.section}>
        <View style={[styles.alertsBanner, { backgroundColor: 'rgba(0, 31, 63, 0.05)' }]}>
          ...
          <View style={{ flexDirection: 'row', gap: 8 }}>
            <TouchableOpacity onPress={() => handleGenerateHistoricalSessions(true)} style={[styles.regenerateButton, isGeneratingSessions && { opacity: 0.6 }]} disabled={isGeneratingSessions}>
              <Text style={styles.regenerateButtonText}>
                {isGeneratingSessions ? '...' : 'Regenerate'}
              </Text>
            </TouchableOpacity>
            <TouchableOpacity onPress={() => handleGenerateHistoricalSessions(false)} style={[styles.calculateButton, isGeneratingSessions && { opacity: 0.6 }]} disabled={isGeneratingSessions}>
      ...
      <View style={styles.section}>
        <LivePPHTracker targetPPH={targetPPH} onSessionComplete={handleLiveSessionComplete} historicalAvgPPH={sessionAnalytics.pointsPerHour}/>
      </View>

      <View style={styles.section}>
        <PPHGoalsCard analytics={sessionAnalytics} sessions={sessions} targetPPH={targetPPH} onTargetChange={setTargetPPH}/>
      </View>

      <View style={styles.section}>
        <PointsPerHourCard analytics={sessionAnalytics} sessions={sessions}/>
      </View>
      ...
      <View style={styles.section}>
        <WeeklyGoalsCard compact={true} onGoalComplete={(goal) => {
            haptics.success();
            setCelebrationData({
      ...
      <View style={styles.section}>
        <CasinoSessionTracker date={todayDateString} goldenTimeSlots={goldenTimeSlots} sessions={todaySessions} summary={todaySummary} onAddSession={() => setShowAddSessionModal(true)} onRemoveSession={handleRemoveSession}/>
      </View>

      <View style={styles.section}>
        <CasinoIntelligenceCard analytics={sessionAnalytics} completedCruises={bookedCruises.filter(c => {
            if (c.completionState === 'completed' || c.status === 'completed')
                return true;
            if (c.returnDate) {
                const returnDate = new Date(c.returnDate);
    ...
    const renderCalcsTab = () => (<View style={styles.tabContent}>
      <View style={styles.section}>
        ...
        <View style={styles.calcsGrid}>
          {highValueCalculations.map((calc) => (<View key={calc.id} style={styles.calcCard}>
    ...
    const renderChartsTab = () => (<View style={styles.tabContent}>
      <View style={styles.section}>
        <TierProgressionChart playerContext={playerContext} bookedCruises={bookedCruises} monthsAhead={24}/>
      </View>

      <View style={styles.section}>
        <ROIProjectionChart roiProjection={baselineSimulation.roiProjection} comparisonROI={baselineSimulation.roiProjection.projectedROI} totalSpent={realAnalytics.totalOutOfPocket} totalRetailValue={realAnalytics.totalRetailValue} totalPointsEarned={currentPoints}/>
      </View>

      <View style={styles.section}>
        <RiskAnalysisChart riskAnalysis={baselineSimulation.riskAnalysis} totalSpent={realAnalytics.completedOutOfPocket} totalRetailValue={realAnalytics.completedRetailValue} totalSavings={realAnalytics.completedProfit} casinoNetResult={realAnalytics.completedNetCasinoResult || 0} totalCruises={realAnalytics.completedCruisesCount} pointsEarned={currentPoints}/>
      </View>
    </View>);
    return (<LinearGradient colors={['#E3F2FD', '#90CAF9']} style={styles.container}>
      ...
      <SafeAreaView style={styles.safeArea} edges={['top']}>
        ...
        <View style={styles.tabsContainer}>
          <TouchableOpacity style={[styles.tabButton, activeTab === 'intelligence' && styles.tabButtonActive]} onPress={() => setActiveTab('intelligence')} activeOpacity={0.7}>
          ...
          <TouchableOpacity style={[styles.tabButton, activeTab === 'charts' && styles.tabButtonActive]} onPress={() => setActiveTab('charts')} activeOpacity={0.7}>
          ...
          <TouchableOpacity style={[styles.tabButton, activeTab === 'session' && styles.tabButtonActive]} onPress={() => setActiveTab('session')} activeOpacity={0.7}>
          ...
          <TouchableOpacity style={[styles.tabButton, activeTab === 'calcs' && styles.tabButtonActive]} onPress={() => setActiveTab('calcs')} activeOpacity={0.7}>
        ...
        <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.scrollContent} removeClippedSubviews={true} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={onRefresh} tintColor={COLORS.navyDeep} colors={[COLORS.navyDeep]}/>}>
          {!isScreenReady ? (<View style={{ paddingTop: 40, alignItems: 'center' }}>
      ...
      <AddSessionModal visible={showAddSessionModal} onClose={() => setShowAddSessionModal(false)} onSave={handleAddSession} date={todayDateString} goldenTimeSlots={goldenTimeSlots}/>
