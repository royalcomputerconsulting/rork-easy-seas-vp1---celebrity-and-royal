...
import { View, Text, StyleSheet, FlatList, TouchableOpacity, RefreshControl, Platform, ActivityIndicator, Modal, } from 'react-native';
import { Stack, useRouter } from 'expo-router';
import { SafeAreaView } from 'react-native-safe-area-context';
import { Ship, X, Bell, Sparkles, ListFilter, Bot, SlidersHorizontal, Check, Bookmark, } from 'lucide-react-native';
...
export default function SchedulingScreen() {
    ...
    const _getSmartRecommendations = useCallback((cruises: Cruise[]): Cruise[] => {
        ...
        const recommendations = getRecommendedCruises(cruises, allBooked, offers, {
            limit: 20,
    ...
    const filteredCruises = useMemo(() => {
        ...
        if (filters.searchQuery.trim()) {
            ...
            result = result.filter(c => c.shipName?.toLowerCase().includes(query) ||
    ...
    const renderCruiseCard = useCallback(({ item, index: _index }: {
        item: Cruise;
        index: number;
    }) => {
        const isBooked = bookedIds.has(item.id) || activeTab === 'booked';
        return (<CruiseCard cruise={item} onPress={() => handleCruisePress(item)} variant={isBooked ? 'booked' : 'available'} mini={true}/>);
    ...
    const renderSlotOffers = useCallback((offers: CruiseOffer[]) => {
        ...
        return (<View style={styles.slotOffersContainer}>
        <Text style={styles.slotOffersTitle}>Available Offers ({offers.length} options):</Text>
        {Object.entries(groupedByCode).map(([code, codeOffers]) => (<View key={code} style={styles.offerGroup}>
            ...
            {codeOffers.map((offer, i) => (<TouchableOpacity key={`${offer.cruiseId}_${i}`} style={styles.offerOption} onPress={() => handleCruisePress(offer.cruise)} activeOpacity={0.7}>
    ...
    const renderB2BSetCard = useCallback((set: BackToBackSet, index: number) => {
        ...
        return (<View key={set.id} style={styles.b2bSetCard}>
        <LinearGradient colors={['#E0F7FA', '#DBEAFE', '#E0F2FE']} start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }} style={styles.b2bSetHeader}>
        ...
        {slots.map((slot, slotIndex) => (<View key={slot.key} style={[
        ...
        <View style={styles.b2bSetFooter}>
          ...
          <View style={styles.b2bOfferTags}>
            {(set.offerCodes || []).filter(code => code && code.trim()).map((code, i) => (<View key={i} style={styles.b2bOfferTag}>
    ...
    const renderHeader = () => (<View style={styles.headerContent}>
      <CompactDashboardHeader memberName={currentUser?.name || clubRoyaleProfile.memberName} onSettingsPress={handleSettingsPress} onAlertsPress={handleAlertsPress} alertCount={alertCount} availableCruises={stats.available} bookedCruises={stats.booked} activeOffers={alertCount} onCruisesPress={() => setActiveTab('available')} onBookedPress={() => router.push('/booked' as any)} onOffersPress={() => setActiveTab('foryou')}/>

      <MinimalistFilterBar tabs={TABS.map(tab => ({ key: tab.key, label: tab.label }))} activeTab={activeTab} onTabPress={(key) => setActiveTab(key as ViewTab)} actions={[
        ...
        ]} ships={availableShips} selectedShips={filters.selectedShips} onShipToggle={toggleShipFilter} onClearShips={() => setFilters(prev => ({ ...prev, selectedShips: [] }))} showingCount={stats.showing} totalCount={stats.total} bookedCount={stats.booked}/>

      {/* Compact sort and filter row */}
      <View style={styles.sortFilterRow}>
        <View style={styles.sortChips}>
          <TouchableOpacity style={[styles.sortChip, (filters.sortBy === 'date-asc' || filters.sortBy === 'nights-desc') && styles.sortChipActive]} onPress={() => handleSortChange(filters.sortBy === 'date-asc' ? 'nights-desc' : 'date-asc')} activeOpacity={0.7}>
          ...
          <TouchableOpacity style={[styles.sortChip, filters.sortBy === 'date-desc' && styles.sortChipActive]} onPress={() => handleSortChange('date-desc')} activeOpacity={0.7}>
            <Text style={[styles.sortChipText, filters.sortBy === 'date-desc' && styles.sortChipTextActive]}>Latest</Text>
          </TouchableOpacity>
          <TouchableOpacity style={[styles.sortChip, filters.sortBy === 'value-desc' && styles.sortChipActive]} onPress={() => handleSortChange('value-desc')} activeOpacity={0.7}>
            <Text style={[styles.sortChipText, filters.sortBy === 'value-desc' && styles.sortChipTextActive]}>Value</Text>
          </TouchableOpacity>
        </View>
        <TouchableOpacity style={[styles.filterToggle, showAdvancedFilters && styles.filterToggleActive]} onPress={() => setShowAdvancedFilters(!showAdvancedFilters)} activeOpacity={0.7}>
    ...
    return (<View style={styles.container}>
      ...
      <SafeAreaView style={styles.safeArea} edges={['top']}>
        {activeTab === 'foryou' ? (<FlatList data={sortedB2bSets} renderItem={({ item, index }) => renderB2BSetCard(item, index)} keyExtractor={(item) => item.id} contentContainerStyle={styles.listContent} ListHeaderComponent={renderHeader} ListEmptyComponent={renderEmpty} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={onRefresh} tintColor={COLORS.navyDeep} colors={[COLORS.navyDeep]}/>} showsVerticalScrollIndicator={true} persistentScrollbar={true}/>) : (<FlatList data={filteredCruises} renderItem={renderCruiseCard} keyExtractor={(item) => item.id} contentContainerStyle={styles.listContent} ListHeaderComponent={renderHeader} ListFooterComponent={renderListFooter} ListEmptyComponent={renderEmpty} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={onRefresh} tintColor={COLORS.navyDeep} colors={[COLORS.navyDeep]}/>} showsVerticalScrollIndicator={true} persistentScrollbar={true} removeClippedSubviews={Platform.OS === 'android'} initialNumToRender={10} maxToRenderPerBatch={20} windowSize={21} updateCellsBatchingPeriod={50} getItemLayout={(data, index) => ({
                length: 140,
                offset: 140 * index,
                index,
            })}/>)}
      ...
      <TouchableOpacity style={styles.agentFab} onPress={handleAgentToggle} activeOpacity={0.85}>
        <LinearGradient colors={[COLORS.goldAccent, COLORS.beigeWarm]} style={styles.agentFabGradient}>
          {isVisible ? (<X size={24} color={COLORS.navyDeep}/>) : (<Bot size={24} color={COLORS.navyDeep}/>)}
        </LinearGradient>
      </TouchableOpacity>

      {/* Agent X Chat Modal */}
      <Modal visible={isVisible} animationType="slide" transparent={true} onRequestClose={handleAgentClose}>
        <View style={styles.modalOverlay}>
          ...
          <View style={[styles.agentChatContainer, isExpanded && styles.agentChatExpanded]}>
            <AgentXChat messages={messages} onSendMessage={sendMessage} isLoading={agentLoading} isExpanded={isExpanded} onToggleExpand={toggleExpanded} onClose={handleAgentClose} showHeader={true} placeholder="Ask about cruises, tier progress, offers..."/>
          </View>
        </View>
      </Modal>
