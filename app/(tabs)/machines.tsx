...
import { ActivityIndicator, Animated, FlatList, PanResponder, Platform, StyleSheet, Text, TextInput, TouchableOpacity, View, Image, } from 'react-native';
...
export default function AtlasScreen() {
    ...
    const { myAtlasMachines, favoriteMachines, toggleFavorite, isLoading, isLoadingIndex, reload, } = useSlotMachineLibrary();
    const { sessions, updateSession, removeSession, } = useCasinoSessions();
    ...
    const filteredMachines = useMemo(() => {
        ...
        if (searchQuery) {
            ...
            filtered = filtered.filter(m => m.machineName.toLowerCase().includes(query) ||
        ...
        if (selectedShip) {
            filtered = filtered.filter(m => m.shipAssignments?.some((s: any) => s.shipName === selectedShip));
        }
    ...
    const renderMachineItem = useCallback(({ item, index }: {
        item: (typeof filteredMachines)[number];
        index: number;
    }) => {
        return (<View style={[styles.gridItem, index % 2 === 1 && styles.gridItemRight]}>
          <AtlasCard machine={item} onPress={() => handleMachinePress(item.id)} isFavorite={item.isFavorite} onToggleFavorite={() => handleToggleFavorite(item.id)} compact={true} locked={false}/>
        </View>);
    ...
    const listHeader = useMemo(() => {
        ...
        {uiError ? (<View style={styles.errorBanner} testID="machines.errorBanner">
            ...
            <TouchableOpacity style={styles.retryButton} onPress={() => {
                ...
                }} activeOpacity={0.85} testID="machines.errorRetry">
              <Text style={styles.retryButtonText}>Retry</Text>
            </TouchableOpacity>
          </View>) : null}

        {isLoading ? (<View style={styles.initialLoading} testID="machines.initialLoading">
        ...
        <View style={styles.sessionsSection}>
          <View style={styles.sectionHeader}>
            <TouchableOpacity style={styles.sectionToggle} onPress={() => setShowSessionsSection(!showSessionsSection)} activeOpacity={0.7} testID="machines.sessions.toggle">
              <Text style={styles.sectionToggleText}>Slot Play Sessions</Text>
              {showSessionsSection ? (<ChevronUp size={20} color={COLORS.navyDeep}/>) : (<ChevronDown size={20} color={COLORS.navyDeep}/>)}
            </TouchableOpacity>
            <TouchableOpacity style={styles.addSessionButton} onPress={() => setShowQuickSessionModal(true)} activeOpacity={0.7} testID="machines.sessions.add">
        ...
        <View style={styles.searchSection}>
          <View style={styles.searchBar}>
            ...
            <TextInput style={styles.searchInput} placeholder="Search machines..." value={searchQuery} onChangeText={setSearchQuery} placeholderTextColor={COLORS.textMuted} testID="machines.search.input"/>
            {searchQuery ? (<TouchableOpacity onPress={() => setSearchQuery('')} testID="machines.search.clear">
        ...
        <View style={styles.filtersContainer}>
          <FlatList data={[
            ...
            ]} horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.filtersRow} keyExtractor={(item) => item.key} renderItem={({ item }) => {
                if (item.key === 'all') {
                    return (<TouchableOpacity style={[styles.filterChip, activeFilter === 'all' && styles.filterChipActive]} onPress={() => {
                            setActiveFilter('all');
                            setSelectedManufacturer('');
                            setSelectedShip('');
                        }} activeOpacity={0.7} testID="machines.filter.all">
                ...
                if (item.key === 'favorites') {
                    return (<View style={styles.favoritesChipContainer}>
                    <TouchableOpacity style={[styles.filterChip, activeFilter === 'favorites' && styles.filterChipActive]} onPress={() => {
                            setActiveFilter('favorites');
                            setSelectedManufacturer('');
                            setSelectedShip('');
                        }} activeOpacity={0.7} testID="machines.filter.favorites">
                      <Star size={14} color={activeFilter === 'favorites' ? COLORS.white : COLORS.goldDark} fill={activeFilter === 'favorites' ? COLORS.white : 'none'}/>
                      <Text style={[styles.filterChipText, activeFilter === 'favorites' && styles.filterChipTextActive]}>
                        Favorites ({favoriteMachines.length})
                      </Text>
                    </TouchableOpacity>
                    
                    <TouchableOpacity style={[styles.exportAllButton, isExporting && styles.exportButtonDisabled]} onPress={handleExportAll} activeOpacity={0.7} disabled={isExporting} testID="machines.exportAll">
                      {exportProgress ? (<Text style={styles.exportProgressText}>
                          {exportProgress.current}/{exportProgress.total}
                        </Text>) : (<Download size={14} color={COLORS.white}/>)}
                    </TouchableOpacity>
                  </View>);
                }
                if (item.key === 'clear') {
                    return (<TouchableOpacity style={[styles.filterChip, styles.clearFilterChip]} onPress={handleClearFilters} activeOpacity={0.7} testID="machines.filter.clear">
    ...
    return (<>
      ...
      <LinearGradient colors={['#1E3A8A', '#60A5FA', '#E8F4FC']} locations={[0, 0.5, 1]} style={styles.gradientContainer}>
        <SafeAreaView style={styles.container} edges={['top']}>
        <View style={styles.listShell} onLayout={(e) => {
          ...
          <FlatList ref={(r) => {
            listRef.current = r;
        }} data={isLoading ? [] : filteredMachines} keyExtractor={(item) => item.id} renderItem={({ item, index }) => {
            try {
        ...
        }} numColumns={2} columnWrapperStyle={styles.gridRow} ListHeaderComponent={listHeader} contentContainerStyle={styles.listContent} showsVerticalScrollIndicator={false} onContentSizeChange={(_w, h) => {
            if (h && isFinite(h))
        ...
        })} scrollEventThrottle={16} onScrollToIndexFailed={(info) => {
        ...
        }} initialNumToRender={12} maxToRenderPerBatch={16} windowSize={10} removeClippedSubviews={Platform.OS !== 'web'} keyboardShouldPersistTaps="handled" testID="machines.list" ListEmptyComponent={<View style={styles.emptyState}>
      ...
      <EditMachineSessionModal visible={editingSession !== null} session={editingSession} onClose={() => setEditingSession(null)} onSave={updateSession} onDelete={removeSession}/>
