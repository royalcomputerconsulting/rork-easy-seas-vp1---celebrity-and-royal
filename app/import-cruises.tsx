...
export default function ImportCruisesScreen() {
    ...
    const handleRegenerateFeedToken = useCallback(() => {
        Alert.alert('Regenerate Feed URL', 'This will create a new unique URL. Your old URL will stop working. Any calendar apps subscribed to the old URL will need to be updated.\n\nContinue?', [
        ...
        try {
            ...
            if (result.pricing && result.pricing.length > 0) {
                ...
                for (const pricing of result.pricing) {
                    ...
                    if (pricing.interiorPrice)
                        updatePayload.interiorPrice = pricing.interiorPrice;
                    if (pricing.oceanviewPrice)
                        updatePayload.oceanviewPrice = pricing.oceanviewPrice;
                    if (pricing.balconyPrice)
                        updatePayload.balconyPrice = pricing.balconyPrice;
                    if (pricing.suitePrice)
                        updatePayload.suitePrice = pricing.suitePrice;
                    if (pricing.portTaxesFees)
                        updatePayload.taxes = pricing.portTaxesFees;
                ...
                const missingCruises = upcomingCruises.filter(c => !result.pricing.some((p: CruisePricing) => p.bookingId === c.id));
        ...
        try {
            ...
            if (delimiter && (firstLine.toLowerCase().includes('ship name') || firstLine.toLowerCase().includes('sailing date'))) {
                ...
                for (let i = 1; i < lines.length; i++) {
                    ...
                    const cabinType = roomType ? (roomType.match(/interior/i) ? 'Interior' :
                        roomType.match(/ocean|view/i) ? 'Oceanview' :
                            roomType.match(/balcony/i) ? 'Balcony' :
                                roomType.match(/suite/i) ? 'Suite' : 'Balcony') : 'Balcony';
            ...
            for (let i = 0; i < lines.length; i++) {
                ...
                if (line.includes('Ship:') || line.includes('ship:') ||
                    ...
                    if (portMatch)
                        departurePort = portMatch[1].trim();
    ...
    const renderCalendarTab = () => (<>
      <View style={styles.calFeedCard}>
        ...
        <Pressable style={[styles.publishButton, isPublishingFeed && styles.publishButtonDisabled]} onPress={handlePublishCalendarFeed} disabled={isPublishingFeed}>
          {isPublishingFeed ? (<ActivityIndicator size="small" color="#fff"/>) : (<Rss size={18} color="#fff"/>)}
        ...
        {calendarFeedUrl ? (<>
            ...
            <View style={styles.feedActionsRow}>
              <Pressable style={[styles.feedActionBtn, isCopied && styles.feedActionBtnActive]} onPress={handleCopyFeedUrl}>
                {isCopied ? (<CheckCircle size={14} color="#10b981"/>) : (<Copy size={14} color="#e2e8f0"/>)}
      ...
      <View style={styles.calExportCard}>
        ...
        <Pressable style={[styles.exportICSButton, isExportingICS && styles.exportICSButtonDisabled]} onPress={handleExportCalendarICS} disabled={isExportingICS || (bookedCruises.length === 0 && calendarEvents.length === 0)}>
          {isExportingICS ? (<ActivityIndicator size="small" color="#fff"/>) : (<Download size={18} color="#fff"/>)}
    ...
    return (<>
      ...
      <ScrollView style={styles.container} contentContainerStyle={styles.content}>
        <View style={styles.header}>
          {searchMode === 'calendar' ? (<Calendar size={32} color="#a78bfa"/>) : (<Search size={32} color="#60a5fa"/>)}
        ...
        <View style={styles.modeToggle}>
          <Pressable style={[styles.modeButton, searchMode === 'auto' && styles.modeButtonActive]} onPress={() => setSearchMode('auto')}>
          ...
          <Pressable style={[styles.modeButton, searchMode === 'manual' && styles.modeButtonActive]} onPress={() => setSearchMode('manual')}>
          ...
          <Pressable style={[styles.modeButton, searchMode === 'calendar' && styles.modeButtonActiveCal]} onPress={() => setSearchMode('calendar')}>
        ...
        {searchMode === 'calendar' ? (renderCalendarTab()) : searchMode === 'auto' ? (<>
            <View style={styles.instructionsCard}>
              ...
              <Pressable style={[styles.searchButton, searchingDeals && styles.searchButtonDisabled]} onPress={searchForDeals} disabled={searchingDeals || upcomingCruises.length === 0}>
                {searchingDeals ? (<>
              ...
              <View style={styles.step}>
                ...
                <View style={styles.stepContent}>
                  ...
                  <View style={styles.websiteLinks}>
                    <Pressable style={styles.websiteButton} onPress={() => openWebsite('https://www.icruise.com')}>
                      <Globe size={16} color="#60a5fa"/>
                      <Text style={styles.websiteButtonText}>ICruise.com</Text>
                      <ExternalLink size={14} color="#60a5fa"/>
                    </Pressable>

                    <Pressable style={styles.websiteButton} onPress={() => openWebsite('https://www.cruisesheet.com')}>
            ...
            <View style={styles.inputCard}>
              ...
              <TextInput style={styles.textInput} multiline placeholder={"Paste cruise data here...\n\nExample:\nShip: Navigator of the Seas\nSail Date: 03/15/2026\n7-Night Caribbean from Miami\nBalcony from $899"} placeholderTextColor="#64748b" value={pastedData} onChangeText={setPastedData} editable={!importing}/>
            ...
            <Pressable style={[styles.importButton, importing && styles.importButtonDisabled]} onPress={handleImport} disabled={importing || !pastedData.trim()}>
              {importing ? (<>
                  <ActivityIndicator size="small" color="#fff"/>
