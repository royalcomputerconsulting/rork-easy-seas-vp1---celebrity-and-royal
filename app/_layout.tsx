...
function FreshStartHandler({ onComplete }: {
    onComplete: () => void;
}) {
        ...
        const handleFirstLaunch = async () => {
            try {
                ...
                if (!hasLaunchedBefore) {
                    if (__DEV__)
                        console.log('[FreshStartHandler] First time user');
    ...
    return (<View style={freshStartStyles.container}>
...
function WebResponsiveWrapper({ children }: {
    ...
    return (<View style={webStyles.outerContainer}>
...
function RootLayoutNav() {
    ...
    return (<Stack screenOptions={screenOptions}>
      ...
      <Stack.Screen name="paywall-monthly" options={{
            presentation: "modal",
            headerShown: false,
            animation: 'slide_from_bottom' as const,
        }}/>
      ...
      <Stack.Screen name="day-agenda" options={{
            headerShown: false,
        }}/>
      <Stack.Screen name="offer-details" options={{
            presentation: "modal",
            headerShown: false,
            animation: 'slide_from_bottom' as const,
        }}/>
      <Stack.Screen name="add-machine-wizard" options={{
            presentation: "modal",
            headerShown: false,
            animation: 'slide_from_bottom' as const,
        }}/>
      <Stack.Screen name="add-machines-to-ship" options={{
            presentation: "modal",
            headerShown: false,
            animation: 'slide_from_bottom' as const,
        }}/>
      <Stack.Screen name="deck-plan" options={{
            headerShown: false,
        }}/>
      <Stack.Screen name="global-library" options={{
            presentation: "modal",
            headerShown: false,
            animation: 'slide_from_bottom' as const,
        }}/>
      <Stack.Screen name="machine-detail/[id]" options={{
            headerShown: false,
        }}/>
      <Stack.Screen name="edit-machine/[id]" options={{
            presentation: "modal",
            headerShown: false,
            animation: 'slide_from_bottom' as const,
        }}/>
      <Stack.Screen name="pricing-summary" options={{
            presentation: "modal",
            headerShown: false,
            animation: 'slide_from_bottom' as const,
        }}/>
      <Stack.Screen name="royal-caribbean-sync" options={{
            headerShown: true,
        }}/>
      <Stack.Screen name="carnival-sync" options={{
            headerShown: true,
        }}/>
    </Stack>);
}
...
function AppContentInner({ showSplash, setShowSplash, isClearing, setIsClearing }: {
    ...
    useEffect(() => {
        if (isAuthenticated && !initialCheckComplete && !forceSkipRestore) {
            const timeout = setTimeout(() => {
                if (__DEV__)
                    console.log('[AppContent] Cloud restore safety timeout');
    ...
    useEffect(() => {
        ...
        if (emailSyncDoneRef.current === authenticatedEmail)
            return;
    ...
    useEffect(() => {
        if (!isAuthenticated || !lastRestoreTime)
            return;
        if (lastRestoreHandledRef.current === lastRestoreTime)
            return;
    ...
    if (authLoading) {
        return (<WelcomeSplash onAnimationComplete={() => { }} duration={1500}/>);
    }
    if (!isAuthenticated) {
        if (showLandingPage) {
    ...
    if (showSplash) {
        return (<WelcomeSplash onAnimationComplete={handleSplashComplete} duration={800}/>);
    }
    if (isAuthenticated && !initialCheckComplete && !forceSkipRestore) {
        return (<View style={rootStyles.cloudRestoreContainer} testID="cloudRestoreScreen">
...
const DataProviders = composeProviders(CoreDataProvider, CrewRecognitionProvider, HistoricalPerformanceProvider, PriceHistoryProvider, PriceTrackingProvider, FinancialsProvider, LoyaltyProvider, SimpleAnalyticsProvider, DeckPlanProvider, CelebrityProvider);
const CasinoProviders = composeProviders(CasinoStrategyProvider, CasinoSessionProvider, SlotMachineProvider, SlotMachineLibraryProvider, MachineStrategyProvider, BankrollProvider, GamificationProvider, PPHAlertsProvider);
const ServiceProviders = composeProviders(TaxProvider, AlertsProvider, AgentXProvider, CertificatesProvider);
          ...
          <ErrorBoundary>
            <WebResponsiveWrapper>
              {isStorageReady ? (<AuthProvider>
                ...
                </AuthProvider>) : (<View style={rootStyles.storageBootstrapContainer}>
                ...
                : 'Checking your saved data and finishing startup.'}
                    ...
                    {storageError ? (<Text style={rootStyles.storageBootstrapError}>{storageError}</Text>) : null}
                  </View>
                </View>)}
            </WebResponsiveWrapper>
          </ErrorBoundary>
