...
(function() {
  ...
  // Try API-based extraction first (using Bearer token auth)
  async function fetchCourtesyHoldsViaAPI() {
    log('🔄 Step 3: Trying API-based courtesy holds extraction...', 'info');
    ...
    try {
      ...
      log('📡 Calling ' + endpoint + ' for courtesy holds...', 'info');
      ...
      if (data && data.payload && data.payload.profileBookings) {
        ...
        // Filter for courtesy holds only (status='OF', 'HD', 'HO')
        var holdStatuses = ['OF', 'HD', 'HO'];
        var holds = bookings.filter(function(b) {
          return holdStatuses.indexOf(b.bookingStatus) >= 0;
        });
        
        log('📊 API returned ' + bookings.length + ' total bookings, ' + holds.length + ' are courtesy holds', 'info');
        
        if (holds.length === 0) {
          log('ℹ️ No courtesy holds found in API response', 'info');
          return [];
        }
        
        // Fetch enrichment data for the holds
        var enrichmentMap = await fetchEnrichmentData(holds);
        
        // Convert API bookings to hold format
        var parsedHolds = [];
        for (var i = 0; i < holds.length; i++) {
          ...
          // Format offer expiration date if available
          var holdExpiration = '';
          ...
          var departurePort = enrichment.departurePortName || '';
          var itinerary = '';
