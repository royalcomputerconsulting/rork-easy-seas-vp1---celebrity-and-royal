...
assert.match(screen, /today-on-cruise-weather-card/);
assert.match(screen, /today-on-cruise-agenda-card/);
assert.match(screen, /today-on-cruise-essentials-card/);
assert.match(screen, /SailingWeatherCard/);
assert.match(booked, /booked-today-on-cruise-button/);
assert.match(booked, /pathname: '\/today-on-cruise'/);

console.log('PASS build396_today_on_cruise_regression — active/next voyage selection, cruise-day location, local agenda, essentials, weather reuse, and My Cruises navigation verified');
