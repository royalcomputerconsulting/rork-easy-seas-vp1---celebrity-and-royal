...
assert.match(commandCenter, /malformed \$\{skippedTruthRows === 1 \? 'record was' : 'records were'\} safely skipped/);
assert.match(casinoRoute, /class CasinoRouteErrorBoundary/);
assert.match(casinoRoute, /casino-safe-error-state/);
assert.match(casinoRoute, /<CasinoCommandCenter \/>/);
assert.match(casinoRoute, /casino-safe-data-health/);
assert.doesNotMatch(casinoRoute, /DeferredCasinoCommandCenter/);
assert.doesNotMatch(casinoRoute, /runAfterUiSettles/);
