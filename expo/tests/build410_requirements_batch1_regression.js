...
assert.equal(appJson.expo.version, '13.0.45');
assert.equal(appJson.expo.ios.buildNumber, '411');
assert.equal(appJson.expo.android.versionCode, 130068);
assert.match(pkg.scripts['verify:source-release'], /build410_requirements_batch1_regression\.js/);
...
for (const label of ['Intelligence', 'Charts', 'Session', 'Calcs']) {
...
assert.match(casinoScreen, /InteractionManager\.runAfterInteractions/, 'charts must defer until navigation settles');
