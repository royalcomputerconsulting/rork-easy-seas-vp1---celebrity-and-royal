...
assert.equal(eligible.length, 11, 'same-sailing cabins must share one calendar schedule and a courtesy hold must not become a sailing');
assert.equal(eligible.some((cruise) => cruise.reservationNumber === '1860399'), false, 'courtesy hold must not mark cruise days');
...
for (let day = 10; day <= 30; day += 1) {
  assert.equal(septemberCruiseDates.has(`2026-09-${String(day).padStart(2, '0')}`), true, `September ${day} must reflect a confirmed sailing`);
}
