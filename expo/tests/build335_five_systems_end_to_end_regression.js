...
const ask = compileTs('lib/askMyData.ts', {
  ...
  '@/lib/cruiseRecordIntegrity': { formatGuestEligibility: (count, fallback = '') => count == null ? fallback : `${count} guest${count === 1 ? '' : 's'}` },
  '@/lib/navigation/cruiseDetails': { buildCruiseDetailsPath: ({ id }) => `/cruise-details?id=${id}` },
  '@/lib/offers/offerInstanceIdentity': { buildOfferDetailsPath: ({ id }) => `/offer-details?id=${id}` },
});
const certificate = {
...
assert.match(royal, /runBoundedSyncCheckpoint\(\s*'(WRITE_|COMMIT_)/);
