...
assert.match(crewProvider, /runAfterUiSettles/, 'large import work must yield to navigation');
...
assert.match(trustCenter, /emitAppDataEvent\('cloudDataRestored'\)/);
...
assert.match(weather, /start_date=/);
assert.doesNotMatch(weather, /forecast_days=16&start_date=/, 'explicit forecast ranges must not send Open-Meteo mutually exclusive date parameters');
...
assert.match(weatherCard, /neither live forecast provider returned usable data/);
