...
assert.match(royalProvider, /canPersistSyncToTarget\([\s\S]*?syncOwnershipRef\.current,[\s\S]*?targetProfile\?\.id,[\s\S]*?targetProfile\?\.email,[\s\S]*?activeProfiles\.map\(\(profile\) => profile\.id\)/);
