...
assert.equal(USER_CONFIRMED_CURRENT_ROYAL_BOOKINGS.length, 13);
assert.equal(new Set(USER_CONFIRMED_CURRENT_ROYAL_BOOKINGS.map((row) => row.bookingId)).size, 13);
assert.equal(new Set(USER_CONFIRMED_CURRENT_ROYAL_BOOKINGS.map((row) => `${row.shipName}|${row.sailDate}`)).size, 12);
...
assert.deepEqual(
  ...
  { shipName: 'Icon of the Seas', sailDate: '2026-09-26', returnDate: '2026-10-03', nights: 7, cabinType: 'Interior' },
);
