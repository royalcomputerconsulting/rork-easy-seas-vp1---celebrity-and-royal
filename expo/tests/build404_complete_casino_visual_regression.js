...
for (const label of ['Intelligence', 'Charts', 'Session', 'Calcs']) assert.ok(ui.includes(`label: '${label}'`), `missing four-page tab ${label}`);
for (const feature of [
  'Current season intelligence', 'Historical annual value', 'Cruise portfolio',
  'Casino charts', 'Points by cruise', 'Cash result by cruise', 'Annual value capture',
  'Session tracking', 'Points-per-hour target', 'Session streaks',
  'Casino calculations', 'Coin-in volume', 'Theoretical loss', 'Average daily theo',
  'Points per hour', 'Value per hour', 'Value per session', 'Risk per hour',
  'Theo variance', 'Offer safety index', 'Sustainability', 'Press efficiency',
...
const rows = [
  { cruiseId: 'a', shipName: 'A', sailDate: '2026-04-01', points: evidence(1000), hours: evidence(10), coinIn: evidence(5000), theoreticalLoss: evidence(400), netGamingResult: evidence(500), seaDays: 2, portDays: 2 },
  { cruiseId: 'b', shipName: 'B', sailDate: '2026-05-01', points: evidence(2000), hours: evidence(20), coinIn: evidence(10000), theoreticalLoss: evidence(800), netGamingResult: evidence(-200), seaDays: 3, portDays: 2 },
];
