...
const royalCruise = { id:'star', shipName:'Star of the Seas', sailDate:'2026-07-05', returnDate:'2026-07-12', nights:7, brand:'royal', pointsEarned:6500, slotPointsConfirmed:true, casinoStartingCash:200, casinoEndingCash:2032, cashResult:1832, itinerary:[{day:1,port:'Miami',isSeaDay:false},{day:2,port:'At Sea',isSeaDay:true},{day:3,port:'Cozumel',isSeaDay:false},{day:4,port:'At Sea',isSeaDay:true}] };
...
for (const label of ['Intelligence', 'Charts', 'Play', 'Calcs']) assert.ok(casinoUi.includes(`label: '${label}'`));
assert.match(casinoUi, /Blue Chip Club resets every August 1/);
assert.match(casinoUi, /setTimeout\(\(\) => setChartsReady\(true\), 0\)/, 'charts must yield one turn without relying on InteractionManager');
...
assert.match(casinoUi, /Cruise-derived play/);
assert.match(casinoUi, /Optional actual sessions/);
