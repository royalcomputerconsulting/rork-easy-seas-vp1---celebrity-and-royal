...
assert.match(summary.footnotes.join(' '), /unassigned annual\/account points remain separate/);
assert.equal(pointTruth.CONFIRMED_CLUB_ROYALE_2026_POINTS, 23446, 'current season account total is not the four-Quantum subset');

const currentSubset = pointTruth.KNOWN_CURRENT_CLUB_ROYALE_CRUISES.reduce((sum, row) => sum + row.pointsEarned, 0);
assert.equal(currentSubset, 21460);
assert.equal(pointTruth.CONFIRMED_CLUB_ROYALE_2026_POINTS - currentSubset, 1986, 'certificate-backed current-season rows leave only unallocated reconciliation points instead of inventing fake cruise rows');
