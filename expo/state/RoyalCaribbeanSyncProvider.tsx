...
import type { CoreDataCruiseCommitReceipt } from './CoreDataProvider';
...
import {
  ...
  verifyBookedCruiseSyncReadback,
  verifySyncReadback,
  type SyncOwnershipSnapshot,
} from '@/lib/sync/syncRunIntegrity';
import { verifyProfileSyncReadback } from '@/lib/sync/profileSyncReadback';
...
import { getCruiseInventoryProvider } from '@/lib/cruiseInventory/cruiseCanonicalIdentity';
...
function scopeLoyaltyForCruiseLine(
  ...
  const common = {
    ...
    loyaltyObservedAt: data.loyaltyObservedAt,
    loyaltySourceEndpoint: data.loyaltySourceEndpoint,
...
const INITIAL_STATE: RoyalCaribbeanSyncState = {
  ...
  syncCounts: null,
  syncPreview: null,
  // Offer cards, casino availability, Golden Hours, and retail comparisons
  // all depend on the provider's cruise-search enrichment payload. Keep this
  // authoritative enrichment enabled for every Royal/Celebrity sync.
...
export const [RoyalCaribbeanSyncProvider, useRoyalCaribbeanSync] = createContextHook(() => {
  ...
  const resolvedSyncOwnerProfile = currentUser ?? users.find((profile) => profile.isOwner && profile.active !== false) ?? users.find((profile) => profile.active !== false) ?? null;
  currentSyncOwnerRef.current = {
    profileId: String(resolvedSyncOwnerProfile?.id ?? '').trim(),
  ...
  const readBackPersistedRows = useCallback(async (storageKey: string, expectedRows: unknown[], label: string) => {
    ...
    const report = label === 'booked-cruise'
      ? verifyBookedCruiseSyncReadback(expectedRows, storedRows)
      : verifySyncReadback(expectedRows, storedRows);
  ...
  const readBackCruiseInventory = useCallback(async (
    coreDataContext: any,
    expectedProviderRows: number,
    provider: string,
    receipt: CoreDataCruiseCommitReceipt | null,
  ) => {
    ...
    if (!integrity.reconciled || (expectedProviderRows > 0 && integrity.rawRows === 0)) {
      addLog(
    ...
    const providerReconciliations = receipt?.reconciliations.filter((entry) => entry.provider === provider) ?? [];
    const generationIds = providerReconciliations.map((entry) => entry.generationId);
    const missingActiveGenerationIds = generationIds.filter((generationId) => !receipt?.activeCounts.activeGenerationIds.includes(generationId));
    const providerCanonicalRows = providerReconciliations.reduce((sum, entry) => sum + entry.canonicalRows, 0);
    const providerReadbackRows = providerReconciliations.reduce((sum, entry) => sum + entry.readbackRows, 0);
    const providerAvailableRows = receipt?.activeCounts.byProvider[provider] ?? 0;
    const providerReceiptValid = expectedProviderRows === 0 || (
      providerReconciliations.length === 1
      && providerReconciliations.every((entry) => entry.state === 'active' && entry.persistedRows === entry.canonicalRows && entry.readbackRows === entry.canonicalRows)
      && missingActiveGenerationIds.length === 0
      && providerAvailableRows > 0
    );
    if (!providerReceiptValid) {
      addLog(
        `Provider generation readback failed for ${provider}: expected ${expectedProviderRows.toLocaleString()} source row(s), `
        + `${providerCanonicalRows.toLocaleString()} canonical and ${providerReadbackRows.toLocaleString()} read back across ${generationIds.length} generation(s).`,
        'error',
      );
      throw new Error(`SYNC_PROVIDER_GENERATION_READBACK_FAILED:${provider}`);
    }
    addLog(
      `Cruise database verified: ${providerAvailableRows.toLocaleString()} ${provider} offer-sailing option row(s) from generation ${generationIds.join(', ') || 'preserved'}; `
      + `${providerCanonicalRows.toLocaleString()} canonical inventory row(s) read back. Grand total: ${integrity.durableSourceRows.toLocaleString()} available row(s).`,
    ...
    return {
      ...
      provider,
      generationIds,
      providerAvailableRows,
      providerCanonicalRows,
      providerReadbackRows,
      readBackAt: receipt?.readBackAt ?? new Date().toISOString(),
    };
  }, [addLog]);
    ...
    switch (msgType) {
      ...
      case 'loyalty_data':
        if (msg.loyalty && typeof msg.loyalty === 'object') {
          ...
          const converted = {
            ...convertLoyaltyInfoToExtended(loyaltyInfo, ''),
            lastSyncTimestamp: new Date().toISOString(),
            loyaltyObservedAt: new Date().toISOString(),
            loyaltySourceEndpoint: String((msg as any).sourceUrl ?? (msg as any).url ?? 'webview-message:loyalty_data'),
          };
      ...
      case 'network_payload': {
        ...
        if (endpoint === 'loyalty' && data) {
          ...
          const convertedLoyalty = {
            ...convertLoyaltyInfoToExtended(loyaltyInfo, accountId),
            lastSyncTimestamp: new Date().toISOString(),
            loyaltyObservedAt: new Date().toISOString(),
            loyaltySourceEndpoint: typeof url === 'string' && url.length > 0 ? url : 'webview-network-capture:loyalty',
          };
  ...
  const runIngestion = useCallback(async () => {
    ...
    if (!resolvedSyncOwnerProfile?.id || !authenticatedEmail) {
  ...
  const syncToApp = useCallback(async (coreDataContext: any, loyaltyContext: any, providedExtendedLoyalty?: ExtendedLoyaltyData | null, targetOptions?: SyncTargetOptions) => {
    ...
    const isActiveSyncTarget = Boolean(targetProfile?.id && currentUser?.id && targetProfile.id === currentUser.id);
    ...
    if (!canPersistSyncToTarget(
      syncOwnershipRef.current,
      targetProfile?.id,
      targetProfile?.email,
    ...
    const syncPublicationWarnings: string[] = [];
    ...
    try {
      ...
      const finalProviderAvailableCruises = finalCruises.filter((cruise) => getCruiseInventoryProvider(cruise) === syncSource);
      ...
      let cruiseCommitReceipt: CoreDataCruiseCommitReceipt | null = null;
      const cruiseCommitOptions = {
        ...datasetCommitOptions('cruises'),
        cruiseInventoryProviders: [syncSource],
        onCruiseInventoryCommitted: (receipt: CoreDataCruiseCommitReceipt) => {
          cruiseCommitReceipt = receipt;
        },
      ...
      // These datasets share the same local Health Trust repository and
      // coordinated storage layer. Commit them in order so iOS never receives
      // overlapping BEGIN statements and each checkpoint has an unambiguous
      // durable result before the next dataset starts.
      if (offersRequireCommit) {
        await runBoundedSyncCheckpoint(
          'COMMIT_OFFERS',
          () => coreDataContext.setCasinoOffers(finalOffers, datasetCommitOptions('offers')),
          addLog,
          45000,
        );
      }
      if (cruisesRequireCommit) {
        await runBoundedSyncCheckpoint(
          'COMMIT_AVAILABLE_CRUISES',
          () => coreDataContext.setCruises(finalCruises, cruiseCommitOptions),
          addLog,
          45000,
        );
      }
      if (bookingsRequireCommit) {
        await runBoundedSyncCheckpoint(
          'COMMIT_BOOKINGS_HISTORY',
          () => coreDataContext.setBookedCruises(finalBookedCruises, datasetCommitOptions('bookedCruises')),
          addLog,
          45000,
        );
      }
      ...
      await yieldSyncUi();
      const [offerReadback, cruiseInventoryReadback, bookedReadback] = await Promise.all([
        ...
        runBoundedSyncCheckpoint(
          'READBACK_AVAILABLE_CRUISES',
          () => readBackCruiseInventory(coreDataContext, finalProviderAvailableCruises.length, syncSource, cruiseCommitReceipt),
          addLog,
          30000,
        ),
      ...
      if (syncSource === 'royal' && isActiveSyncTarget && preview.loyalty) {
        ...
        try {
          ...
          syncPublicationWarnings.push(`loyalty points: ${String(loyaltyError)}`);
          addLog(`⚠️ Warning: Failed to update loyalty points: ${String(loyaltyError)}`, 'warning');
        }
      } else if (syncSource !== 'carnival' && !isActiveSyncTarget) {
        addLog('A different profile is the sync target — loyalty totals will be saved to that profile without changing the active profile', 'info');
      }
      
      if (syncSource !== 'carnival' && isActiveSyncTarget && effectiveExtendedLoyalty && loyaltyContext.setExtendedLoyaltyData) {
        try {
          ...
          syncPublicationWarnings.push(`extended loyalty: ${String(extLoyaltyError)}`);
      ...
      if (syncSource !== 'carnival' && targetProfile && updateUserProfile) {
        try {
          ...
          if (syncSource === 'royal') {
            ...
            profileUpdates.clubRoyaleLastSyncAt = new Date().toISOString();
            if (effectiveExtendedLoyalty?.loyaltySourceEndpoint) profileUpdates.clubRoyaleLoyaltySourceEndpoint = effectiveExtendedLoyalty.loyaltySourceEndpoint;
          } else if (syncSource === 'celebrity') {
            profileUpdates.preferredBrand = 'celebrity';
            profileUpdates.celebrityLastSyncAt = new Date().toISOString();
            if (effectiveExtendedLoyalty?.loyaltySourceEndpoint) profileUpdates.celebrityLoyaltySourceEndpoint = effectiveExtendedLoyalty.loyaltySourceEndpoint;
          }
          ...
          if (Object.keys(profileUpdates).length > 0) {
            ...
            const persistedUsers = await quotaSafeGetJsonItem<unknown>(
              getUserScopedKey(ALL_STORAGE_KEYS.USERS, authenticatedEmail),
              [],
            );
            const profileReadback = verifyProfileSyncReadback(persistedUsers, targetProfile.id, profileUpdates);
            if (!profileReadback.verified) {
              throw new Error(`PROFILE_READBACK_FAILED:${profileReadback.mismatchedFields.join(',') || 'target_missing'}`);
            }
            addLog(`✅ ${targetSlotLabel} profile updated and read back (${profileReadback.checkedFields.length} field(s))`, 'success');
          } else {
          ...
          syncPublicationWarnings.push(`profile readback: ${String(profileSyncError)}`);
      ...
      if (cruiseLine === 'carnival' && carnivalUserDataRef.current) {
        try {
          ...
          if (targetProfile && updateUserProfile) {
            ...
            const profileReadback = verifyProfileSyncReadback(persistedUsers, targetProfile.id, expectedCarnivalProfile);
            if (!profileReadback.verified) {
              throw new Error(`CARNIVAL_PROFILE_READBACK_FAILED:${profileReadback.mismatchedFields.join(',') || 'target_missing'}`);
            }
          ...
          syncPublicationWarnings.push(`Carnival loyalty readback: ${String(carnivalLoyaltyError)}`);
      ...
      // The transaction writes authoritative local storage and SQLite first.
      // Publish that verified commit back into the live providers so Offers,
      // Cruises, Booked, loyalty summaries, and inventory subscribers update
      // immediately without requiring an app restart. refreshData is now
      // strictly local-first and does not restore stale backend data.
      await runBoundedSyncCheckpoint(
      ...
      addLog('✅ Synced offers, cruises, and bookings published to the active app profile.', 'success');
      addLog(
        `✅ COMMITTED READBACK ${transactionManifest.runId}: ${offerReadback.matchedRows.toLocaleString()} offer record(s), `
        + `${cruiseInventoryReadback.providerAvailableRows.toLocaleString()} ${syncSource} sailing option row(s), `
        + `${bookedReadback.matchedRows.toLocaleString()} booked/history row(s).`,
        'success',
      );
      appendDiagnosticJournal('SYNC_COMMITTED_READBACK_VERIFIED', {
        runId: transactionManifest.runId,
        source: syncSource,
        committedAt: transactionManifest.committedAt,
        inventoryGenerationIds: cruiseInventoryReadback.generationIds,
        offerRows: offerReadback.matchedRows,
        providerAvailableCruiseRows: cruiseInventoryReadback.providerAvailableRows,
        providerCanonicalRows: cruiseInventoryReadback.providerCanonicalRows,
        bookedRows: bookedReadback.matchedRows,
        grandTotalAvailableCruiseRows: cruiseInventoryReadback.durableSourceRows,
      });

      const carnivalOutcome = syncSource === 'carnival' ? state.syncCounts?.carnivalOutcome : undefined;
      const availableCruiseRowsAfterReconciliation =
        cruiseInventoryReadback?.providerAvailableRows
        ?? counts.canonicalRows
        ?? finalCruises.length;
      const committedRunId = transactionManifest.runId;
      const committedAt = transactionManifest.committedAt;
      const finalStatus: SyncStatus = carnivalOutcome === 'partial'
        ? 'partial'
        : carnivalOutcome === 'complete_with_warnings'
          ? 'complete_with_warnings'
          : handoffEvidence.unaccountedRows > 0 || syncPublicationWarnings.length > 0
            ? 'complete_with_warnings'
            : 'complete';
      ...
      addLog(
        finalStatus === 'complete'
          ...
          : syncSource === 'carnival' && carnivalOutcome === 'complete_with_warnings'
            ? '✅ Carnival offers and sailings saved; optional loyalty evidence is incomplete.'
            : `⚠️ Sync data was saved with incomplete evidence clearly marked${syncPublicationWarnings.length > 0 ? ` (${syncPublicationWarnings.length} publication warning${syncPublicationWarnings.length === 1 ? '' : 's'})` : ''}.`,
        finalStatus === 'complete' || (syncSource === 'carnival' && carnivalOutcome === 'complete_with_warnings') ? 'success' : 'warning',
      );
      ...
      setState(prev => ({ 
        ...
        lastSyncTimestamp: new Date().toISOString(),
        syncCounts: {
          commitRunId: committedRunId,
          committedAt,
          inventoryGenerationIds: cruiseInventoryReadback.generationIds,
          offerCount: incomingUniqueOfferCount,
          offerRows: normalizedOffers.length,
          storageOfferReadbackRows: offerReadback.matchedRows,
          ...
          availableCruiseRows: availableCruiseRowsAfterReconciliation,
          physicalSailingRows: cruiseInventoryReadback?.providerReadbackRows ?? finalProviderAvailableCruises.length,
          offerSailingRelationships: cruiseInventoryReadback?.providerAvailableRows ?? counts.retainedVariants,
          grandTotalAvailableCruiseRows: cruiseInventoryReadback?.durableSourceRows ?? finalCruises.length,
          storageBookedReadbackRows: bookedReadback.matchedRows,
          totalImportedCruises: availableCruiseRowsAfterReconciliation,
      ...
      console.log('[RoyalCaribbeanSync] Sync complete - verified local commit published to live app state');
      ...
      if (datasetWritesStarted && !localTransactionCommitted) {
        ...
        try {
          await runBoundedSyncCheckpoint(
            'ROLLBACK_OFFERS',
            () => coreDataContext.setCasinoOffers(rollbackSnapshot.offers),
            addLog,
            45000,
          );
          await runBoundedSyncCheckpoint(
            'ROLLBACK_AVAILABLE_CRUISES',
            () => coreDataContext.setCruises(rollbackSnapshot.cruises, { cruiseInventoryProviders: [syncSource] }),
            addLog,
            45000,
          );
          await runBoundedSyncCheckpoint(
            'ROLLBACK_BOOKINGS',
            () => coreDataContext.setBookedCruises(rollbackSnapshot.bookedCruises),
            addLog,
            45000,
          );
