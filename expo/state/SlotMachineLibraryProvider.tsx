...
export const [SlotMachineLibraryProvider, useSlotMachineLibrary] = createContextHook(() => {
  ...
  // The atlas is deliberately loaded on focus after navigation settles. Do
  // not render a permanent startup spinner before that deferred task begins;
  // initializeAndLoadData owns the true loading window.
  ...
  const loadMachinesFromIndex = useCallback(async (
    ...
    try {
      ...
      await replaceHighVolumeDomain(authenticatedEmail || 'local-default', 'machine_encyclopedia', updatedEncyclopedia, encyclopediaKeyRef.current);
  ...
  const ensureEncyclopediaFullyLoadedForPro = useCallback(async (
    ...
    try {
      ...
      await replaceHighVolumeDomain(authenticatedEmail || 'local-default', 'machine_encyclopedia', updated, encyclopediaKeyRef.current);
    ...
    try {
      ...
      try {
        [loadedEncyclopedia, loadedAtlasIds, indexLoaded] = await Promise.all([
          listHighVolumeDomain<MachineEncyclopediaEntry>(authenticatedEmail || 'local-default', 'machine_encyclopedia'),
          listHighVolumeDomain<string>(authenticatedEmail || 'local-default', 'slot_atlas'),
          AsyncStorage.getItem(STORAGE_KEY_INDEX_LOADED),
        ]);
        if (loadedEncyclopedia.length === 0) {
          loadedEncyclopedia = await quotaSafeGetJsonItem<MachineEncyclopediaEntry[]>(encyclopediaKeyRef.current, [], Array.isArray);
          if (loadedEncyclopedia.length > 0) await replaceHighVolumeDomain(authenticatedEmail || 'local-default', 'machine_encyclopedia', loadedEncyclopedia, encyclopediaKeyRef.current);
        }
        if (loadedAtlasIds.length === 0) {
          loadedAtlasIds = await quotaSafeGetJsonItem<string[]>(atlasKeyRef.current, [], Array.isArray);
          if (loadedAtlasIds.length > 0) await replaceHighVolumeDomain(authenticatedEmail || 'local-default', 'slot_atlas', loadedAtlasIds, atlasKeyRef.current);
        }
      } catch (storageError) {
  ...
  const saveEncyclopedia = async (data: MachineEncyclopediaEntry[]) => {
    try {
      await replaceHighVolumeDomain(authenticatedEmail || 'local-default', 'machine_encyclopedia', data, encyclopediaKeyRef.current);
  ...
  const saveAtlas = async (ids: string[]) => {
    try {
      await replaceHighVolumeDomain(authenticatedEmail || 'local-default', 'slot_atlas', ids, atlasKeyRef.current);
