...
interface CasinoSessionState {
  ...
  getSessionAnalytics: (options?: { includeGenerated?: boolean; program?: CasinoSession['program']; includeUnprogrammedSessionIds?: string[]; dateFrom?: string; dateTo?: string; profileId?: string; includeUnassignedForProfile?: boolean }) => SessionAnalytics;
...
export const [CasinoSessionProvider, useCasinoSessions] = createContextHook((): CasinoSessionState => {
  ...
  const persistSessions = useCallback(async (newSessions: CasinoSession[]) => {
    try {
      await replaceHighVolumeDomain(authenticatedEmail || 'local-default', 'casino_sessions', newSessions, storageKeyRef.current);
  ...
  const loadSessions = useCallback(async () => {
    try {
      setIsLoading(true);
      const stored = await hydrateHighVolumeDomain<CasinoSession>(authenticatedEmail || 'local-default', 'casino_sessions');
      setSessions(stored);
  ...
  useEffect(() => {
    if (authenticatedEmail !== lastEmailRef.current) {
      ...
      if (previousEmail !== null && previousEmail !== authenticatedEmail) {
        console.log('[CasinoSessionProvider] User changed from', previousEmail, 'to', authenticatedEmail, '- resetting sessions');
        setSessions([]);
      }
    }
    
    void loadSessions();
  }, [loadSessions, authenticatedEmail]);

  useEffect(() => {
    const handleDataCleared = () => {
      console.log('[CasinoSessionProvider] Data cleared event detected, resetting sessions');
      setSessions([]);
      setIsLoading(false);
    };

    const handleCloudRestore = () => {
      console.log('[CasinoSessionProvider] Cloud data restored, reloading sessions');
      void loadSessions();
    };
    ...
    try {
      ...
      console.log('[CasinoSessionProvider] Could not set up event listeners:', e);
    }
  }, [loadSessions]);
  ...
  const getSessionAnalytics = useCallback((options?: { includeGenerated?: boolean; program?: CasinoSession['program']; includeUnprogrammedSessionIds?: string[]; dateFrom?: string; dateTo?: string; profileId?: string; includeUnassignedForProfile?: boolean }): SessionAnalytics => {
    ...
    const analyzedSessions = sessions.filter((session) => {
      ...
      if (options?.dateFrom && (!sessionDate || sessionDate < options.dateFrom)) return false;
      if (options?.dateTo && (!sessionDate || sessionDate > options.dateTo)) return false;
      if (options?.profileId) {
        if (session.pointEarningProfileId && session.pointEarningProfileId !== options.profileId) return false;
        if (!session.pointEarningProfileId && !options.includeUnassignedForProfile) return false;
      }
  ...
  return useMemo(() => ({
    ...
    addQuickMachineWin,
    reload,
  }), [sessions, isLoading, addSession, removeSession, updateSession, getSessionsForDate, getDailySummary, getTotalPlayedForDate, clearSessionsForDate, getSessionAnalytics, getSessionsByMachineType, getSessionsByDenomination, getSessionsByMachine, getMachineAnalytics, generateHistoricalSessions, hasSessionsForCruise, getTotalPointsForCruise, clearAllAutoGeneratedSessions, addQuickMachineWin, reload]);
});
