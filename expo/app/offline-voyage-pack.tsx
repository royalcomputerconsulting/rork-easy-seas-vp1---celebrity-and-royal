    ...
    try {
      ...
      const details = [
        weatherReport.refreshedDates.length > 0
          ? `${weatherReport.refreshedDates.length} weather day${weatherReport.refreshedDates.length === 1 ? '' : 's'} saved: ${weatherReport.datedForecastDates.length} dated provider forecast${weatherReport.datedForecastDates.length === 1 ? '' : 's'} and ${weatherReport.planningDates.length} planning/current-area card${weatherReport.planningDates.length === 1 ? '' : 's'}.`
          : availableFrom
      ...
      <SafeAreaView style={s.safe} edges={['top']}>
        ...
        <ScrollView contentContainerStyle={s.content}>
          <TabIdentityBand
            tab="booked"
            compact
            detail={cruise ? `${cruise.shipName} · ${status?.completionPercent ?? 0}% ready offline` : 'No upcoming voyage selected'}
            testID="offline-voyage-pack-story-card"
          />
          <ThemedSectionHeader
            tab="booked"
            emoji="📦"
            title="Offline voyage readiness"
            subtitle="Weather, itinerary, certificates, deck plans, and calendar records retained for the sailing."
            tone="info"
            compact
            style={s.sectionHeader}
            testID="offline-voyage-pack-section"
          />
          {!cruise || !status ? <View style={s.card}><Text style={s.muted}>No upcoming cruise is available.</Text></View> : <>
            ...
            {status.sections.map((section) => <View key={section.key} style={s.card} testID={`offline-pack-${section.key}`}>
              <View style={s.row}>
                {section.ready ? <CheckCircle2 size={18} color="#4EC0A5" /> : <TriangleAlert size={18} color="#FBBF24" />}
                <View style={s.copy}>
...
const s = StyleSheet.create({
  root: { flex: 1, backgroundColor: C.background }, safe: { flex: 1 },
