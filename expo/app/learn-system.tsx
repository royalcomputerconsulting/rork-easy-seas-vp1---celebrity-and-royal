...
function getLearnGroupAccent(groupTitle: string): string {
  const index = TOPIC_GROUPS.findIndex((group) => group.groupTitle === groupTitle);
  return ['#167C80', '#1C2F7A', '#B7892E'][Math.max(0, index) % 3];
}

function TopicGroupSection({ group }: { group: typeof TOPIC_GROUPS[0] }) {
  const [expanded, setExpanded] = useState(false);
  const groupAccent = getLearnGroupAccent(group.groupTitle);

  return (
    <View style={groupStyles.container}>
      <TouchableOpacity
        style={[groupStyles.groupHeader, { borderLeftColor: groupAccent }]}
        onPress={() => setExpanded(!expanded)}
        activeOpacity={0.75}
      >
        <Text style={groupStyles.groupTitle}>{group.groupTitle}</Text>
        <View style={[groupStyles.countBadge, { backgroundColor: `${groupAccent}18` }]}> 
          <Text style={[groupStyles.countText, { color: groupAccent }]}>{group.topics.length}</Text>
        </View>
        {expanded
          ? <ChevronDown size={16} color={groupAccent} />
            ...
            return (
              <View key={topic.id} style={styles.topicCard} testID={`learn-system-topic-${topic.id}`}>
                <View style={styles.topicHeader}>
                  <View style={[styles.topicIcon, { backgroundColor: `${groupAccent}18` }]}> 
                    <TopicIcon size={18} color={groupAccent} />
                  </View>
                ...
                <View style={styles.topicBullets}>
                  {topic.bullets.map((bullet) => (
                    <View key={bullet} style={styles.bulletRow}>
                      <View style={[styles.bulletDot, { backgroundColor: groupAccent }]} />
...
export default function LearnSystemScreen() {
  ...
  return (
    <View style={styles.container}>
      ...
      <LinearGradient colors={['#E8F3F7', '#FBF8F2', '#F2EEE6']} style={StyleSheet.absoluteFill} />
      <SafeAreaView style={styles.safeArea} edges={['top']}>
        <View style={styles.header}>
          <View style={styles.headerTitleRow}>
            <View style={styles.headerIcon}>
              <BookOpen size={22} color="#0E7FA7" />
            </View>
        ...
        <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
          <View style={styles.heroCard} testID="learn-system-hero">
            ...
            <Text style={styles.heroBody}>Browse profiles, offers, cruises, loyalty, certificates, casino tools, Agent SEA, and data recovery. Open only the section you need.</Text>
          </View>

          {/* Quick navigation chips */}
          <View style={styles.quickNavRow}>
            {TOPIC_GROUPS.map((group) => (
              <View key={group.groupTitle} style={[styles.quickNavChip, { borderColor: getLearnGroupAccent(group.groupTitle) }]}> 
                <Text style={[styles.quickNavText, { color: getLearnGroupAccent(group.groupTitle) }]}>{group.groupTitle}</Text>
              </View>
            ))}
          </View>
