...
import { shiftAgendaDate } from '@/lib/calendar/agendaNavigation';
import { buildCruiseDetailsParams } from '@/lib/navigation/cruiseDetails';
import { selectPrimaryVoyageWeather, selectVoyageWeatherBlock } from '@/lib/calendar/voyageWeatherSelection';
import { ThemedSectionHeader } from '@/components/ui/ThemedSectionCard';
import { AppleCalendarSchedule } from '@/components/AppleCalendarSchedule';
import type { AppleCalendarAgendaEvent } from '@/lib/calendar/appleCalendarSchedule';
...
export default function DayAgendaScreen() {
  ...
  const [appleCalendarEvents, setAppleCalendarEvents] = useState<AppleCalendarAgendaEvent[]>([]);
  ...
  const moveAgendaDay = useCallback((offset: number) => {
    ...
    const nextDate = shiftAgendaDate(currentDateKey, offset);
  ...
  const mergedCruiseBookings = useMemo((): MergedCruiseData[] => {
    ...
    normalizedBookedCruises.forEach((cruise: BookedCruise) => {
      ...
      if (cruiseMap.has(key)) {
        ...
        cruiseMap.set(key, {
          id: cruise.id,
          shipName: cruise.shipName || 'Ship not stated',
  ...
  const getCasinoContext = useCallback((cruise: MergedCruiseData, dayNum: number): CasinoDayContext => {
    ...
    return {
      ...
      port: currentDay?.port || 'Port not stated',
    };
  }, []);
  ...
  const handleItemPress = useCallback((item: AgendaItem) => {
    if (item.type === 'cruise' && 'sailDate' in item.data) {
      ...
      router.push({
        ...
        params: buildCruiseDetailsParams(cruiseData, { source: 'calendar' }),
      });
    }
  }, [router]);
  ...
  const opportunePlayingTimes = useMemo((): TimelineEvent[] => {
    ...
    const casinoEvents = timelineEvents.filter(e => e.type === 'casino' && e.startTime && e.color !== '#676A70');
  ...
  const weatherVoyages = useMemo(
    () => selectVoyageWeatherBlock(allWeatherCruises, dateStr),
    [allWeatherCruises, dateStr],
  );
  const primaryWeatherVoyage = useMemo(
    () => selectPrimaryVoyageWeather(weatherVoyages, dateStr),
    [dateStr, weatherVoyages],
  );
  ...
  const dayScheduleData = useMemo(() => {
    ...
    appleCalendarEvents.forEach((event) => {
      const sourceLabel = `Apple Calendar • ${event.calendarTitle}`;
      if (event.allDay) {
        addAllDayItem({
          id: `apple-calendar-all-day-${event.id}`,
          title: event.title,
          subtitle: event.location || sourceLabel,
          detail: sourceLabel,
          color: event.calendarColor || EVENT_COLORS.personal,
          icon: 'calendar',
        });
        return;
      }
      const startMinutes = event.startDate.getHours() * 60 + event.startDate.getMinutes();
      const rawEndMinutes = event.endDate.getHours() * 60 + event.endDate.getMinutes();
      addScheduleBlock({
        id: `apple-calendar-timed-${event.id}`,
        title: event.title,
        subtitle: sourceLabel,
        location: event.location,
        startMinutes,
        endMinutes: rawEndMinutes > startMinutes ? rawEndMinutes : Math.min(MINUTES_PER_DAY, startMinutes + 30),
        color: event.calendarColor || EVENT_COLORS.personal,
        icon: 'calendar',
        type: 'personal',
      });
  ...
  }, [appleCalendarEvents, calendarEvents, getCasinoContext, getDayOfCruise, getItineraryForDay, mergedCruiseBookings, selectedDate, selectedDayWindow.end, selectedDayWindow.start]);
    ...
    <View style={styles.container}>
      ...
      <SafeAreaView style={styles.safeArea} edges={['top', 'bottom']}>
        <ResponsiveContainer>
          ...
          >
            <ChevronLeft size={24} color="#1C2F7A" />
          </TouchableOpacity>
          <View style={styles.dateCopy}>
            <Text style={styles.dateText} numberOfLines={1} adjustsFontSizeToFit minimumFontScale={0.82}>{formattedDate}</Text>
            <Text style={styles.eventCount}>{agendaItems.length + appleCalendarEvents.length} {(agendaItems.length + appleCalendarEvents.length) === 1 ? 'event' : 'events'}</Text>
          </View>
          <TouchableOpacity style={styles.dayArrow} onPress={() => moveAgendaDay(-1)} accessibilityLabel="Previous day agenda" testID="day-agenda-previous-day"><ChevronLeft size={18} color="#1C2F7A" /></TouchableOpacity>
          <TouchableOpacity style={styles.dayArrow} onPress={() => moveAgendaDay(1)} accessibilityLabel="Next day agenda" testID="day-agenda-next-day"><ChevronRight size={18} color="#1C2F7A" /></TouchableOpacity>
          <TouchableOpacity
            ...
            disabled={isSyncing}
            accessibilityRole="button"
            accessibilityLabel={isSyncing ? 'Refreshing day agenda' : 'Refresh day agenda'}
            testID="day-agenda-refresh"
          >
            <RefreshCcw size={20} color={isSyncing ? '#A8B3BC' : '#1C2F7A'} />
          </TouchableOpacity>
        </View>
        ...
        >
          <ResponsiveContainer>
            <TabIdentityBand
              ...
              detail={`${agendaItems.length + appleCalendarEvents.length} ${(agendaItems.length + appleCalendarEvents.length) === 1 ? 'event' : 'events'} · weather, plans, and casino windows`}
              dense
              testID="day-agenda-identity"
            />
            ...
            <TouchableOpacity
              ...
              onPress={() => router.push('/easy-seas-home' as any)}
              activeOpacity={0.82}
              accessibilityLabel="Open today's priorities"
              testID="day-agenda-todays-priorities"
            >
              <View style={styles.prioritiesIcon}><ListChecks size={20} color="#0E7FA7" /></View>
              <View style={styles.prioritiesCopy}>
                <Text style={styles.prioritiesTitle}>Today’s Priorities</Text>
                <Text style={styles.prioritiesSubtitle}>{agendaItems.length + appleCalendarEvents.length} scheduled · {dayScheduleData.allDayItems.length} all-day · {formatDurationLabel(dayScheduleData.casinoMinutes)} casino availability</Text>
              </View>
              <ChevronRight size={20} color="#1C2F7A" />
            </TouchableOpacity>
          ...
          <View style={styles.sectionContainer}>
            <ThemedSectionHeader
              ...
              emoji="🌦️"
              title="Weather"
              subtitle={mergedCruiseBookings.length > 0
                ? 'Daily wind, waves, temperature, and sea-state snapshots saved for offline access.'
                : allWeatherCruises.length > 0
                  ? 'The next voyage is pinned here with the latest available forecast cards.'
                  : 'Add a booked cruise to unlock daily wind, waves, temperature, and sea-state snapshots.'}
              compact
              testID="day-agenda-weather-heading"
            />
            {primaryWeatherVoyage ? (
              <View style={styles.weatherVoyageBlock} testID="agenda-weather-voyage-block">
                <VoyageWeatherSection
                  key={`${primaryWeatherVoyage.shipName}-${primaryWeatherVoyage.sailDate}-${primaryWeatherVoyage.returnDate}`}
                  cruise={primaryWeatherVoyage}
          ...
          <View style={styles.sectionContainer}>
            <ThemedSectionHeader
              tab="calendar"
              emoji="🕐"
              title="24-hour agenda"
              subtitle="Port windows, casino availability, and every timed plan for this day."
              compact
            ...
            <View style={styles.dayScheduleCard}>
              ...
              <AppleCalendarSchedule
                agendaDate={selectedDate}
                ownerProfileId={currentUser?.id}
                embedded
                onEventsChange={setAppleCalendarEvents}
              />
          ...
          {agendaItems.length > 0 && (
            <View style={styles.sectionContainer}>
              <ThemedSectionHeader tab="calendar" emoji="🚢" title="Cruises & events" compact testID="day-agenda-events-heading" />
          ...
          {opportunePlayingTimes.length > 0 && (
            <View style={styles.sectionContainer}>
              <ThemedSectionHeader tab="calendar" emoji="🎰" title="Opportune playing times" tone="casino" compact testID="day-agenda-golden-hours-heading" />
          ...
          {timelineEvents.length > 0 && (
            <View style={styles.sectionContainer}>
              <ThemedSectionHeader tab="calendar" emoji="📍" title="Day timeline" compact testID="day-agenda-timeline-heading" />
          ...
          <View style={styles.sectionContainer}>
            <ThemedSectionHeader tab="calendar" emoji="✨" title="Daily luck" compact testID="day-agenda-luck-heading" />
...
const styles = StyleSheet.create({
  ...
  dateCopy: { flex: 1 },
  dayArrow: { width: 44, height: 44, borderRadius: 22, alignItems: 'center', justifyContent: 'center', backgroundColor: '#FFFDF9', borderWidth: 1, borderColor: '#D9E1E6' },
  dateText: {
  ...
  prioritiesIcon: { width: 44, height: 44, borderRadius: 22, alignItems: 'center', justifyContent: 'center', backgroundColor: '#E8F7F5' },
  prioritiesCopy: { flex: 1, minWidth: 0 },
  prioritiesTitle: { fontFamily: TYPOGRAPHY.fontFamilyEditorialSemibold, fontSize: 20, color: '#17324D' },
  prioritiesSubtitle: { marginTop: 3, fontSize: 12, lineHeight: 17, color: '#66737F' },
  weatherVoyageBlock: {
    gap: SPACING.md,
  },
  weatherBlockNotice: {
  ...
  weatherBlockNoticeTitle: {
    color: '#17324D',
    fontSize: TYPOGRAPHY.fontSizeSM,
    fontWeight: TYPOGRAPHY.fontWeightBold,
  },
  weatherBlockNoticeText: {
    color: '#66737F',
