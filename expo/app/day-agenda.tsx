...
import { ChevronLeft, Ship, Plane, User, Calendar, Clock, MapPin, Anchor, Waves, Users, Dices, RefreshCcw, } from 'lucide-react-native';
...
export default function DayAgendaScreen() {
    ...
    const renderDayStatusBadge = useCallback((status: 'start' | 'middle' | 'end' | 'single') => {
        ...
        return (<View style={styles.dayStatusBadge}>
    ...
    const calculateOverlap = useCallback((start1: string, end1: string, start2: string, end2: string): {
        start: string;
        end: string;
    } | null => {
    ...
    const opportunePlayingTimes = useMemo((): TimelineEvent[] => {
        ...
        casinoEvents.forEach(casinoEvent => {
            ...
            enabledSessions.forEach(session => {
                ...
                if (overlap) {
                    const existingOpportune = opportuneTimes.find(t => t.startTime === overlap.start && t.endTime === overlap.end);
    ...
    const renderTimelineEvent = useCallback((event: TimelineEvent) => {
        ...
        <View style={styles.timelineTime}>
          {event.startTime ? (<Text style={[
                    styles.timelineTimeText,
                    isOpportune && styles.timelineTimeTextOpportune,
                ]}>{event.startTime}</Text>) : event.isAllDay ? (<Text style={styles.timelineAllDay}>All Day</Text>) : null}
    ...
    const renderAgendaItem = useCallback((item: AgendaItem) => {
        ...
        return (<TouchableOpacity key={item.id} style={styles.agendaItem} activeOpacity={isCruise ? 0.7 : 1} onPress={() => handleItemPress(item)}>
        ...
        <View style={styles.itemContent}>
          <View style={styles.itemHeader}>
            ...
            {item.isAllDay ? (<View style={styles.allDayBadge}>
                <Text style={styles.allDayText}>Day {dayNum}</Text>
              </View>) : item.startTime && (<View style={styles.timeContainer}>
    ...
    return (<View style={styles.container}>
      ...
      <SafeAreaView style={styles.safeArea} edges={['top']}>
        <View style={styles.header}>
          ...
          <TouchableOpacity style={styles.syncButton} onPress={handleSyncEvents} activeOpacity={0.7} disabled={isSyncing}>
        ...
        <ScrollView style={styles.scrollView} contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
          {agendaItems.length === 0 && timelineEvents.length === 0 ? (<View style={styles.emptyState}>
