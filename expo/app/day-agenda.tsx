...
import { selectVoyageWeatherBlock } from '@/lib/calendar/voyageWeatherSelection';
      ...
      <SafeAreaView style={styles.safeArea} edges={['top', 'bottom']}>
        <ResponsiveContainer>
          ...
          <View style={styles.dateHeader}>
            <TouchableOpacity style={styles.dayArrow} onPress={() => moveAgendaDay(-1)} accessibilityLabel="Previous day agenda" testID="day-agenda-previous-day"><ChevronLeft size={20} color="#1C2F7A" /></TouchableOpacity>
            <View style={styles.dateCopy}>
              <Text style={styles.dateText}>{formattedDate}</Text>
              <Text style={styles.eventCount}>{agendaItems.length} {agendaItems.length === 1 ? 'event' : 'events'}</Text>
            </View>
            <TouchableOpacity style={styles.dayArrow} onPress={() => moveAgendaDay(1)} accessibilityLabel="Next day agenda" testID="day-agenda-next-day"><ChevronRight size={20} color="#1C2F7A" /></TouchableOpacity>
          </View>
        </ResponsiveContainer>
        ...
        >
          <ResponsiveContainer>
            ...
            >
              ...
              <View style={styles.prioritiesCopy}>
                ...
                <Text style={styles.prioritiesSubtitle}>Your explainable pre-cruise, onboard, casino, and post-cruise actions.</Text>
              </View>
          ...
          <View style={styles.sectionContainer}>
            ...
            />
            {weatherVoyages.length > 0 ? (
              <View style={styles.weatherVoyageBlock} testID="agenda-weather-voyage-block">
                {weatherVoyages.length > 1 ? (
                  <View style={styles.weatherBlockNotice} testID="agenda-weather-back-to-back-summary">
                    <Text style={styles.weatherBlockNoticeTitle}>Back-to-back weather block</Text>
                    <Text style={styles.weatherBlockNoticeText}>
                      {weatherVoyages.length} consecutive {weatherVoyages[0].shipName} sailings · {weatherVoyages[0].sailDate} through {weatherVoyages[weatherVoyages.length - 1].returnDate}
                    </Text>
                  </View>
                ) : null}
                {weatherVoyages.map((voyage) => (
                  <VoyageWeatherSection key={`${voyage.shipName}-${voyage.sailDate}-${voyage.returnDate}`} cruise={voyage} defaultExpanded />
                ))}
              </View>
            ) : (
