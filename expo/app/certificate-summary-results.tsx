...
import { Bot, ChevronLeft, Copy, FileDown, FileText, ListPlus, Search, Ship, SlidersHorizontal, Users, X } from 'lucide-react-native';
    ...
    return (
      <View style={styles.card} testID={`certificate-summary-results.row-${item.optionId}`}>
        <View style={styles.cardTop}>
          <View style={styles.codePill}><Text style={styles.codeText}>{item.certificateCode}</Text></View>
          <Text style={styles.points}>{item.points == null ? 'Points unknown' : `${item.points.toLocaleString()} pts`}</Text>
        </View>
        <View style={styles.shipRow}><View style={styles.shipIcon}><Ship size={17} color="#0E7FA7" /></View><View style={styles.flex}><Text style={styles.shipName}>{item.shipName}</Text><Text style={styles.date}>{item.sailDate}{item.startDay || item.endDay ? ` · ${item.startDay || 'Start day not stated'} → ${item.endDay || 'End day not stated'}` : ''}</Text></View></View>
        <Text style={styles.itinerary}>{item.itinerary || 'Itinerary not stated'}</Text>
        <Text style={styles.meta}>{item.departurePort || 'Departure port not stated'} · {item.nights == null ? 'Nights not stated' : `${item.nights} nights`}{item.region ? ` · ${item.region}` : ''}</Text>
        <View style={styles.entitlementRow}>
          <View style={styles.entitlement}><Text style={styles.entitlementText}>{item.cabinLabel || 'Cabin not stated'} · {item.isGty ? 'GTY' : 'Assigned category'}</Text></View>
          <View style={styles.entitlement}><Users size={12} color="#24415B" /><Text style={styles.entitlementText}>{item.guestCount == null ? 'Guests not stated' : `${item.guestCount} guest${item.guestCount === 1 ? '' : 's'}`}</Text></View>
        </View>
        {item.shipClass ? <Text style={styles.classText}>{item.shipClass}</Text> : <Text style={styles.warningText}>Ship class missing</Text>}
        ...
        <View style={[styles.actions, { flexWrap: 'wrap' }]}>
          ...
          <TouchableOpacity
            ...
            onPress={() => router.push({
              ...
              params: buildCruiseDetailsParams({
                id: item.optionId,
                ...
                nights: item.nights,
                destination: item.itinerary,
                itineraryName: item.itinerary,
                departurePort: item.departurePort,
                cabinType: item.cabinLabel,
