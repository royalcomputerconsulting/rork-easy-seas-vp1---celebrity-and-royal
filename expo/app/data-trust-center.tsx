...
export default function DataTrustCenter() {
  ...
  const webCompatibilityMessage = 'The browser preview does not open the native indexed trust database. Integrity scans, migrations, encrypted backups, and restore previews remain fully available in the iOS app.';
  ...
  const runIntegrity = async () => {
    ...
    try {
      ...
      const allCruises = [...(core.bookedCruises ?? []), ...availableCruises];
      ...
      const casinoRows = (core.bookedCruises ?? []).map((row) => ({ ...(row as any), id: row.id, ownerId: (row as any).ownerProfileId ?? privateOwner, points: (row as any).casinoPoints ?? (row as any).pointsEarned ?? 0, coinIn: (row as any).coinIn ?? 0 }));
      ...
      const detected = scanIntegrity({ ownerId: privateOwner, cruises: allCruises as any[], offers: (core.casinoOffers ?? []) as any[], offerSailings, certificates: searchableCertificates as any[], loyalty: loyaltyRows, casinoTotals: casinoRows });
  ...
  return <SafeAreaView style={styles.safe}><Stack.Screen options={{ headerShown: false }}/><View style={styles.header}><TouchableOpacity style={styles.back} onPress={() => router.back()} accessibilityLabel="Back"><ArrowLeft color={T.color.deepNavy}/></TouchableOpacity><View style={{ flex: 1 }}><Text style={styles.eyebrow}>LOCAL · PRIVATE · EXPLAINABLE</Text><Text style={styles.title}>Data trust center</Text></View><ShieldCheck color="#0E7FA7"/></View><ScrollView ref={contentRef} contentContainerStyle={styles.content}>
    ...
    <Text onLayout={(event) => setBackupSectionY(event.nativeEvent.layout.y)} style={styles.section}>Incremental encrypted backup</Text><View style={styles.card} testID={`data-trust-${intent ?? 'manage'}-backup`}>
      {intent ? <View style={styles.intentBanner}><Text style={styles.intentMode}>{intent === 'backup' ? 'SAVE ALL WORKFLOW' : 'LOAD ALL WORKFLOW'}</Text><Text style={styles.detail}>{intent === 'backup' ? 'Enter a password, then create or resume the encrypted backup. Progress is resumable and the share sheet opens only after exact file readback succeeds.' : 'Enter the backup password or recovery key, then preview the file. No record changes until you approve the conflict-safe restore preview.'}</Text></View> : null}
      <Text style={styles.detail}>AES-256-GCM encryption, user-controlled password, recovery key, versioned manifest, and conflict-safe preview. Every export includes its complete snapshot chain. No backend is required.</Text>
      <View style={styles.credentialTabs}>
        <TouchableOpacity style={[styles.credentialTab, restoreCredentialType === 'password' && styles.credentialTabOn]} onPress={() => { setRestoreCredentialType('password'); setPassword(''); }} disabled={isWebPreview}><Text style={[styles.credentialTabText, restoreCredentialType === 'password' && styles.credentialTabTextOn]}>Password</Text></TouchableOpacity>
        <TouchableOpacity style={[styles.credentialTab, restoreCredentialType === 'recovery-key' && styles.credentialTabOn]} onPress={() => { setRestoreCredentialType('recovery-key'); setPassword(''); setShowRecoveryKeyInput(true); }} disabled={isWebPreview}><Text style={[styles.credentialTabText, restoreCredentialType === 'recovery-key' && styles.credentialTabTextOn]}>Recovery key</Text></TouchableOpacity>
      </View>
      <View style={styles.credentialInputRow}>
        <TextInput value={password} onChangeText={setPassword} secureTextEntry={restoreCredentialType === 'password' || !showRecoveryKeyInput} autoCapitalize="none" autoCorrect={false} placeholder={restoreCredentialType === 'password' ? 'Backup password (8+ characters)' : 'Paste recovery key'} style={[styles.input, styles.credentialInput]} accessibilityLabel={restoreCredentialType === 'password' ? 'Backup password' : 'Backup recovery key'} editable={!isWebPreview}/>
        {restoreCredentialType === 'recovery-key' ? <TouchableOpacity style={styles.credentialUtilityButton} onPress={() => setShowRecoveryKeyInput((shown) => !shown)} accessibilityRole="button" accessibilityLabel={showRecoveryKeyInput ? 'Hide recovery key' : 'Show recovery key'} testID="data-trust-toggle-recovery-key"><Eye size={18} color={T.color.navy}/><Text style={styles.credentialUtilityText}>{showRecoveryKeyInput ? 'Hide' : 'Show'}</Text></TouchableOpacity> : null}
      </View>
      {restoreCredentialType === 'recovery-key' ? <TouchableOpacity style={styles.pasteButton} onPress={() => void pasteRecoveryKey()} disabled={!!busy || isWebPreview} accessibilityRole="button" testID="data-trust-paste-recovery-key"><KeyRound size={17} color={T.color.navy}/><Text style={styles.pasteButtonText}>Paste recovery key</Text></TouchableOpacity> : null}
      <View style={styles.actions}>
        {restoreCredentialType === 'password' && intent !== 'restore' ? <Action icon={<Download color={T.color.navy}/>} title={backupResume ? "Resume encrypted backup" : "Create encrypted backup"} subtitle={backupResume ? `${backupResume.completedDatasets.length} datasets already completed; unpublished partial work is retained in memory` : backupChain.length ? 'Save only changed records inside a complete restore chain' : 'Create a complete encrypted baseline'} onPress={createBackup} disabled={!!busy || isWebPreview}/> : null}
        <Action icon={<Upload color={T.color.teal}/>} title="Load Encrypted Backup" subtitle="Choose the .easyseas file, decrypt it, and review every change before recovery" onPress={() => void pickRestore()} disabled={!!busy || isWebPreview}/>
      </View>
      {recoveryKey ? <View style={styles.recovery} testID="data-trust-saved-recovery-key"><Text style={styles.recoveryTitle}>Recovery key</Text><Text selectable selectionColor={T.color.teal} style={styles.recoveryValue}>{recoveryKey}</Text><Text style={styles.small}>Long-press the key to select it, or use Copy. Store it outside the app. Easy Seas cannot recreate it.</Text><View style={styles.recoveryActions}><TouchableOpacity style={styles.recoveryButton} onPress={copyRecoveryKey} accessibilityRole="button" testID="data-trust-copy-recovery-key"><Copy size={16} color={T.color.navy}/><Text style={styles.recoveryButtonText}>Copy key</Text></TouchableOpacity><TouchableOpacity style={styles.recoveryButtonPrimary} onPress={() => void loadWithSavedRecoveryKey()} disabled={!!busy || isWebPreview} accessibilityRole="button" testID="data-trust-load-with-saved-recovery-key"><KeyRound size={16} color="#FFFFFF"/><Text style={styles.recoveryButtonPrimaryText}>Use key to load backup</Text></TouchableOpacity></View></View> : null}
    </View>
    {restorePreview ? <View style={styles.card}><Text style={styles.sectionIn}>Restore preview</Text>{restorePreview.rows.map((row) => <Text key={row.dataset} style={styles.previewRow}>{row.dataset}: +{row.add} · update {row.update} · preserve {row.preserve} · conflict {row.conflict} · reject {row.reject}</Text>)}<Text style={styles.warningText}>{restorePreview.conflicts.length} conflicts will preserve current app values by default.</Text><TouchableOpacity style={styles.primary} onPress={applySafeRestore} accessibilityRole="button" accessibilityLabel="Recover data from this backup. Apply safe additions; preserve conflicts." testID="data-trust-recover-all-data"><Text style={styles.primaryText}>Recover data from this backup</Text></TouchableOpacity></View> : null}
