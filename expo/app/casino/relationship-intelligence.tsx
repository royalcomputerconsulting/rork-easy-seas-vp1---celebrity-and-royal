...
import { ArrowLeft, Bot, ChevronRight, Gauge, ShieldAlert, Sparkles, Target, TrendingUp } from 'lucide-react-native';
...
export default function CasinoRelationshipIntelligenceScreen() {
  ...
  const { clubRoyalePoints, clubRoyalePointsSource } = useLoyalty();
  ...
  return <View style={styles.root}><Stack.Screen options={{ headerShown: false }} /><SafeAreaView style={styles.safe} edges={['top']}>
    ...
    <ScrollView contentContainerStyle={styles.content}>
      ...
      <Section title="Easy Seas Casino Intelligence" icon={<Bot size={18} color={C.gold} />}>
        ...
        <TouchableOpacity style={styles.primary} onPress={() => router.push('/ask-my-data' as any)} testID="relationship-intelligence-ask-agent"><Bot size={17} color={C.deepNavy} /><Text style={styles.primaryText}>Ask Easy Seas</Text></TouchableOpacity>
      </Section>
    </ScrollView>
  </SafeAreaView></View>;
}

function Section({ title, icon, children }: { title: string; icon: React.ReactNode; children: React.ReactNode }) { return <View style={styles.section}><View style={styles.sectionTitle}>{icon}<Text style={styles.sectionTitleText}>{title}</Text></View>{children}</View>; }
