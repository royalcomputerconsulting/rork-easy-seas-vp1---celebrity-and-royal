...
export default function CasinoExportReportScreen() {
  ...
  const reportMarkdown = useMemo(() => {
    ...
    lines.push('');
    lines.push('## Verified 2025 Club Royale Annual Report');
    lines.push(`- Completed Royal Cruises: ${cruiseEconomicsSummary.totals.cruises}`);
    lines.push(`- Total Nights: ${cruiseEconomicsSummary.totals.totalNights}`);
    lines.push(`- Total Points: ${Math.round(cruiseEconomicsSummary.totals.totalPoints).toLocaleString()}`);
    lines.push(`- Total Coin-In (gaming volume only): $${Math.round(cruiseEconomicsSummary.totals.totalCoinIn).toLocaleString()}`);
    lines.push(`- Total Retail: $${cruiseEconomicsSummary.totals.totalRetailValue.toLocaleString()}`);
    lines.push(`- Total Paid: $${cruiseEconomicsSummary.totals.totalPaid.toLocaleString()}`);
    lines.push(`- Cruise Value Captured: $${cruiseEconomicsSummary.totals.totalCruiseValueCaptured.toLocaleString()}`);
    lines.push(`- Cash Result: $${cruiseEconomicsSummary.totals.totalCashResult.toLocaleString()}`);
    const allocatedAnnualPoints = cruiseEconomicsSummary.rows.reduce((sum, row) => sum + row.points, 0);
    const unallocatedAnnualPoints = Math.max(0, cruiseEconomicsSummary.totals.totalPoints - allocatedAnnualPoints);
    if (unallocatedAnnualPoints > 0) lines.push(`- Unallocated confirmed points: ${unallocatedAnnualPoints.toLocaleString()} (included in annual total but not invented on a specific cruise)`);
    lines.push('');
...
const styles = StyleSheet.create({
  container: { flex: 1, backgroundColor: DARK_ROYAL_COLORS.background },
  safeArea: { flex: 1 },
  header: {
    flexDirection: 'row',
    alignItems: 'center',
