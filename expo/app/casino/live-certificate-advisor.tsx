          ...
          {snapshot ? (
            <>
              <View style={styles.hero}>
                ...
                <Text style={styles.heroMeta}>{snapshot.recommendation.currentPoints.toLocaleString()} points · {snapshot.recommendation.confidence} confidence · {(snapshot.playMode ?? 'neutral').replaceAll('-', ' ')}</Text>
              </View>
              ...
              <Metric icon={<TrendingUp size={18} color={COLORS.navyDeep} />} label="Expected net vacation value" value={`$${snapshot.recommendation.expectedNetVacationValue.toFixed(0)}`} />
              <Metric icon={<Lock size={18} color={COLORS.navyDeep} />} label="Bankroll survival" value={snapshot.recommendation.bankrollImpact.bankrollSurvivalProbability === null ? 'Not enough evidence' : `${(snapshot.recommendation.bankrollImpact.bankrollSurvivalProbability * 100).toFixed(0)}% · P90 $${snapshot.recommendation.bankrollImpact.requiredBankrollP90?.toFixed(0) ?? '—'}`} />
              ...
              <View style={styles.card}><Text style={styles.cardTitle}>Bounded session scenarios</Text>{(snapshot.sessionScenarios?.length ? snapshot.sessionScenarios : [snapshot.oneMoreSessionScenario]).map(scenario => <Text key={scenario.sessionMinutes} style={styles.row}>• {scenario.sessionMinutes} min: +{scenario.expectedAdditionalPoints} pts · ${scenario.expectedAdditionalLoss.toFixed(0)} expected loss · {scenario.permitted ? 'within gates' : 'not permitted'}</Text>)}</View>
              <View style={styles.card}><Text style={styles.cardTitle}>Target comparison</Text>{snapshot.recommendation.candidateEvaluations.slice(0, 5).map(candidate => <Text key={candidate.thresholdDefinitionId} style={styles.row}>• {candidate.targetPoints.toLocaleString()} ({candidate.targetLabel}): EV ${candidate.expectedNetVacationValue.toFixed(0)} · {(candidate.probabilityOfSuccess * 100).toFixed(0)}% reach · {(candidate.bankrollSurvivalProbability ?? 0) * 100 | 0}% bankroll survival</Text>)}</View>
...
const styles = StyleSheet.create({
  container: { flex: 1, backgroundColor: '#F5F5F4' }, header: { flexDirection: 'row', alignItems: 'center', gap: 12, padding: SPACING.lg, backgroundColor: '#FFF', borderBottomWidth: 1, borderColor: '#D5D5D0' },
  ...
  metric: { flexDirection: 'row', gap: 12, backgroundColor: '#FFF', borderWidth: 1, borderColor: '#D5D5D0', borderRadius: BORDER_RADIUS.lg, padding: SPACING.md, marginBottom: SPACING.sm, alignItems: 'center' }, metricLabel: { color: '#64748B', fontSize: 12, fontWeight: '700' }, metricValue: { color: COLORS.navyDeep, fontWeight: '900', fontSize: 16, marginTop: 2 },
  card: { backgroundColor: '#FFF', borderWidth: 1, borderColor: '#D5D5D0', borderRadius: BORDER_RADIUS.lg, padding: SPACING.md, marginTop: SPACING.sm }, cardTitle: { color: COLORS.navyDeep, fontSize: 16, fontWeight: '900', marginBottom: 8 }, row: { color: '#334155', lineHeight: 20, marginBottom: 5 },
