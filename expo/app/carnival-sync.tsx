...
function CarnivalSyncScreen() {
  ...
  const getStatusColor = () => {
    switch (state.status) {
      ...
      case 'invalid_response': return '#A52B34';
      case 'login_expired': return CARNIVAL_GOLD;
      case 'error': return '#A52B34';
  ...
  const handleExportSyncLog = async () => {
    try {
      ...
      if (state.syncCounts) {
        const availableCruiseRows = state.syncCounts.availableCruiseRows || state.syncCounts.offerSailingRelationships || state.syncCounts.totalImportedCruises || state.syncCounts.offerRows || 0;
        const physicalSailingRows = state.syncCounts.physicalSailingRows || state.syncCounts.canonicalRows || 0;
        const providerBookingRows = (state.syncCounts.bookedCruises || 0) + (state.syncCounts.completedCruises || 0) + (state.syncCounts.courtesyHolds || 0);
        logContent += `\n--- CURRENT CARNIVAL SYNC RESULT ---\n`;
        logContent += `Offers: ${state.syncCounts.offerCount} unique deal(s)\n`;
        logContent += `Sailings: ${state.syncCounts.offerRows} scraped offer-sailing row(s)\n`;
        logContent += `Available Cruise Rows After Reconciliation: ${availableCruiseRows}\n`;
        logContent += `Physical Sailings After Reconciliation: ${physicalSailingRows}\n`;
        logContent += `Offer-Sailing Relationships Read Back: ${state.syncCounts.offerSailingRelationships || 0}\n`;
        ...
        logContent += `Provider Booking/History Records After Reconciliation: ${providerBookingRows}\n`;
      }
      logContent += `\n--- DETAILED LOG ---\n`;
      ...
      <View style={styles.container}>
        ...
        >
          <View style={[styles.contentColumn, Platform.OS === 'web' && styles.contentColumnWeb]}>
            {!carnivalSyncAccess.enabled && (
              <View style={styles.accessNotice} testID="carnival-sync-access-notice">
                <AlertCircle size={18} color="#E6B63D" />
          ...
          <View style={[styles.pillRow, isCompactWindow && styles.pillRowCompact]}>
            ...
            <View style={[styles.pill, { backgroundColor: '#4EC0A520', borderColor: '#4EC0A540' }]}>
              <Anchor size={12} color="#4EC0A5" />
              <Text style={[styles.pillText, { color: '#4EC0A5' }]}>Cruise Deals</Text>
            </View>
          </View>
              ...
              {Platform.OS === 'web' ? (
                <View style={[styles.webWorkspace, isCompactWindow && styles.webWorkspaceCompact]} testID="carnival-web-workspace">
                  ...
                  <View style={[styles.webWorkspaceButtonRow, isCompactWindow && styles.webWorkspaceButtonRowCompact]}>
                    ...
                    >
                      <ExternalLink size={18} color="#D5D5D0" />
            ...
            {Platform.OS === 'web' ? (
              ...
              <View style={styles.mobileActionsContainer}>
                ...
                <View style={styles.quickActionsGrid}>
                  ...
                  >
                    <RefreshCcw size={20} color="#4EC0A5" />
            ...
            <View style={styles.modalOverlay}>
              <View style={styles.confirmationModal}>
                ...
                >
                  <View style={styles.countCard}>
                    ...
                    <View style={styles.countInfo}>
                      <Text style={styles.countNumber}>
                        {(state.syncCounts?.availableCruiseRows || state.syncCounts?.offerRows || 0).toLocaleString()} rows
                      </Text>
                      ...
                      {state.syncCounts && state.syncCounts.offerRows > 0 && (
                        <Text style={styles.countDetail}>
                          {state.syncCounts.offerCount} unique deal{state.syncCounts.offerCount !== 1 ? 's' : ''} · offer-sailing rows
                        </Text>
                      )}
                    </View>
                  </View>
                  ...
                  <View style={styles.countCard}>
                    <View style={[styles.countIconContainer, { backgroundColor: '#4EC0A520' }]}>
                    ...
                    <Text style={styles.warningText}>
                      {state.syncCounts?.carnivalOutcome === 'complete'
                        ...
                        : state.syncCounts?.carnivalOutcome === 'complete_with_warnings'
                          ? 'Carnival offers and available sailings are complete enough to save. One or more optional loyalty/profile fields are incomplete.'
          ...
          {(state.status === 'complete' || state.status === 'complete_with_warnings' || state.status === 'partial') && state.lastSyncTimestamp && (
            <View style={styles.successContainer}>
              <CheckCircle size={24} color="#4EC0A5" />
              <View style={styles.successContent}>
                ...
                <Text style={styles.successMessage}>
                  {state.syncCounts
                    ? `Synced ${state.syncCounts.offerCount} deal${state.syncCounts.offerCount !== 1 ? 's' : ''} with ${(state.syncCounts.availableCruiseRows || state.syncCounts.offerRows || 0).toLocaleString()} available offer-sailing row${(state.syncCounts.availableCruiseRows || state.syncCounts.offerRows || 0) !== 1 ? 's' : ''} and ${state.syncCounts.upcomingCruises} booked cruise${state.syncCounts.upcomingCruises !== 1 ? 's' : ''}.`
