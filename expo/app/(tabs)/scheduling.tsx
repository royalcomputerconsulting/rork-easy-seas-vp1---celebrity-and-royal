...
import { DataStateCard, InlineLoading } from '@/components/ui/EasySeasPrimitives';
...
import { matchesCruiseDiscoveryFilters } from '@/lib/cruises/cruiseDiscoveryFilters';
import { useExperience } from '@/state/ExperienceProvider';
import { buildCruiseDetailsParams } from '@/lib/navigation/cruiseDetails';
import { getCanonicalCruiseInventoryKey } from '@/lib/cruiseInventory/cruiseCanonicalIdentity';
...
export default function SchedulingScreen() {
  ...
  const { colors: experienceColors, isDark, minimumControlSize, preferences } = useExperience();
  ...
  const { queryCruises, totalCruises, facets, isInventoryReady, refreshCounts } = useCruiseInventory();
  ...
  useEffect(() => {
    if (restoredScrollRef.current || catalogLoading || catalogRows.length === 0) return;
    restoredScrollRef.current = true;
    if (schedulingViewStateCache.scrollOffset <= 0) return;
    const timer = setTimeout(() => cruiseListRef.current?.scrollToOffset({ offset: schedulingViewStateCache.scrollOffset, animated: false }), 0);
  ...
  const loadMoreCatalogRows = useCallback(async () => {
    ...
    try {
      ...
      setCatalogRows((current) => {
        const known = new Set(current.map((cruise) => cruise.id));
        return [...current, ...page.rows.filter((cruise) => !known.has(cruise.id))];
      });
      setCatalogTotal(page.total);
      nextCatalogCursorRef.current = page.nextCursor;
    } catch (error) {
  ...
  const activeFilterLabels = useMemo(() => {
    ...
    filters.selectedShips.forEach((value) => labels.push(`Ship: ${value}`));
    filters.selectedShipClasses.forEach((value) => labels.push(`Class: ${value}`));
    if (filters.cabinType !== 'all') labels.push(`Room: ${filters.cabinType}`);
    filters.guestCounts.forEach((value) => labels.push(`${value} guest${value === 1 ? '' : 's'}`));
    filters.departurePorts.forEach((value) => labels.push(`From: ${value}`));
    filters.regions.forEach((value) => labels.push(`Region: ${value}`));
    if (filters.dateFrom) labels.push(`After: ${filters.dateFrom}`);
    if (filters.dateTo) labels.push(`Before: ${filters.dateTo}`);
    if (filters.minNights) labels.push(`${filters.minNights}+ nights`);
    if (filters.maxNights) labels.push(`Up to ${filters.maxNights} nights`);
    if (filters.eligibility === 'offer') labels.push('Offer attached');
    if (filters.eligibility === 'certificate') labels.push('Downloaded certificate');
    if (filters.noConflicts) labels.push('No booking conflicts');
    return labels;
  }, [filters]);
  ...
  const filteredCruises = useMemo(() => {
    ...
    if (filters.searchQuery.trim()) {
      ...
      result = result.filter(c =>
        ...
        getCruiseShipClass(c).toLowerCase().includes(query) ||
        c.destination?.toLowerCase().includes(query) ||
        c.destinationRegion?.toLowerCase().includes(query) ||
        ...
        c.offerName?.toLowerCase().includes(query) ||
        c.guestsInfo?.toLowerCase().includes(query)
      );
    }
    ...
    return (
      <ResponsiveContainer>
        ...
        <View style={styles.b2bSetFooter}>
          ...
          >
            <Text style={styles.b2bOperationalButtonText}>Build Operational Trip Plan</Text>
          </TouchableOpacity>
        </View>
        </View>
      </ResponsiveContainer>
  ...
  const renderHeader = () => (
    <ResponsiveContainer>
      ...
      <TabIdentityBand tab="cruises" compact detail={`${stats.catalogCruises.toLocaleString()} in catalog · ${stats.indexedMatches.toLocaleString()} match${stats.indexedMatches === 1 ? '' : 'es'} · ${stats.loaded.toLocaleString()} loaded`} />
      <View style={styles.discoverySection}>
      <ThemedSectionHeader
        tab="cruises"
        emoji="🔎"
        title="Find your next cruise"
        subtitle="Search, sort, and narrow every available sailing."
        compact
      ...
      <View style={styles.sortFilterRow}>
        ...
        <TouchableOpacity
          style={[styles.filterToggle, { minWidth: minimumControlSize, minHeight: minimumControlSize }, activeFilterCount > 0 && styles.filterToggleActive]}
          onPress={() => setShowFilterSheet(true)}
          activeOpacity={0.7}
          accessibilityRole="button"
          accessibilityLabel={`Open cruise filters${activeFilterCount ? `, ${activeFilterCount} active` : ''}`}
          testID="cruises-open-filter-screen"
        >
          <SlidersHorizontal size={14} color={activeFilterCount > 0 ? COLORS.navyDeep : CLEAN_THEME.text.secondary} />
          {activeFilterCount > 0 ? <Text style={styles.filterCountText}>{activeFilterCount}</Text> : null}
        </TouchableOpacity>
      </View>
      <TouchableOpacity
      ...
      >
        ...
        <View style={styles.agentInlineCopy}>
          ...
          <Text style={styles.agentInlineSubtitle}>Compare these sailings using your offers, certificates, bookings, and preferences.</Text>
        </View>
      ...
      {activeFilterLabels.length > 0 ? (
        <View style={styles.activeFilterSummary} testID="cruises-active-filter-summary">
          ...
          <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.activeFilterChipRow}>
            {activeFilterLabels.map((label, index) => <View key={`${label}-${index}`} style={styles.activeFilterChip}><Text style={styles.activeFilterChipText}>{label}</Text></View>)}
          </ScrollView>
        </View>
      ) : null}
      </View>
  ...
  const renderListFooter = useCallback(() => {
    ...
    return (
      <View style={styles.catalogFooter}>
        {catalogLoadingMore ? <InlineLoading label="Loading more cruise rows…" /> : null}
        {!catalogLoadingMore && hasMoreRows ? (
          <TouchableOpacity
            style={styles.catalogLoadMoreButton}
            onPress={() => { void loadMoreCatalogRows(); }}
            accessibilityRole="button"
            accessibilityLabel={`Load the next ${Math.min(75, remainingRows || 75)} cruise rows`}
            testID="cruises-catalog-load-more"
          >
            <Text style={styles.catalogLoadMoreText}>Load next {Math.min(75, remainingRows || 75)} cruises</Text>
            <Text style={styles.catalogLoadMoreMeta}>{catalogRows.length.toLocaleString()} of {catalogTotal.toLocaleString()} rows loaded</Text>
          </TouchableOpacity>
        ) : null}
        {!catalogLoadingMore && !hasMoreRows && catalogRows.length > 0 ? (
          <View style={styles.catalogEndCard} testID="cruises-catalog-end">
            ...
            <Text style={styles.catalogEndTitle}>End of cruise catalog</Text>
            <Text style={styles.catalogEndMeta}>All {catalogRows.length.toLocaleString()} indexed rows are loaded; {filteredCruises.length.toLocaleString()} remain visible after local booking and schedule checks.</Text>
          </View>
        ) : null}
        <TouchableOpacity
          ...
          accessibilityRole="button"
          accessibilityLabel="Return to cruise discovery and favorites"
          testID="cruises-return-to-discovery"
        >
          <Text style={styles.catalogReturnText}>Back to discovery and favorites</Text>
        </TouchableOpacity>
      </View>
    );
      ...
      <Modal visible={showFilterSheet} animationType="slide" presentationStyle="pageSheet" onRequestClose={() => setShowFilterSheet(false)}>
        <SafeAreaView style={styles.filterSheetSafe} edges={['top', 'bottom']}>
          ...
          <ScrollView contentContainerStyle={styles.filterSheetContent} keyboardShouldPersistTaps="handled">
            ...
            <Text style={styles.filterSheetHelp}>The main search field matches ship, class, itinerary, destination, port, offer code, offer name, and guest text. Missing provider values remain unstated; Easy Seas does not fabricate cabin, guest, price, or casino data.</Text>
          </ScrollView>
          <View style={styles.filterSheetActions}>
            ...
            <TouchableOpacity style={styles.filterApplyButton} onPress={() => setShowFilterSheet(false)} testID="cruises-filter-apply"><Text style={styles.filterApplyText}>View loaded results</Text></TouchableOpacity>
          </View>
        </SafeAreaView>
      </Modal>
...
const styles = StyleSheet.create({
  ...
  filterSheetHelp: { paddingHorizontal: 4, fontSize: 12, lineHeight: 18, color: '#60727F' },
  filterSheetActions: {
    flexDirection: 'row',
    gap: 10,
