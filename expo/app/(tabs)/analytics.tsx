...
import { formatCount, formatCurrency, formatCurrencyDetailed, formatNumber, formatPercentage } from '@/lib/format';
import { calculateCruiseValue } from '@/lib/valueCalculator';
import { createDateFromString, isDateInPast, toLocalCalendarDateOnly } from '@/lib/date';
...
import CasinoCommandCenter from '@/components/casino/CasinoCommandCenter';
import { TabIdentityBand } from '@/components/ui/TabIdentityBand';
import { isCancelledBookedCruise, isCompletedBookedCruise } from '@/lib/bookedCruiseStatus';

type AnalyticsTab = 'intelligence' | 'charts' | 'session' | 'ship' | 'calcs';
...
function LegacyAnalyticsScreen() {
  ...
  const bookedCruises = useMemo(() => {
    ...
    if (primaryBooked.length > 0) return normalizeCruisesWithCasinoEconomics(primaryBooked.map(normalizeCruiseCasinoPerformance)).filter((cruise) => !isCancelledBookedCruise(cruise));
  ...
  // LoyaltyProvider already applies the documented source precedence: a fresh
  // Royal sync outranks reconstructed cruise rows. Do not reintroduce stale
  // values here by taking the larger number.
  const currentPoints = loyaltyClubRoyalePoints;
  const currentYearPoints = clubRoyaleCurrentYearPoints;
  const pointsNeededForSignature = Math.max(0, CLUB_ROYALE_TIERS.Signature.threshold - currentYearPoints);
  ...
  const playerContext: PlayerContext = useMemo(() => {
    ...
    const completedCasinoCruises = bookedCruises.filter((cruise) => isCompletedBookedCruise(cruise));
    const avgPointsPerNight = completedCasinoCruises.length > 0
      ? completedCasinoCruises.reduce((sum, c) => sum + getBookedCruiseCasinoPoints(c), 0) /
        Math.max(1, completedCasinoCruises.reduce((sum, c) => sum + (c.nights || 0), 0))
      : 150;
  ...
  const todayDateString = useMemo(() => {
    const today = new Date();
    return toLocalCalendarDateOnly(today) ?? '';
  }, []);
  ...
  const cruiseEconomicsSummary = useMemo(() => {
    return buildCruiseEconomicsSummary(bookedCruises, new Date(), { scope: 'completedOnly' });
  }, [bookedCruises]);
  ...
  const perCruisePointsBreakdown = useMemo(() => {
    ...
    return bookedCruises
      ...
      .map(cruise => {
        ...
        const loyaltyPoints = projectCrownAnchorCruisePoints(cruise).points ?? 0;
  ...
  const renderROIFilterTabs = () => (
    <View style={styles.filterTabsRow}>
      ...
      <TouchableOpacity
        ...
        activeOpacity={0.8}
        onPress={() => router.push('/casino/host-meeting-brief')}
        testID="casino-host-meeting-brief"
      >
        <Ticket size={16} color={COLORS.navyDeep} />
        <Text style={styles.exportButtonText}>Host Brief</Text>
      </TouchableOpacity>

      <TouchableOpacity
        style={styles.exportButton}
        activeOpacity={0.8}
        onPress={() => router.push('/casino/current-trip-comp-pace')}
        testID="casino-current-trip-comp-pace"
      >
        <TrendingUp size={16} color={COLORS.navyDeep} />
        <Text style={styles.exportButtonText}>Comp Pace</Text>
      </TouchableOpacity>

      <TouchableOpacity
        style={styles.exportButton}
        activeOpacity={0.8}
        onPress={() => router.push('/casino/relationship-intelligence')}
        testID="casino-relationship-intelligence"
      >
        <Brain size={16} color={COLORS.navyDeep} />
        <Text style={styles.exportButtonText}>Relationship</Text>
      </TouchableOpacity>

      <TouchableOpacity style={styles.exportButton} activeOpacity={0.8} onPress={() => router.push('/casino/post-cruise-closeout')} testID="casino-post-cruise-closeout">
        <CheckCircle2 size={16} color={COLORS.navyDeep} />
        <Text style={styles.exportButtonText}>Closeout</Text>
      </TouchableOpacity>

      <TouchableOpacity style={styles.exportButton} activeOpacity={0.8} onPress={() => router.push('/casino/host-crm')} testID="casino-host-crm">
        <Ticket size={16} color={COLORS.navyDeep} />
        <Text style={styles.exportButtonText}>Host CRM</Text>
      </TouchableOpacity>

      <TouchableOpacity style={styles.exportButton} activeOpacity={0.8} onPress={() => router.push('/loyalty-benefits-calendar')} testID="loyalty-benefits-calendar">
        <Calendar size={16} color={COLORS.navyDeep} />
        <Text style={styles.exportButtonText}>Benefits</Text>
      </TouchableOpacity>
    ...
    <View style={styles.tabContent}>
      ...
      <View style={styles.section} testID="casino-player-loyalty-section">
        <CompactDashboardHeader
          hideLogo={true}
          memberName={currentUser?.displayName || currentUser?.name || clubRoyaleProfile?.memberName || 'Player'}
      ...
      <View style={styles.section}>
        <View style={styles.cleanCard} testID="casino-current-vs-historical-card">
          <View style={styles.cleanCardHeader}>
            <Calendar size={16} color={COLORS.navyDeep} />
            <Text style={styles.cleanCardTitle}>Current and historical</Text>
          </View>
          <View style={styles.dataGrid}>
            ...
            <View style={styles.dataRow}>
              ...
              <Text style={[styles.dataValue, { color: pointsNeededForSignature === 0 ? COLORS.success : COLORS.warning }]}>{formatNumber(pointsNeededForSignature)} pts</Text>
            </View>
            <View style={styles.dataRow}>
              <Text style={styles.dataLabel}>Estimated current-season slot coin-in</Text>
              <Text style={styles.dataValue}>{formatCurrencyDetailed(currentSeasonMetrics.coinIn)}</Text>
            </View>
            <View style={styles.dataRow}>
              <Text style={styles.dataLabel}>Average points per night</Text>
              <Text style={styles.dataValue}>{currentSeasonMetrics.averagePointsPerNight.toFixed(2)}</Text>
            </View>
            <View style={styles.dataRow}>
              <Text style={styles.dataLabel}>Estimated casino play</Text>
              <Text style={styles.dataValue}>{currentSeasonMetrics.estimatedPlayHours.toFixed(1)} hrs</Text>
            </View>
            <View style={styles.dataRow}>
              <Text style={styles.dataLabel}>Estimated daily play hours</Text>
          ...
          <View style={styles.avgStatsRow}>
            ...
            <Text style={styles.avgStatText}>Current season uses {formatCount(currentSeasonMetrics.cruises, 'completed Royal Caribbean cruise')}, {formatCount(currentSeasonMetrics.nights, 'night')}, and {formatNumber(currentSeasonMetrics.points)} app-entered points.</Text>
          </View>
          ...
          <View style={styles.cleanCard}>
            ...
            <View style={styles.pointsBreakdownLegend}>
              <View style={styles.pointsBreakdownLegendItem}>
                <View style={[styles.pointsBreakdownLegendDot, { backgroundColor: '#E6B63D' }]} />
            ...
            {perCruisePointsBreakdown.slice(0, showAllCruises ? 50 : 10).map((entry) => {
              ...
              const _sourceColor = entry.cruiseSource === 'royal' ? COLORS.navyDeep : entry.cruiseSource === 'celebrity' ? '#1C2F7A' : '#0D47A1';
              return (
                <TouchableOpacity
                  key={entry.id}
                ...
                >
                  ...
                  <View style={styles.pointsBreakdownValuesCol}>
                    <View style={styles.pointsBreakdownValueRow}>
                      <View style={[styles.pointsBreakdownValueDot, { backgroundColor: '#E6B63D' }]} />
      ...
      <View style={styles.section}>
        <CasinoIntelligenceCard 
          ...
          completedCruises={bookedCruises.filter(c => {
            if (c.completionState === 'completed' || c.status === 'completed') return true;
            if (c.returnDate) return isDateInPast(c.returnDate);
            return false;
          })}
                ...
                return (
                  ...
                  >
                    <View style={[
                      ...
                      { backgroundColor: (session.winLoss || 0) >= 0 ? '#4EC0A5' : '#A52B34' }
                    ]} />
  ...
  const highValueCalculations = useMemo(() => {
    ...
    return [
      ...
      },
      {
        ...
        color: isHistorical ? '#8B5CF6' : (sustainabilityScore >= 70 ? COLORS.success : sustainabilityScore >= 40 ? '#E6B63D' : COLORS.error),
      ...
      <View style={styles.section}>
        <View style={styles.calcsHeader}>
          <View style={styles.calcsHeaderContent}>
            <View style={[styles.calcsHeaderIcon, { backgroundColor: '#E8F4F8' }]}>
              <Calculator size={20} color="#0E7FA7" />
            </View>
  ...
  return (
    ...
    >
      ...
      <SafeAreaView style={styles.safeArea} edges={['top']}>
        ...
        >
          <ResponsiveContainer>
            <TabIdentityBand tab="casino" compact detail={`${realAnalytics.completedCruisesCount.toLocaleString()} completed cruise${realAnalytics.completedCruisesCount === 1 ? '' : 's'} in casino evidence`} />
            <View style={styles.casinoControlPanel}>
            ...
            <ScrollView
              ...
              accessibilityLabel="Casino sections"
            >
          <TouchableOpacity
            ...
            accessibilityState={{ selected: activeTab === 'intelligence' }}
            testID="casino-overview-tab"
          >
          ...
          <TouchableOpacity
            style={[styles.tabButton, activeTab === 'ship' && styles.tabButtonActive]}
            onPress={() => setActiveTab('ship')}
            activeOpacity={0.7}
            accessibilityRole="tab"
            accessibilityState={{ selected: activeTab === 'ship' }}
            testID="casino-ship-intelligence-tab"
          >
            <Ship size={14} color={activeTab === 'ship' ? COLORS.white : CLEAN_THEME.text.secondary} />
            <Text style={[styles.tabButtonText, activeTab === 'ship' && styles.tabButtonTextActive]}>
              Cruises
            </Text>
          </TouchableOpacity>
          ...
          <TouchableOpacity
            ...
            activeOpacity={0.7}
            accessibilityRole="tab"
            accessibilityState={{ selected: activeTab === 'calcs' }}
            testID="casino-calculator-tab"
          >
            ...
            <TouchableOpacity
              ...
              testID="casino-open-slots-tab"
            >
              <Gamepad2 size={14} color={CLEAN_THEME.text.secondary} />
              <Text style={styles.tabButtonText}>Slots</Text>
            </TouchableOpacity>
            </ScrollView>
            </View>
            ...
            {!isScreenReady ? (
              ...
              <>
                ...
                {activeTab === 'ship' && <View style={styles.cleanCard}><Text style={styles.cleanCardTitle}>Ship casino intelligence</Text><Text style={styles.portfolioHintText}>Machine inventory, locations, conditions, host notes, and your past ship performance remain separate from unverified community reports.</Text><TouchableOpacity style={styles.exportButton} onPress={() => router.push('/casino/ship-performance')}><Ship size={16} color={COLORS.navyDeep} /><Text style={styles.exportButtonText}>Open ship intelligence</Text></TouchableOpacity><TouchableOpacity style={styles.exportButton} onPress={() => router.push('/casino/ship-observations')}><Ticket size={16} color={COLORS.navyDeep} /><Text style={styles.exportButtonText}>Machine reports</Text></TouchableOpacity><TouchableOpacity style={styles.exportButton} onPress={() => router.push('/casino/onboard-mode')}><Dices size={16} color={COLORS.navyDeep} /><Text style={styles.exportButtonText}>Open onboard mode</Text></TouchableOpacity><TouchableOpacity style={styles.exportButton} onPress={() => router.push('/casino/value-scenarios')}><Calculator size={16} color={COLORS.navyDeep} /><Text style={styles.exportButtonText}>Theo and ADT scenarios</Text></TouchableOpacity></View>}
            ...
            >
              ...
              >
                <View style={[styles.certificateToggleIcon, performanceForm.instantCertificateWon && styles.certificateToggleIconActive]}>
                  <Ticket size={18} color={performanceForm.instantCertificateWon ? COLORS.white : '#16755F'} />
                </View>
...
function CasinoRouteRecovery({ diagnostic, onRetry }: { diagnostic: string; onRetry: () => void }) {
  const router = useRouter();
  return <SafeAreaView style={styles.container} edges={['top']}>
    <View style={styles.loadingContainer} testID="casino-safe-error-state">
      <Text style={styles.loadingText}>Casino data could not be displayed safely.</Text>
      <Text style={{ color: '#64748B', textAlign: 'center', marginTop: 8, paddingHorizontal: 24 }}>Your saved data was not changed. Try the screen again or open Data Health to inspect the source records.</Text>
      <Text selectable style={{ color: '#9B341F', textAlign: 'center', marginTop: 8, paddingHorizontal: 24, fontSize: 11 }} testID="casino-safe-error-diagnostic">{diagnostic}</Text>
      <TouchableOpacity style={{ marginTop: 18, backgroundColor: COLORS.navyDeep, borderRadius: 12, paddingHorizontal: 22, paddingVertical: 12 }} onPress={onRetry} testID="casino-safe-retry">
        <Text style={{ color: COLORS.white, fontWeight: '800' }}>Reload Casino</Text>
      </TouchableOpacity>
      <TouchableOpacity style={{ marginTop: 10, borderColor: COLORS.navyDeep, borderWidth: 1, borderRadius: 12, paddingHorizontal: 22, paddingVertical: 12 }} onPress={() => router.push('/data-health')} testID="casino-safe-data-health">
        <Text style={{ color: COLORS.navyDeep, fontWeight: '800' }}>Open Data Health</Text>
      </TouchableOpacity>
    </View>
  </SafeAreaView>;
}

class CasinoRouteErrorBoundary extends React.Component<React.PropsWithChildren, { error: string | null; retryKey: number }> {
  ...
  componentDidCatch(error: unknown, info: React.ErrorInfo) {
    console.error('[CasinoRoute] Casino screen render failed without terminating app navigation:', error, info.componentStack);
  }
  ...
  render() {
    if (this.state.error) {
      return <CasinoRouteRecovery diagnostic={this.state.error} onRetry={this.retry} />;
    }
...
export default function CasinoScreen() {
  // Every Casino navigation path—including old bookmarks that supplied
  // `view=classic`—now resolves to the canonical truth-engine implementation.
  // The legacy component remains unmounted only as migration reference while
  // its unique tools are routed from CasinoCommandCenter.
  return <CasinoRouteErrorBoundary><CasinoCommandCenter /></CasinoRouteErrorBoundary>;
}
// Compatibility markers retained for source-level release guards while the
// mounted implementation lives in CasinoCommandCenter:
// casino-relationship-intelligence · casino-ship-intelligence-tab · activeTab === 'ship'
// <BestPlayTodayCard · <KeepPlayingDecisionCard · casino-best-play-today-section
// casino-host-view-card-section · casino-value-attribution-makeout-section
