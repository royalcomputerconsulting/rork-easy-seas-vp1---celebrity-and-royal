...
import { View, Text, StyleSheet, ScrollView, TouchableOpacity, Switch, Alert, Linking, ActivityIndicator, TextInput, Image } from 'react-native';
import { Stack, useRouter } from 'expo-router';
import { SafeAreaView } from 'react-native-safe-area-context';
import { Settings as SettingsIcon, Bell, Moon, DollarSign, Download, Upload, Trash2, ChevronRight, Ship, ExternalLink, HelpCircle, Shield, Star, FileSpreadsheet, Calendar, CheckCircle, FolderArchive, FolderInput, Database, Save, RefreshCcw, BookOpen, Crown, FileDown, TrendingDown, Users, Award, Anchor, Link2, Copy, Rss, } from 'lucide-react-native';
...
import { pickAndReadFile, parseOffersCSV, parseICSFile, parseBookedCSV, generateOffersCSV, generateCalendarICS, generateBookedCSV, exportFile, downloadFromURL, healImportedData } from '@/lib/importExport';
import { clearAllAppData, exportAllDataToFile, importAllDataFromFile, } from '@/lib/dataManager';
...
import { getImportedSource, getImportedSourceLabel, mergeImportedBookedCruises, mergeImportedCruises, mergeImportedOffers, } from '@/lib/importMerge';
...
export default function SettingsScreen() {
    ...
    const { currentUser, updateUser, ensureOwner, syncFromStorage: syncUserFromStorage, isLoading: isUserLoading, } = useUser();
    const { clubRoyalePoints: loyaltyClubRoyalePoints, crownAnchorPoints: loyaltyCrownAnchorPoints, crownAnchorLevel: loyaltyCrownAnchorLevel, clubRoyaleTier: loyaltyClubRoyaleTier, setManualClubRoyalePoints, setManualCrownAnchorPoints, syncFromStorage: syncLoyaltyFromStorage, extendedLoyalty, venetianSociety, captainsClub, isLoading: isLoyaltyLoading, } = useLoyalty();
    ...
    const handleRemoveFromWhitelist = async (email: string) => {
        Alert.alert('Remove Email', `Remove ${email} from whitelist? They will no longer be able to log in.`, [
    ...
    const enrichmentData = useMemo(() => {
        if (!isProfileDisplayReady)
            return null;
        if (!extendedLoyalty && !currentUser?.carnivalVifpNumber)
            return null;
        return {
    ...
    const handleImportOffersCSV = useCallback(async () => {
        try {
            ...
            Alert.alert('Import Successful', `${sourceLabel} import updated ${parsedCruises.length} cruises and ${parsedOffers.length} offers from ${result.fileName}.${healNote}`);
    ...
    const handleSubscribeToFeed = useCallback(() => {
        ...
        Linking.openURL(webcalUrl).catch(() => {
            Alert.alert('Subscribe to Calendar', `Copy this URL and add it as a calendar subscription in your calendar app:\n\n${calendarFeedUrl}`, [
    ...
    const handleRegenerateFeedToken = useCallback(() => {
        Alert.alert('Regenerate Feed URL', 'This will create a new unique URL for your calendar feed. Your old URL will stop working. Any calendar apps subscribed to the old URL will need to be updated.\n\nContinue?', [
            { text: 'Cancel', style: 'cancel' },
            {
                text: 'Regenerate',
                style: 'destructive',
    ...
    const handleImportCalendarFromURL = useCallback(async () => {
        try {
            ...
            Alert.prompt('Import Calendar from URL', 'Enter the URL of the .ics calendar file (TripIt, Google Calendar, etc.):', [
                {
                    text: 'Cancel',
                    style: 'cancel',
                    onPress: () => {
                ...
                },
                {
                    ...
                    onPress: async (url?: string) => {
                        ...
                        try {
                            ...
                            Alert.alert('Import Successful', `Imported ${events.length} calendar events from URL`);
                            console.log('[Settings] Import complete:', events.length, 'events');
                        }
                        catch (error) {
                            console.error('[Settings] URL import error:', error);
                            Alert.alert('Import Error', `Failed to fetch calendar from URL.\n\nError: ${error instanceof Error ? error.message : 'Unknown error'}\n\nPlease check the URL and try again.`);
                        }
                        finally {
                            setIsImporting(false);
                        }
    ...
    const handleImportCalendarFromFile = useCallback(async () => {
        try {
            ...
            setLastImportResult({ type: 'calendar', count: events.length });
            Alert.alert('Import Successful', `Imported ${events.length} calendar events from ${result.fileName}`);
            console.log('[Settings] Import complete:', events.length, 'events');
        }
        catch (error) {
            console.error('[Settings] Import error:', error);
    ...
    const handleImportCalendarICS = useCallback(() => {
        Alert.alert('Import Calendar', 'Choose import method:', [
            {
                text: 'From File',
    ...
    const handleImportBookedCSV = useCallback(async () => {
        try {
            ...
            Alert.alert('Import Successful', `${sourceLabel} booked cruises updated from ${result.fileName}. Imported ${parsedBooked.length} cruise row(s).`);
    ...
    const handleExportAllData = useCallback(async () => {
        try {
            ...
            if (result.success) {
                Alert.alert('Export Successful', `All app data has been exported to ${result.fileName}. This includes cruises, offers, booked cruises, events, casino sessions, certificates, machines, crew members, user profile (name, C&A #, playing hours), Club Royale points, loyalty points, and settings.`);
            }
            else {
...
booked-liberty-1,Liberty of the Seas,10-16-2025,10-25-2025,9,9 Night Canada & New England Cruise,"Cape Liberty, NJ (NYC)","Cape Liberty, NJ (NYC)",324123,2,7676777,TRUE,,`;
            ...
            if (success) {
                Alert.alert('Download Successful', 'The Booked CSV template has been downloaded. Fill in your cruise bookings following the template format, then import using "Booked Cruises CSV" option.');
            }
            else {
            ...
            if (emailChanged) {
                if (isAdmin) {
                    return new Promise<void>((resolve) => {
                        Alert.prompt('Admin Email Verification', 'You are an admin. Please enter the password to change your email:', [
                            {
                                text: 'Cancel',
                                style: 'cancel',
                                onPress: () => {
                    ...
                    try {
                        ...
                        if (emailCheck.exists) {
                            Alert.alert('Email Already Exists', 'This email is already associated with another account. Please use a different email address.');
                            setIsSaving(false);
                            return;
                        }
                    }
    ...
    async function continueProfileSave(profileData: {
        name: string;
        email: string;
        crownAnchorNumber: string;
        clubRoyalePoints: number;
        ...
        carnivalPlayersClubTier?: string;
        carnivalPlayersClubPoints?: number;
    }, oldEmail: string | undefined, newEmail: string, emailChanged: boolean) {
        try {
            if (currentUser) {
            ...
            if (emailChanged) {
                ...
                await syncUserFromStorage();
                Alert.alert('Email Updated', 'Your email address has been changed successfully. Your profile has been updated with the new email.', [
                    {
                        text: 'OK',
                        onPress: async () => {
                            await syncUserFromStorage();
    ...
    const handleImportMachinesJSON = useCallback(async () => {
        try {
            ...
            try {
                ...
                Alert.alert('Invalid JSON Format', 'The file does not contain valid JSON. Please check the file and try again.\n\nError: ' + (jsonError instanceof Error ? jsonError.message : 'Unknown error'));
                setIsImportingMachines(false);
                return;
            }
            ...
            if (!importResult.success) {
                ...
                Alert.alert('Import Failed', importResult.error || 'Failed to import machines data. Please check that the file contains an array of machine objects.');
                setIsImportingMachines(false);
                return;
            }
            ...
            Alert.alert('Import Successful', `Successfully imported ${importResult.count} machines from ${result.fileName}`);
            console.log('[Settings] Import complete:', importResult.count, 'machines');
        }
        catch (error) {
            console.error('[Settings] Import machines error:', error);
            Alert.alert('Import Error', 'Failed to import the file.\n\nError: ' + (error instanceof Error ? error.message : 'Unknown error') + '\n\nPlease check the file format and try again.');
        }
        finally {
            setIsImportingMachines(false);
        }
    ...
    const handleExportMachinesJSON = useCallback(async () => {
        try {
            ...
            if (success) {
                Alert.alert('Export Successful', `Exported ${myAtlasMachines.length} machines with complete verbose data including:\n\n• AP Analysis & Summary\n• Core Mechanics\n• AP Triggers & Walk-Away Rules\n• Jackpot Reset Values\n• Ship Notes\n• Denominations\n\nFile: ${fileName}`);
            }
            else {
                Alert.alert('Export Info', 'File saved but sharing may not be available on this device.');
            }
    ...
    const handleSaveMockData = useCallback(async () => {
        try {
            ...
            if (result.success) {
                Alert.alert('Mock Data Saved', result.message);
            }
            else {
    ...
    const renderSettingRow = (icon: React.ReactNode, label: string, value?: string | React.ReactNode, onPress?: () => void, isDanger?: boolean) => (<TouchableOpacity style={styles.settingRow} onPress={onPress} disabled={!onPress} activeOpacity={onPress ? 0.7 : 1}>
      <View style={styles.settingLeft}>
        {renderNodeInView(icon)}
        <Text style={[styles.settingLabel, isDanger && styles.dangerLabel]}>{label}</Text>
      </View>
      <View style={styles.settingRight}>
        {typeof value === 'string' ? (<Text style={styles.settingValue}>{value}</Text>) : (renderNodeInView(value))}
        
      </View>
    </TouchableOpacity>);
    const renderToggle = (value: boolean, onToggle: (val: boolean) => void) => (<Switch value={value} onValueChange={onToggle} trackColor={{ false: '#E5E7EB', true: 'rgba(0, 31, 63, 0.3)' }} thumbColor={value ? COLORS.navyDeep : '#9CA3AF'} ios_backgroundColor="#E5E7EB"/>);
    const renderSectionHeader = (icon: React.ReactNode, title: string, subtitle: string, gradientColors: [
        string,
        string
    ] = ['#0369A1', '#0284C7']) => (<LinearGradient colors={gradientColors} style={styles.sectionGradientHeader}>
      ...
      <SafeAreaView style={styles.safeArea} edges={['top']}>
        <ScrollView showsVerticalScrollIndicator={true} persistentScrollbar={true} contentContainerStyle={styles.scrollContent}>
          ...
          <View style={styles.dataOverviewCard}>
            <LinearGradient colors={['#0369A1', '#0284C7'] as [
        string,
        string
          ...
          <View style={[styles.sectionCard, { marginBottom: SPACING.md }]}>
            ...
            <TouchableOpacity style={styles.quickActionFullWidth} onPress={() => router.push('/royal-caribbean-sync' as any)} activeOpacity={0.7}>
            ...
            <TouchableOpacity style={styles.quickActionFullWidth} onPress={() => router.push('/carnival-sync' as any)} activeOpacity={0.7}>
            ...
            <TouchableOpacity style={styles.quickActionFullWidth} onPress={() => router.push('/pricing-summary' as any)} activeOpacity={0.7}>
            ...
            <TouchableOpacity style={styles.quickActionFullWidth} onPress={handleImportOffersCSV} activeOpacity={0.7} disabled={isImporting}>
              <View style={[styles.quickActionIconSmall, { backgroundColor: 'rgba(33, 150, 243, 0.1)' }]}>
                {isImporting ? (<ActivityIndicator size="small" color={COLORS.info}/>) : (<FileDown size={16} color={COLORS.info}/>)}
              </View>
              <Text style={styles.quickActionLabelInline}>Load Import Offers.CSV</Text>
              <ChevronRight size={16} color={CLEAN_THEME.text.secondary}/>
            </TouchableOpacity>
            <View style={styles.quickActionsRow}>
              <TouchableOpacity style={styles.quickActionHalf} onPress={handleExportAllData} activeOpacity={0.7} disabled={isExportingAll}>
                <View style={[styles.quickActionIconSmall, { backgroundColor: 'rgba(76, 175, 80, 0.1)' }]}>
                  {isExportingAll ? (<ActivityIndicator size="small" color={COLORS.success}/>) : (<Save size={16} color={COLORS.success}/>)}
                </View>
                <Text style={styles.quickActionLabelInline}>Save All</Text>
              </TouchableOpacity>
              <TouchableOpacity style={styles.quickActionHalf} onPress={handleImportAllData} activeOpacity={0.7} disabled={isImportingAll}>
                <View style={[styles.quickActionIconSmall, { backgroundColor: 'rgba(33, 150, 243, 0.1)' }]}>
                  {isImportingAll ? (<ActivityIndicator size="small" color={COLORS.info}/>) : (<FolderInput size={16} color={COLORS.info}/>)}
                </View>
          ...
          {isProfileDisplayReady ? (<>
              <UserProfileCard key={`profile-${normalizedAuthenticatedEmail ?? 'guest'}`} currentValues={currentProfileValues} enrichmentData={enrichmentData} onSave={handleSaveProfile} isSaving={isSaving}/>

              <PlayingHoursCard key={`playing-hours-${normalizedAuthenticatedEmail ?? 'guest'}`} currentValues={currentPlayingHours} onSave={handleSavePlayingHours} isSaving={isSavingPlayingHours}/>
            </>) : (<>
          ...
          <View style={styles.section}>
            <View style={styles.sectionCard}>
              ...
              {renderSettingRow(<DollarSign size={18} color={COLORS.navyDeep}/>, 'Show Taxes in List', renderToggle(settings.showTaxesInList, (val) => updateSettings({ showTaxesInList: val })))}
              {renderSettingRow(<DollarSign size={18} color={COLORS.navyDeep}/>, 'Price Per Night', renderToggle(settings.showPricePerNight, (val) => updateSettings({ showPricePerNight: val })))}
              {renderSettingRow(<Moon size={18} color={COLORS.navyDeep}/>, 'Theme', settings.theme === 'dark' ? 'Dark' : settings.theme === 'light' ? 'Light' : 'System')}
            </View>
          </View>
          ...
          <View style={styles.section}>
            <View style={styles.sectionCard}>
              ...
              {renderSettingRow(<Bell size={18} color={COLORS.navyDeep}/>, 'Price Drop Alerts', renderToggle(settings.priceDropAlerts, (val) => updateSettings({ priceDropAlerts: val })))}
              {renderSettingRow(<Bell size={18} color={COLORS.navyDeep}/>, 'Daily Summary', renderToggle(settings.dailySummaryNotifications || false, (val) => updateSettings({ dailySummaryNotifications: val })))}
            </View>
          </View>
          ...
          <View style={styles.section}>
            <View style={styles.sectionCard}>
              ...
              {renderSettingRow(<FileSpreadsheet size={18} color={COLORS.navyDeep}/>, 'Offers CSV', isImporting ? (<ActivityIndicator size="small" color={COLORS.navyDeep}/>) : lastImportResult?.type === 'offers' ? (<View style={styles.successBadge}>
                    <CheckCircle size={12} color={COLORS.success}/>
                    <Text style={styles.successText}>{lastImportResult.count}</Text>
                  </View>) : undefined, handleImportOffersCSV)}
              {renderSettingRow(<Ship size={18} color={COLORS.navyDeep}/>, 'Booked Cruises CSV', isImporting ? (<ActivityIndicator size="small" color={COLORS.navyDeep}/>) : lastImportResult?.type === 'booked' ? (<View style={styles.successBadge}>
                    <CheckCircle size={12} color={COLORS.success}/>
                    <Text style={styles.successText}>{lastImportResult.count}</Text>
                  </View>) : undefined, handleImportBookedCSV)}
              {renderSettingRow(<Calendar size={18} color={COLORS.navyDeep}/>, 'Calendar (.ics)', isImporting ? (<ActivityIndicator size="small" color={COLORS.navyDeep}/>) : lastImportResult?.type === 'calendar' ? (<View style={styles.successBadge}>
                    <CheckCircle size={12} color={COLORS.success}/>
                    <Text style={styles.successText}>{lastImportResult.count}</Text>
                  </View>) : undefined, handleImportCalendarICS)}
    <View style={styles.dataDivider}/>
              ...
              <View style={styles.calendarFeedSection}>
                <TouchableOpacity style={styles.publishFeedButton} onPress={handlePublishCalendarFeed} activeOpacity={0.7} disabled={isPublishingFeed}>
                  {isPublishingFeed ? (<ActivityIndicator size="small" color={COLORS.white}/>) : (<Rss size={16} color={COLORS.white}/>)}
              ...
              {renderSettingRow(<Upload size={18} color={COLORS.navyDeep}/>, 'Offers CSV', isExporting ? (<ActivityIndicator size="small" color={COLORS.navyDeep}/>) : (<Text style={styles.countBadge}>
                    {dataStats.sailings} sailings
                  </Text>), handleExportOffersCSV)}
              {renderSettingRow(<Ship size={18} color={COLORS.navyDeep}/>, 'Booked Cruises CSV', isExporting ? (<ActivityIndicator size="small" color={COLORS.navyDeep}/>) : (<Text style={styles.countBadge}>
                    {dataStats.booked} booked
                  </Text>), handleExportBookedCSV)}
              {renderSettingRow(<Download size={18} color={COLORS.navyDeep}/>, 'Calendar (.ics)', isExporting ? (<ActivityIndicator size="small" color={COLORS.navyDeep}/>) : (<Text style={styles.countBadge}>
                    {dataStats.events} events
                  </Text>), handleExportCalendarICS)}
        {renderSettingRow(<FolderArchive size={18} color={COLORS.success}/>, 'Export All App Data', isExportingAll ? (<ActivityIndicator size="small" color={COLORS.navyDeep}/>) : (<Text style={styles.countBadge}>
                    Complete
                  </Text>), handleExportAllData)}
              {renderSettingRow(<FolderInput size={18} color={COLORS.info}/>, 'Restore from Backup', isImportingAll ? (<ActivityIndicator size="small" color={COLORS.navyDeep}/>) : undefined, handleImportAllData)}
              {renderSettingRow(<Save size={18} color="#9C27B0"/>, 'Save as Mock Data', isSavingMockData ? (<ActivityIndicator size="small" color="#9C27B0"/>) : (<Text style={styles.countBadge}>
                    .ts files
                  </Text>), handleSaveMockData)}
              ...
              {renderSettingRow(<Download size={18} color="#5a2ea6"/>, 'Download Chrome Extension', isDownloadingExtension ? (<ActivityIndicator size="small" color="#5a2ea6"/>) : (<Text style={styles.countBadge}>v1.0.0</Text>), handleDownloadExtension)}
              {renderSettingRow(<FileSpreadsheet size={18} color={COLORS.success}/>, 'Download Booked CSV Template', isDownloadingTemplate ? (<ActivityIndicator size="small" color={COLORS.success}/>) : (<Text style={styles.countBadge}>Template</Text>), handleDownloadCSVTemplate)}
          ...
          <View style={styles.section}>
            <View style={styles.sectionCard}>
              ...
              {renderSettingRow(<HelpCircle size={18} color={COLORS.navyDeep}/>, 'Help Center', undefined, () => Alert.alert('Getting Started Guide', `STEP 1: Install Chrome Extension
━━━━━━━━━━━━━━━━━━━━━━━━━
...
✓ You're all set! Start exploring your cruise data.`))}
              {renderSettingRow(<BookOpen size={18} color={COLORS.navyDeep}/>, 'User Manual', <ChevronRight size={14} color={CLEAN_THEME.text.secondary}/>, () => setIsUserManualVisible(true))}
              {renderSettingRow(<BookOpen size={18} color={COLORS.navyDeep}/>, 'Purchase "Smooth Sailing (In Rough Waters)" on Amazon', <ExternalLink size={14} color={CLEAN_THEME.text.secondary}/>, () => handleOpenLink('https://www.amazon.com/Smooth-Sailing-Rough-Waters-Consistently/dp/B0G4NMSM31/ref=sr_1_1?crid=BWS5ZWAQCC46&dib=eyJ2IjoiMSJ9.pTShQ0uJgtzeHg_EAFai2a6YTAan0h_35hcv7ZH0QKfGjHj071QN20LucGBJIEps.F_tIgnCOSc3EqGF6wUtOWK_hXH-5Ti3Miy6KYQ_JaLY&dib_tag=se&keywords=smooth+sailing+in+rough+waters&qid=1766758613&s=books&sprefix=smooth+sailing+in+rough+water%2Cstripbooks%2C189&sr=1-1'))}
              {renderSettingRow(<Star size={18} color={COLORS.navyDeep}/>, 'Rate App', <ExternalLink size={14} color={CLEAN_THEME.text.secondary}/>, () => handleOpenLink('https://apps.apple.com/us/app/easy-seas/id6758175890?ppid=9a051237-cab0-4164-9459-4c55a1976721'))}
            </View>
          </View>
          ...
          <View style={styles.section}>
            <View style={styles.sectionCard}>
              ...
              <View style={styles.subscriptionStatusBanner}>
                <Crown size={18} color={entitlement.subscriptionDisplayStatus === 'annual' ? '#10B981' :
            ...
            entitlement.subscriptionDisplayStatus === 'monthly' ? 'Monthly plan active — all features unlocked' :
              ...
              <View style={styles.dataDivider}/>
              {renderSettingRow(<RefreshCcw size={18} color={COLORS.navyDeep}/>, 'Restore Purchases', <ChevronRight size={14} color={CLEAN_THEME.text.secondary}/>, () => { void entitlement.restore(); })}
              {renderSettingRow(<ExternalLink size={18} color={COLORS.navyDeep}/>, 'Manage Subscriptions', <ExternalLink size={14} color={CLEAN_THEME.text.secondary}/>, () => { void entitlement.openManageSubscription(); })}
              {renderSettingRow(<Calendar size={18} color={COLORS.navyDeep}/>, 'Purchase a Monthly Subscription', <ChevronRight size={14} color={CLEAN_THEME.text.secondary}/>, () => router.push('/paywall-monthly' as any))}
              {renderSettingRow(<Crown size={18} color={COLORS.navyDeep}/>, 'Purchase an Annual Subscription', <ChevronRight size={14} color={CLEAN_THEME.text.secondary}/>, () => router.push('/paywall' as any))}
              <View style={styles.dataDivider}/>
              {renderSettingRow(<Shield size={18} color={COLORS.navyDeep}/>, 'Privacy Policy', <ExternalLink size={14} color={CLEAN_THEME.text.secondary}/>, () => entitlement.openPrivacyPolicy())}
              {renderSettingRow(<Shield size={18} color={COLORS.navyDeep}/>, 'Terms of Use (EULA)', <ExternalLink size={14} color={CLEAN_THEME.text.secondary}/>, () => entitlement.openTerms())}
            </View>
          ...
          <View style={styles.section}>
            <View style={[styles.sectionCard, styles.dangerCard]}>
              ...
              {renderSettingRow(<Trash2 size={18} color={COLORS.error}/>, 'Clear All Data', undefined, handleClearData, true)}
            </View>
          </View>

    <View style={styles.section}>
            <View style={styles.aboutPromoSection}>
              <Image source={{ uri: 'https://pub-e001eb4506b145aa938b5d3badbff6a5.r2.dev/attachments/h9viq065jv5liw4sy78sl' }} style={styles.aboutBanner} resizeMode="cover"/>
              <View style={styles.aboutQrRow}>
                <Image source={{ uri: 'https://pub-e001eb4506b145aa938b5d3badbff6a5.r2.dev/attachments/t70pmy9bqdf95tfyftktj' }} style={styles.aboutQrCode} resizeMode="contain"/>
                <View style={styles.aboutQrText}>
                  ...
                  <TouchableOpacity onPress={() => handleOpenLink('https://apps.apple.com/us/app/easy-seas/id6758175890?ppid=9a051237-cab0-4164-9459-4c55a1976721')} style={styles.aboutAppStoreButton} activeOpacity={0.7}>
      ...
      <UserManualModal visible={isUserManualVisible} onClose={() => setIsUserManualVisible(false)}/>
    </View>);
}
