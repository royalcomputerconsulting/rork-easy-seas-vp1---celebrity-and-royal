...
export default function SettingsScreen() {
  ...
  const dataStats = useMemo(() => {
    ...
    return result;
  }, [coreData.isLoading, cruiseInventoryCount, cruises.length, bookedCruises, casinoOffers, inventoryAvailableCruises, inventoryAvailableOptions, inventoryPhysicalSailings, localData.booked, localData.calendar, localData.cruises?.length, localData.offers, localData.tripit, myAtlasMachines.length, crewStats, totalOfferSailingRelationships]);
  ...
  const handleExportAllData = useCallback(async () => {
    try {
      ...
      recordDiagnosticEvent({
        ...
        message: 'Save All flushed pending writes and opened the indexed encrypted backup workflow',
        data: {
      ...
      setDataOperation((current) => current?.id === 'save-all' ? { ...current, current: 1, committed: true, status: 'success', completedAt: new Date().toISOString(), message: 'Pending writes are durable. Opening encrypted backup.' } : current);
      router.push({ pathname: '/data-trust-center', params: { intent: 'backup' } } as any);
    } catch (error) {
  ...
  }, [bookedCruises.length, casinoOffers.length, coreData, router, searchableCertificates]);
  ...
  const handleImportAllData = useCallback(async () => {
    try {
      ...
      recordDiagnosticEvent({
        ...
        message: 'Load All flushed pending writes and opened the conflict-safe encrypted restore workflow',
        data: { source: 'data_trust_center' },
      });
      router.push({ pathname: '/data-trust-center', params: { intent: 'restore' } } as any);
    } catch (error) {
      Alert.alert('Load All could not start', `${error instanceof Error ? error.message : 'Pending app writes could not be finalized.'} Existing saved data was preserved.`);
    } finally {
...
booked-liberty-1,Liberty of the Seas,10-16-2025,10-25-2025,9,9 Night Canada & New England Cruise,"Cape Liberty, NJ (NYC)","Cape Liberty, NJ (NYC)",324123,2,7676777,TRUE,,`;
  ...
  const settingsSearchItems: Array<{
    ...
    { id: 'security', group: 'Security', label: requiresCredentialEnrollment ? 'Set device PIN' : 'Device protection', detail: 'Protect locally stored reservations, loyalty, and casino data', onPress: () => { if (requiresCredentialEnrollment) setIsCredentialEnrollmentVisible(true); } },
    ...
    { id: 'save-all', group: 'Data Import & Backup', label: 'Save all app data', detail: 'Create an indexed, encrypted, resumable Easy Seas backup', onPress: () => void handleExportAllData() },
    { id: 'load-all', group: 'Data Import & Backup', label: 'Restore from backup', detail: 'Preview every change before a conflict-safe restore', onPress: () => void handleImportAllData() },
    ...
    { id: 'trust', group: 'Appearance', label: 'Data Trust Center', detail: 'Integrity, provenance, repairs, and restore diagnostics', onPress: () => router.push('/data-trust-center' as any) },
    ...
    { id: 'books', group: 'About', label: 'Books by Scott Astin', detail: 'Open all Scott Astin books on Amazon', onPress: () => void handleOpenLink(SCOTT_ASTIN_ALL_BOOKS_URL) },
  ...
  const settingsShortcutItems = [
    ...
    { label: 'Data trust', emoji: '🧭', onPress: () => router.push('/data-trust-center' as any) },
  ...
  return (
    <View style={[styles.container, { backgroundColor: experienceColors.background }]}> 
      ...
      <SafeAreaView style={styles.safeArea} edges={['top']}>
        ...
        >
          <ResponsiveContainer>
            <TabIdentityBand tab="settings" compact detail={`${dataStats.cruises.toLocaleString()} cruises · ${dataStats.booked.toLocaleString()} booked`} />
            ...
            >
              <View style={styles.dataOverviewHeaderContent}>
                ...
                <View style={styles.dataOverviewTitleGroup}>
                  ...
                  <Text style={styles.dataOverviewSubtitle}>{coreData.isLoading ? 'Restoring saved data…' : `${dataStats.cruises.toLocaleString()} cruises in system`}</Text>
                </View>
              </View>
            </LinearGradient>
            <View style={styles.dataOverviewBody}>
              ...
              <View style={styles.dataOverviewStatsRow}>
                <View style={styles.dataOverviewStatCard}>
                  ...
                  <Text style={styles.dataOverviewMiniStat}>{dataStats.physicalSailings.toLocaleString()} physical</Text>
                  <Text style={styles.dataOverviewStatValue}>{dataStats.cruises}</Text>
                  <Text style={styles.dataOverviewStatLabel}>Available options</Text>
                </View>
                <View style={styles.dataOverviewStatCard}>
              ...
              <View style={styles.dataOverviewGrid}>
                ...
                <View style={styles.dataOverviewGridCard}>
                  ...
                  <Text style={styles.dataOverviewGridLabel}>Unique Crew</Text>
                  <Text style={styles.dataOverviewMiniStat}>{dataStats.crewRecognitionEntries} records</Text>
                </View>
              </View>
                </>
              )}
          ...
          <View style={styles.settingsSearchCard} testID="settings-search-card">
              ...
              <View style={styles.settingsShortcutSection}>
                <View style={styles.settingsGroupSummary} testID="settings-group-summary">
                  {settingsShortcutItems.map((item) => {
                    ...
                    return <TouchableOpacity key={item.label} style={[styles.settingsGroupChip, isActive && styles.settingsGroupChipActive]} onPress={() => { setSettingsSearchQuery(''); if (item.onPress) item.onPress(); else if (item.group) setActiveSettingsGroup(item.group); }} accessibilityRole="button" accessibilityState={{ selected: isActive }} testID={`settings-shortcut-${item.label.toLowerCase().replace(/[^a-z0-9]+/g, '-')}`}>
          ...
          {activeSettingsGroup === 'Notifications' ? <View style={[styles.sectionCard, { marginBottom: SPACING.md }]} testID="voyage-notification-settings-card">
            {renderSectionHeader(<Bell size={18} color={COLORS.white} />, 'Notifications', 'Voyage reminders are local, actionable & deduplicated', ['#6D28D9', '#7C3AED'])}
          ...
          <View style={[styles.sectionCard, { marginBottom: SPACING.md }]}> 
            ...
            >
              ...
              <Text style={styles.quickActionLabelInline}>Import offers CSV</Text>
              <ChevronRight size={16} color={CLEAN_THEME.text.secondary} />
            </TouchableOpacity>
            <TouchableOpacity 
              style={styles.quickActionFullWidth} 
            ...
            <TouchableOpacity
              style={[styles.quickActionFullWidth, isImportingLegacyBackup && styles.quickActionBusy]}
              onPress={() => void handleImportLegacyBackup()}
              activeOpacity={0.7}
              disabled={isImportingLegacyBackup}
              accessibilityRole="button"
              accessibilityLabel="Import an earlier Easy Seas JSON backup"
              accessibilityState={{ disabled: isImportingLegacyBackup, busy: isImportingLegacyBackup }}
              testID="settings-import-earlier-backup"
            >
              <View style={[styles.quickActionIconSmall, { backgroundColor: 'rgba(28, 47, 122, 0.10)' }]}>
                {isImportingLegacyBackup ? (
                  <ActivityIndicator size="small" color={COLORS.navyDeep} />
                ) : (
                  <FolderArchive size={16} color={COLORS.navyDeep} />
                )}
              </View>
              <View style={{ flex: 1 }}>
                <Text style={styles.quickActionLabelInline}>{isImportingLegacyBackup ? 'Importing earlier backup…' : 'Import earlier Easy Seas backup'}</Text>
                <Text style={styles.subsectionHelper}>For unencrypted Easy Seas JSON backups created by earlier app versions</Text>
              </View>
              <ChevronRight size={16} color={CLEAN_THEME.text.secondary} />
            </TouchableOpacity>
            <View style={styles.quickActionsRow}>
              <TouchableOpacity 
                style={[styles.quickActionHalf, isExportingAll && styles.quickActionBusy]} 
                onPress={() => void handleExportAllData()}
                activeOpacity={0.7}
                disabled={isExportingAll}
                accessibilityRole="button"
                accessibilityLabel="Save All Easy Seas data"
                accessibilityState={{ disabled: isExportingAll, busy: isExportingAll }}
              ...
              >
                ...
                <Text style={styles.quickActionLabelInline}>{isExportingAll ? 'Saving…' : 'Save all'}</Text>
              </TouchableOpacity>
              <TouchableOpacity 
                style={[styles.quickActionHalf, isImportingAll && styles.quickActionBusy]} 
                onPress={() => void handleImportAllData()}
                activeOpacity={0.7}
                disabled={isImportingAll}
                accessibilityRole="button"
                accessibilityLabel="Load All Easy Seas data from backup"
                accessibilityState={{ disabled: isImportingAll, busy: isImportingAll }}
              ...
              >
                ...
                <Text style={styles.quickActionLabelInline}>{isImportingAll ? 'Loading…' : 'Load Encrypted Backup'}</Text>
              </TouchableOpacity>
            </View>
            </View>
          </View>

          {activeSettingsGroup === 'About / Legal' ? <View style={styles.section} testID="settings-scott-astin-books">
            <View style={styles.sectionCard}>
...
<View style={styles.dataDivider} />
              ...
              {renderExportActionRow({
                ...
                label: 'Export All App Data',
                detail: 'Indexed, encrypted, resumable complete backup',
                testID: 'settings-export-all-app-data',
                busy: isExportingAll,
              ...
              {renderExportActionRow({
                ...
                detail: 'Preview additions, updates, conflicts, and rejected rows first',
                testID: 'settings-restore-from-backup',
                busy: isImportingAll,
                onPress: handleImportAllData,
              })}
            </View>
            <Text style={styles.backupHint}>
              Full encrypted backup includes all cruises, offers, events, casino sessions, personal certificates, shared downloaded Club Royale certificate PDFs and parsed sailing rows, user profile, casino program points, loyalty points, crew recognition, provenance, relationships, preferences, and settings.
            </Text>
          ...
          {activeSettingsGroup === 'Security' ? <View style={[styles.sectionCard, { marginBottom: SPACING.md }]} testID="secure-access-settings-card">
          ...
          {activeSettingsGroup === 'Integrations' ? <View style={styles.section} testID="settings-integrations-section">
            <View style={styles.sectionCard}>
          ...
          {activeSettingsGroup === 'Appearance' ? <View style={styles.section}>
            <View style={styles.sectionCard}>
          ...
          {activeSettingsGroup === 'Help' ? <View style={styles.section}>
            <View style={styles.sectionCard}>
...
✓ You're all set! Start exploring your cruise data.`
          ...
          {activeSettingsGroup === 'Purchases' ? <View style={styles.section}>
            <View style={styles.sectionCard}>
...
<View style={styles.dataDivider} />
          ...
          {activeSettingsGroup === 'Danger Zone' ? <View style={styles.section}>
          ...
          {activeSettingsGroup === 'About / Legal' ? <View style={styles.section}>
          ...
          {activeSettingsGroup === 'About / Legal' ? <View style={styles.footer}>
