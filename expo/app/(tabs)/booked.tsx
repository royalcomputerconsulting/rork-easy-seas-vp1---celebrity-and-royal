...
import { DataStateCard, MetricGrid, SegmentedControl } from '@/components/ui/EasySeasPrimitives';
...
const BOOKED_HERO_FALLBACK = require('../../assets/images/section-themes/booked-voyages-v1.png');
...
export default function BookedScreen() {
  ...
  const [showCasinoSummaryDetails, setShowCasinoSummaryDetails] = useState(false);
  ...
  const [showRemoteHeroImage, setShowRemoteHeroImage] = useState(true);

  useEffect(() => {
    setShowRemoteHeroImage(true);
  }, [heroImage]);

  const renderRelationshipSections = useCallback(() => (
    <ResponsiveContainer>
              ...
              <View key={block.id} style={styles.consecutiveCard}>
                ...
                <View style={styles.consecutiveVoyageList}>
                  {block.voyages.map((group, voyageIndex) => (
                    <View key={group.key} testID={`booked-consecutive-voyage-card-${voyageIndex}`} accessibilityLabel={`Voyage ${voyageIndex + 1} of ${block.voyages.length}: ${group.voyage.shipName}`}>
                      <CruiseCard cruise={group.voyage} onPress={() => handleCruisePress(group.voyage)} variant="booked" mini relatedReservationCount={group.reservations.length} />
                    </View>
                  ))}
                </View>
              </View>
  ...
  ), [consecutiveVoyageBlocks, favoriteShipOptions, handleCruisePress]);

  const renderHeader = () => (
    <ResponsiveContainer>
      <View style={styles.headerContent}>
      <View style={[styles.heroContainer, !nextCruise && styles.heroContainerEmpty]} testID="booked-photo-identity-hero">
        <Image source={BOOKED_HERO_FALLBACK} style={styles.heroImage} resizeMode="cover" accessibilityIgnoresInvertColors />
        {showRemoteHeroImage ? (
          <Image
            source={{ uri: heroImage }}
            style={styles.heroImage}
            resizeMode="cover"
            onError={() => setShowRemoteHeroImage(false)}
        ...
        <View style={styles.heroContent}>
            ...
            <>
              ...
              >
                ...
                <Text style={styles.nextCruiseDest}>
                  {nextCruise.nights}-night · {nextCruise.destination || nextCruise.itineraryName || 'Caribbean'}
                </Text>
          ...
          <View style={styles.heroMetricsWrap}>
            <MetricGrid
              metrics={[
                { icon: '🕘', value: stats.upcoming.toLocaleString(), label: 'Upcoming' },
                { icon: '✓', value: stats.completed.toLocaleString(), label: 'Completed' },
                { icon: '✦', value: formatNumber(currentYearPoints), label: 'Season pts' },
              ]}
            />
          </View>
        </View>
      ...
      <View style={styles.casinoSection}>
        ...
        >
          ...
          <MetricGrid
            metrics={[
              { icon: '♠️', value: hasCasinoCruiseData ? formatCurrency(casinoStats.totalCoinIn) : '—', label: hasCasinoCruiseData ? (casinoStats.hasEstimates ? 'Modeled coin-in' : 'Recorded coin-in') : 'Coin-in not recorded' },
              { icon: '💵', value: hasCasinoCruiseData ? `${casinoStats.netResult >= 0 ? '+' : ''}${formatCurrency(casinoStats.netResult)}` : '—', label: 'Cash result' },
              { icon: '✦', value: formatNumber(currentYearPoints), label: 'Current season' },
            ]}
          />
          ...
          <TouchableOpacity
            ...
            onPress={() => setShowCasinoSummaryDetails((value) => !value)}
            accessibilityRole="button"
            accessibilityLabel={`${showCasinoSummaryDetails ? 'Hide' : 'Show'} casino value and evidence`}
            accessibilityState={{ expanded: showCasinoSummaryDetails }}
            testID="booked-casino-summary-details-toggle"
          >
            <View style={{ flex: 1 }}>
              <Text style={[styles.casinoEvidenceToggleTitle, { color: casinoCardTheme.topTextColor }]}>Value and evidence</Text>
              <Text style={[styles.casinoEvidenceToggleMeta, { color: casinoCardTheme.secondaryTextColor }]}>Retail value, paid amount, season reconciliation, and contributing cruises</Text>
            </View>
            <ChevronDown size={18} color={casinoCardTheme.accentColor} style={{ transform: [{ rotate: showCasinoSummaryDetails ? '180deg' : '0deg' }] }} />
          </TouchableOpacity>

          {showCasinoSummaryDetails ? <>
      ...
      <IntelligenceFilterStrip contextLabel="Booked" variant="bookedCruises" compact showProgram={false} />
        ...
        <View style={styles.viewModeToggle}>
          <SegmentedControl
            options={[
              ...
              { value: 'points', label: 'C&A Pts' },
            ]}
            value={viewMode}
            onChange={setViewMode}
            accessibilityLabel="Choose booked cruises view"
          />
        </View>
        <TouchableOpacity
      ...
      {viewMode === 'list' && (
        <>
          <MinimalistFilterBar
            ...
            actions={[
              { key: 'countries', label: 'Map', icon: Globe2, onPress: handleCountriesPress },
              { key: 'refresh', label: 'Refresh', icon: RotateCcw, onPress: onRefresh },
              { key: 'hide', label: hideCompleted ? 'Include done' : 'Upcoming only', icon: EyeOff, active: hideCompleted, onPress: () => setHideCompleted(!hideCompleted) },
          ...
          <ThemedSectionHeader
            ...
            emoji="🚢"
            title="My cruises"
            subtitle="Canonical voyage cards · tap any card for complete details"
            compact
      ...
      {viewMode !== 'list' ? renderRelationshipSections() : null}
      </View>
    </ResponsiveContainer>
  );
    ...
    <View style={[styles.container, { backgroundColor: experienceColors.background }]}> 
      ...
      <SafeAreaView style={styles.safeArea} edges={['top']}>
        <FlatList
          ...
          ListFooterComponent={viewMode === 'list' ? renderRelationshipSections : undefined}
...
const styles = StyleSheet.create({
  ...
  relationshipFooter: { paddingTop: 4, paddingBottom: 4 },
