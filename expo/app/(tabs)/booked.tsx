...
import { View, Text, StyleSheet, FlatList, TouchableOpacity, RefreshControl, Platform, ActivityIndicator, Image, } from 'react-native';
import { Stack, useRouter } from 'expo-router';
import { useDeferredRender } from '@/hooks/useDeferredRender';
import { SafeAreaView } from 'react-native-safe-area-context';
import { Ship, RotateCcw, EyeOff, X, ArrowUpDown, Clock, Award, Plus, List, CheckCircle, Anchor, Dice5, TrendingUp, Coins, Target, DollarSign, Crown, } from 'lucide-react-native';
...
export default function BookedScreen() {
    ...
    const { clubRoyalePoints: loyaltyClubRoyalePoints, clubRoyaleTier: loyaltyClubRoyaleTier, crownAnchorPoints, } = useLoyalty();
    ...
    const filteredCruises = useMemo(() => {
        ...
        if (searchQuery.trim()) {
            ...
            result = result.filter(cruise => cruise.shipName?.toLowerCase().includes(query) ||
    ...
    const renderCruiseCard = useCallback(({ item }: {
        item: BookedCruise;
    }) => {
        const isPast = isDateInPast(item.returnDate);
        return (<CruiseCard cruise={item} onPress={() => handleCruisePress(item)} variant={isPast ? 'completed' : 'booked'} mini={true}/>);
    }, [handleCruisePress]);
    const renderTimelineView = () => {
        ...
        <View style={styles.timelineSection}>
          ...
          {upcomingCruises.length === 0 ? (<View style={styles.timelineEmptyCard}>
              <Ship size={24} color={COLORS.textSecondary}/>
              <Text style={styles.timelineEmptyText}>No upcoming cruises</Text>
            </View>) : (<View style={styles.timelineVerticalList}>
              {upcomingCruises.map((cruise) => {
                    const daysUntil = getDaysUntilCruise(cruise.sailDate);
                    return (<View key={cruise.id} style={styles.timelineItemWrapper}>
                    
                    <View style={styles.timelineCardWrapper}>
                      <CruiseCard cruise={cruise} onPress={() => handleCruisePress(cruise)} variant="booked" mini={true}/>
                    </View>
                  </View>);
                })}
            </View>)}
        ...
        <View style={styles.timelineSection}>
          ...
          {completedCruises.length === 0 ? (<View style={styles.timelineEmptyCard}>
              ...
              {completedCruises.map((cruise) => {
                    ...
                    <View style={styles.timelineCardWrapper}>
                      <CruiseCard cruise={cruise} onPress={() => handleCruisePress(cruise)} variant="completed" mini={true}/>
                    </View>
                  </View>);
                })}
            </View>)}
    ...
    const renderHeader = () => (<View style={styles.headerContent}>
      ...
      <View style={styles.heroContainer}>
        ...
        <LinearGradient colors={['rgba(0, 31, 63, 0.3)', 'rgba(0, 31, 63, 0.85)', 'rgba(0, 31, 63, 0.95)']} style={styles.heroOverlay}/>
      ...
      <View style={styles.casinoSection}>
        <LinearGradient colors={['#E0F2FE', '#DBEAFE', '#E0F7FA']} start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }} style={styles.casinoGradient}>
      ...
      <View style={styles.viewModeRow}>
        <View style={styles.viewModeToggle}>
          <TouchableOpacity style={[styles.viewModeButton, viewMode === 'list' && styles.viewModeButtonActive]} onPress={() => setViewMode('list')} activeOpacity={0.7}>
          ...
          <TouchableOpacity style={[styles.viewModeButton, viewMode === 'timeline' && styles.viewModeButtonActive]} onPress={() => setViewMode('timeline')} activeOpacity={0.7}>
          ...
          <TouchableOpacity style={[styles.viewModeButton, viewMode === 'points' && styles.viewModeButtonActive]} onPress={() => setViewMode('points')} activeOpacity={0.7}>
        ...
        <TouchableOpacity style={styles.addCruiseButton} onPress={handleAddCruise} activeOpacity={0.7}>
    ...
    return (<View style={styles.container}>
      ...
      <SafeAreaView style={styles.safeArea} edges={['top']}>
        <FlatList data={viewMode === 'list' ? filteredCruises : ([] as BookedCruise[])} renderItem={renderCruiseCard} keyExtractor={(item) => item.id} contentContainerStyle={styles.listContent} ListHeaderComponent={renderHeader} ListEmptyComponent={viewMode === 'list' ? renderEmpty : undefined} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={onRefresh} tintColor={COLORS.beigeWarm} colors={[COLORS.beigeWarm]}/>} showsVerticalScrollIndicator={false} removeClippedSubviews={true} initialNumToRender={6} maxToRenderPerBatch={5} windowSize={7} updateCellsBatchingPeriod={50}/>
      </SafeAreaView>

      <AddBookedCruiseModal visible={showAddModal} onClose={() => setShowAddModal(false)} onSave={handleSaveNewCruise}/>
    </View>);
}
