...
const MachineCard = React.memo(function MachineCard({ machine, onPress, onAdd }: MachineCardProps) {
    return (<View style={styles.machineCard}>
      <TouchableOpacity style={styles.machineContent} onPress={() => onPress(machine.id)} activeOpacity={0.7}>
...
export default function GlobalLibraryScreen() {
    ...
    const { globalLibrary, addMachineFromGlobal, } = useSlotMachineLibrary();
    const { filteredLibrary: filteredGlobalLibrary, searchQuery, setSearchQuery, filterManufacturers, setFilterManufacturers, filterVolatility, setFilterVolatility, filterPersistence, setFilterPersistence, filterHasMHB, setFilterHasMHB, sortBy, setSortBy, clearAllFilters, activeFilterCount: filterCount, } = useSlotMachineFilters(globalLibrary);
    ...
    const renderMachineCard = useCallback(({ item }: ListRenderItemInfo<MachineEncyclopediaEntry>) => (<MachineCard machine={item} onPress={handleMachinePress} onAdd={handleAddMachine}/>), [handleMachinePress, handleAddMachine]);
    const ListHeader = useMemo(() => (<Text style={styles.resultCount}>
      {filteredGlobalLibrary.length} machines found
    </Text>), [filteredGlobalLibrary.length]);
    const ListEmpty = useMemo(() => (<View style={styles.emptyState}>
      ...
      <SafeAreaView style={styles.container} edges={['bottom']}>
        <View style={styles.searchSection}>
          <View style={styles.searchBar}>
            ...
            <TextInput style={styles.searchInput} placeholder="Search machines..." placeholderTextColor={COLORS.textMuted} value={searchQuery} onChangeText={setSearchQuery}/>
            
          </View>

          <View style={styles.filterRow}>
            <TouchableOpacity style={[styles.filterButton, activeFilterCount > 0 && styles.filterButtonActive]} onPress={() => setFilterModalVisible(true)} activeOpacity={0.7}>
        ...
        <FlatList data={filteredGlobalLibrary} renderItem={renderMachineCard} keyExtractor={keyExtractor} style={styles.scrollView} contentContainerStyle={styles.content} showsVerticalScrollIndicator={false} ListHeaderComponent={ListHeader} ListEmptyComponent={ListEmpty} initialNumToRender={10} maxToRenderPerBatch={10} windowSize={5} removeClippedSubviews={Platform.OS !== 'web'}/>

        <TouchableOpacity style={styles.bulkAddButton} onPress={() => router.push('/add-machines-to-ship' as any)} activeOpacity={0.7}>
        ...
        <Modal visible={filterModalVisible} animationType="slide" transparent={true} onRequestClose={() => setFilterModalVisible(false)}>
          <View style={styles.modalOverlay}>
            <View style={styles.modalContent}>
              ...
              <ScrollView style={styles.modalScroll} showsVerticalScrollIndicator={false}>
                ...
                <View style={styles.chipRow}>
                  {['Aristocrat', 'IGT', 'Konami', 'Light & Wonder', 'Bally', 'Everi'].map((mfg) => (<TouchableOpacity key={mfg} style={[
                styles.chip,
                ...
                <View style={styles.chipRow}>
                  {['Low', 'Medium-Low', 'Medium', 'Medium-High', 'High'].map((vol) => (<TouchableOpacity key={vol} style={[
                styles.chip,
                ...
                <View style={styles.chipRow}>
                  {['True', 'Pseudo', 'None'].map((type) => (<TouchableOpacity key={type} style={[
                styles.chip,
        ...
        ]} onPress={() => setFilterHasMHB(filterHasMHB === true ? undefined : true)} activeOpacity={0.7}>
                    <Text style={[
            styles.chipText,
            filterHasMHB === true && styles.chipTextActive
              ...
              <TouchableOpacity style={styles.applyButton} onPress={() => setFilterModalVisible(false)} activeOpacity={0.7}>
        ...
        <Modal visible={sortModalVisible} animationType="fade" transparent={true} onRequestClose={() => setSortModalVisible(false)}>
          <TouchableOpacity style={styles.modalOverlay} activeOpacity={1} onPress={() => setSortModalVisible(false)}>
            <View style={styles.sortModal}>
              {(['name', 'manufacturer', 'year', 'volatility'] as const).map((option) => (<TouchableOpacity key={option} style={[
                styles.sortOption,
