...
export function getResolvedCruiseItinerary(cruise: Cruise | BookedCruise): ItineraryDay[] | undefined {
  if (cruise.itinerary && cruise.itinerary.length > 0) {
    return cruise.itinerary.map((day, index) => ({
      day: day.day || index + 1,
      port: day.port || 'At Sea',
      arrival: day.arrival,
  ...
  const rawStops = [...(cruise.ports || []), ...(cruise.itineraryRaw || [])]
    .map((value) => String(value).trim())
    .filter((value) => value.length > 0);
  const totalDays = Math.max(2, (cruise.nights || 0) + 1);

  if (rawStops.length > 0) {
    const derived = rawStops.slice(0, totalDays).map((port, index) => ({
      day: index + 1,
      port,
      isSeaDay: determineSeaDay(port),
    }));

    if (derived.length < totalDays) {
      const filler: ItineraryDay[] = Array.from({ length: totalDays - derived.length }, (_, fillerIndex) => ({
        day: derived.length + fillerIndex + 1,
        port: 'At Sea',
        isSeaDay: true,
        casinoOpen: true,
      }));
      return [...derived, ...filler];
    }
  ...
  return Array.from({ length: totalDays }, (_, index) => {
    if (index === 0) {
      return {
        day: 1,
        port: cruise.departurePort || 'Embarkation',
        departure: '16:00',
        isSeaDay: false,
      };
    }
    ...
    if (index === totalDays - 1) {
      return {
        day: totalDays,
        port: cruise.departurePort || 'Return Port',
        arrival: '07:00',
        isSeaDay: false,
      };
    }
