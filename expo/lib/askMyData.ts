...
export function askMyDataSearch(params: {
  ...
  if (intent.sources.cruises || intent.wantsAllSources) {
    params.cruises.forEach((cruise) => {
      if (!cruisePassesStructuredFilters(cruise, intent)) return;
      const haystack = [cruise.shipName, cruise.destination, cruise.departurePort, cruise.itineraryName, cruise.offerCode, cruise.cabinType, cruise.sourceEmail, cruise.ownerProfileId, cruise.status, cruise.cruiseSource, cruise.ports?.join(' ')].filter(Boolean).join(' ');
      const textScore = scoreText(intent, haystack);
      const reasons: string[] = [];
      ...
      score += addReason(reasons, intent.wantsLowCost && ((cruise.price ?? cruise.totalPrice ?? 0) > 0), `price data available`, 12);
  ...
  console.log('[AskMyData] Semantic search completed:', {
    query: params.query,
    interpretedIntent: getInterpretedIntent(intent),
    filtersApplied,
  ...
  return {
    ...
    noResultsExplanation: rankedResults.length === 0 ? 'No matching records were found in the active profile/brand/program scope. Try a ship name, offer code, port, date, owner email, cabin type, or broaden your filters.' : undefined,
  };
}
