...
import { formatGuestEligibility } from '@/lib/cruiseRecordIntegrity';
...
function parseCalendarMonthConstraint(normalizedQuery: string): { month?: number; year?: number } {
  ...
  if (!monthMatch?.[1]) {
    const now = new Date();
    if (/\bnext\s+(?:calendar\s+)?month\b/.test(normalizedQuery)) {
      const next = new Date(now.getFullYear(), now.getMonth() + 1, 1);
      return { month: next.getMonth() + 1, year: next.getFullYear() };
    }
    if (/\bthis\s+(?:calendar\s+)?month\b/.test(normalizedQuery)) {
...
export function getAskMyDataDateWindow(query: string, now = new Date()): { sailDateFrom?: string; sailDateTo?: string } {
  ...
  } else if (/\bthis\s+(?:calendar\s+)?month\b/.test(normalizedQuery)) {
    month = now.getMonth() + 1;
    year = now.getFullYear();
  } else {
    const parsed = parseCalendarMonthConstraint(normalizedQuery);
    month = parsed.month;
    year = parsed.year;
  }
  if (!month || !year) return {};
  const sailDateFrom = `${year}-${String(month).padStart(2, '0')}-01`;
  const lastDay = new Date(year, month, 0).getDate();
  return { sailDateFrom, sailDateTo: `${year}-${String(month).padStart(2, '0')}-${String(lastDay).padStart(2, '0')}` };
}

const EUROPE_TERMS = [
  'europe', 'european', 'mediterranean', 'adriatic', 'baltic', 'norway', 'norwegian fjords', 'iceland',
  'spain', 'barcelona', 'madrid', 'malaga', 'valencia', 'ibiza', 'portugal', 'lisbon', 'porto',
  'italy', 'rome', 'civitavecchia', 'naples', 'venice', 'ravenna', 'florence', 'la spezia', 'sicily',
  'france', 'marseille', 'cannes', 'nice', 'greece', 'athens', 'piraeus', 'santorini', 'mykonos',
  'croatia', 'dubrovnik', 'split', 'england', 'southampton', 'london', 'scotland', 'ireland', 'amsterdam',
  'netherlands', 'denmark', 'copenhagen', 'sweden', 'stockholm', 'finland', 'helsinki', 'germany', 'hamburg',
];

function isEuropeText(value: unknown): boolean {
  const text = normalize(value);
  return EUROPE_TERMS.some((term) => text.includes(term));
}
...
function parseQueryIntent(query: string): QueryIntent {
  ...
  return {
    ...
    wantsEurope: /\beurope(?:an)?\b|\bmediterranean\b|\badriatic\b|\bbaltic\b|\bnorwegian\s+fjords?\b/.test(normalizedQuery),
    wantsPointsLevels: /\bpoints?\s*(?:level|levels|required|requirement)?\b|\b(?:level|levels)\s*(?:of\s*)?points?\b/.test(normalizedQuery),
...
function buildAnnualTierRewardAnswer(offers: CasinoOffer[], cruises: Cruise[]): { results: AskMyDataResult[]; directAnswer?: string } {
  ...
  cruises.forEach((cruise) => {
    ...
    matches.set(key, {
      ...
      actionRoute: buildCruiseDetailsPath(cruise, { source: 'agent-sea' }),
      confidence: 'high',
...
function offerPassesStructuredFilters(offer: CasinoOffer, intent: QueryIntent): boolean {
  ...
  if ((intent.calendarMonth || intent.calendarYear) && !offer.sailingDate) return false;
  if (intent.wantsEurope && !isEuropeText([offer.destination, offer.itinerary, offer.departurePort, offer.shipName].filter(Boolean).join(' '))) return false;
  return datePasses(offer.sailingDate, intent);
}
...
function cruisePassesStructuredFilters(cruise: Cruise, intent: QueryIntent): boolean {
  ...
  if (intent.wantsEurope && !isEuropeText([
    cruise.destination,
    ...
    cruise.ports?.join(' '),
    cruise.itinerary?.map((day) => day.port).join(' '),
...
function getInterpretedIntent(intent: QueryIntent): string {
  ...
  if (intent.wantsEurope) pieces.push('Europe');
  if (intent.wantsPointsLevels) pieces.push('certificate points levels');
...
function wantsOverviewResult(intent: QueryIntent): boolean {
  ...
  if ((intent.sources.cruises || intent.sources.offers || intent.sources.certificates) && (intent.calendarMonth || intent.wantsEurope)) return false;
...
export function askMyDataSearch(params: {
    ...
    params.offers.forEach((offer) => {
      ...
      if (score > 0 || reasons.length > 0 || intent.wantsExpiring || intent.wantsReviewNeeded) {
        results.push({
          ...
          actionRoute: buildOfferDetailsPath(offer),
  ...
  if (intent.sources.cruises || intent.sources.offers || intent.wantsAllSources) {
    params.cruises.forEach((cruise) => {
      ...
      const record = cruise as unknown as Cruise & Record<string, unknown>;
      const rowPoints = getFirstNumber(record, ['pointRequirement', 'pointsRequired', 'certificatePoints', 'pointsLevel']);
      ...
      if (score > 0 || reasons.length > 0 || intent.wantsBooked || intent.wantsReviewNeeded) {
        ...
        results.push({
          ...
          subtitle: [
            cruise.sailDate,
            cruise.destination || cruise.departurePort || 'destination missing',
            `${cruise.nights} nights`,
            cruise.offerCode ? `offer ${cruise.offerCode}` : null,
            cruise.cabinType || cruise.offerCategory || null,
            formatGuestEligibility(cruise),
            rowPoints !== null ? `${rowPoints.toLocaleString()} points` : null,
          ].filter(Boolean).join(' · '),
          score,
          owner: getOwner(cruise),
          actionLabel: 'View cruise',
  ...
  if (intent.sources.certificates || intent.sources.offers || intent.sources.cruises || intent.wantsAllSources || intent.wantsCertificateFit) {
    (params.certificates as ExtendedCertificate[]).forEach((certificate) => {
      ...
      const certificateHaystack = [
        ...
        certificate.sourcePdfUrl,
        certificate.sourceDocumentArchiveUri,
      ...
      const matchingSailings = parsedSailings
        .filter((sailing) => datePasses(sailing.sailingDate, intent))
        .filter((sailing) => !intent.wantsEurope || isEuropeText([
          sailing.departurePort,
          sailing.itinerary,
        ].filter(Boolean).join(' ')) || params.cruises.some((cruise) =>
          normalize(cruise.shipName) === normalize(sailing.shipName)
          && normalizeDate(cruise.sailDate) === normalizeDate(sailing.sailingDate)
          && isEuropeText([
            cruise.destination,
            cruise.destinationRegion,
            cruise.departurePort,
        ...
        .map((sailing) => {
          ...
          score += addReason(reasons, intent.calendarMonth !== undefined && datePasses(sailing.sailingDate, intent), `sails in requested month on ${sailing.sailingDate}`, 24);
          score += addReason(reasons, intent.wantsEurope, 'European itinerary/port match', 22);
          score += addReason(reasons, intent.wantsPointsLevels && Number.isFinite(sailing.pointRequirement), `${Number(sailing.pointRequirement).toLocaleString()} point level`, 26);
      ...
      matchingSailings.forEach(({ sailing, score, matchedTerms, reasons }) => {
        const benefits = [
          sailing.cabinCategory,
          formatGuestEligibility(sailing.guestCount ?? sailing.occupancy, 'Guests not stated'),
      ...
      const isStructuredSailingQuestion = Boolean(intent.calendarMonth || intent.calendarYear || intent.wantsEurope || intent.wantsPointsLevels);
      if ((!isStructuredSailingQuestion || matchingSailings.length > 0) && (certificateScore > 0 || certificateReasons.length > 0 || intent.wantsCertificateFit)) {
        results.push({
          id: `certificate-${certificate.id}`,
          source: 'certificates',
          title: certificate.label || certificate.certificateCode || `${certificate.type} certificate`,
          subtitle: `${certificate.status ?? 'available'} · $${(certificate.value ?? 0).toLocaleString()}${certificate.issueDate ? ` · issued ${certificate.issueDate}` : ''}${certificate.expiryDate ? ` · expires ${certificate.expiryDate}` : ''}${parsedSailings.length ? ` · ${parsedSailings.length.toLocaleString()} PDF sailing rows` : ''}`,
          ...
          detail: [
            certificate.description,
            certificate.sourcePdfUrl ? `Official PDF: ${certificate.sourcePdfUrl}` : null,
            certificate.sourceDocumentArchiveUri ? 'Original PDF retained locally.' : null,
            certificate.parserWarnings?.length ? `Parser warnings: ${certificate.parserWarnings.join('; ')}` : null,
  ...
  const shouldSearchAgentWideContext = intent.sources.system
    || intent.wantsAllSources
    || intent.tokens.length > 0;

  if (shouldSearchAgentWideContext) {
    additionalContextBlocks.forEach((block) => {
      ...
      score += addReason(reasons, !intent.wantsAllSources && textScore.matchedTerms.length > 0, 'Agent SEA matched this question against app-wide context', 18);
      const agentWideMatch = intent.sources.system || intent.wantsAllSources || textScore.matchedTerms.length > 0;
      score += addReason(reasons, agentWideMatch && block.detail.length > 0, 'detailed app data snapshot available', 8);
      if (!agentWideMatch) return;
  ...
  // Exact availability answers must inspect the complete filtered result set.
  // Limiting first caused false negatives whenever 24 higher-scoring rows
  // preceded a matching sailing in a large downloaded certificate.
  const allRankedResults = results.sort((left, right) => right.score - left.score);
  const rankedResults = allRankedResults.slice(0, 24);

  const catalogRows = allRankedResults.filter((result) =>
    result.id.startsWith('certificate-sailing-') || result.id.startsWith('cruise-') || result.id.startsWith('cruise-offer-'));
  let directAnswer: string | undefined;
  if (intent.calendarMonth && /\bicon\b/.test(intent.normalizedQuery) && /\b(?:is|are|part|included|available)\b/.test(intent.normalizedQuery)) {
    const iconRows = catalogRows.filter((result) => /\bicon\b/i.test(`${result.title} ${result.subtitle}`));
    directAnswer = iconRows.length > 0
      ? `Yes. Icon of the Seas appears in ${iconRows.length} matching saved offer/certificate sailing row${iconRows.length === 1 ? '' : 's'} for ${String(intent.calendarMonth).padStart(2, '0')}/${intent.calendarYear}.\n\n${iconRows.slice(0, 12).map((result, index) => `[S${index + 1}] ${result.title} — ${result.subtitle}`).join('\n')}`
      : `No Icon of the Seas sailing appears in the currently loaded offer and downloaded-certificate rows for ${String(intent.calendarMonth).padStart(2, '0')}/${intent.calendarYear}. This means “not present in saved data,” not guaranteed unavailable on Royal's website.`;
  } else if (intent.calendarMonth && intent.wantsEurope) {
    const distinct = Array.from(new Map(catalogRows.map((result) => [`${result.title}|${result.subtitle}`, result])).values());
    directAnswer = distinct.length > 0
      ? `I found ${distinct.length} matching European offer/certificate sailing option${distinct.length === 1 ? '' : 's'} for ${String(intent.calendarMonth).padStart(2, '0')}/${intent.calendarYear}. Each line retains its recorded certificate points, cabin, and guest level when available.\n\n${distinct.slice(0, 20).map((result, index) => `[S${index + 1}] ${result.title} — ${result.subtitle}`).join('\n')}`
      : `I found no European sailings in the currently loaded offer and downloaded-certificate rows for ${String(intent.calendarMonth).padStart(2, '0')}/${intent.calendarYear}.`;
  }
  ...
  console.log('[AskMyData] Semantic search completed:', {
    ...
    interpretedIntent,
    directAnswer,
    totalMatches: allRankedResults.length,
    filtersApplied,
...
export function formatAskMyDataResponse(response: AskMyDataResponse): string {
  ...
  if (response.results.length === 0) {
    return `I checked ${response.interpretedIntent} across the active local Easy Seas index, but I could not find a saved record that matches that question.\n\n${response.noResultsExplanation}`;
  }

  const topResult = response.results[0];
  const lines = response.results.slice(0, 8).map((result, index) => {
    ...
    return `[S${index + 1}] ${result.title}\n${result.subtitle}${owner}${offerScore}${certificateFit}${reasons}.${detail ? `\n${detail}` : ''}\nAction: ${result.actionLabel}. Confidence: ${result.confidence}.`;
  });

  const sourceCounts = response.results.reduce((counts, result) => {
    counts[result.source] = (counts[result.source] ?? 0) + 1;
    return counts;
  }, {} as Partial<Record<AskMyDataSource, number>>);
  const rankedSources = Object.entries(sourceCounts).sort((left, right) => right[1] - left[1]);
  const primarySource = rankedSources[0]?.[0] as AskMyDataSource | undefined;
  const sourceLabels = rankedSources.map(([source]) => source).join(', ');
  const highConfidenceCount = response.results.filter((result) => result.confidence === 'high').length;
  const visibleCount = response.results.length;
  const matchCount = response.totalMatches ?? visibleCount;
  const countPhrase = matchCount > visibleCount ? `${matchCount} matches (showing the strongest ${visibleCount})` : `${matchCount} match${matchCount === 1 ? '' : 'es'}`;
  const summaryBySource: Partial<Record<AskMyDataSource, string>> = {
    weather: `The strongest saved weather forecast is ${topResult.title}: ${topResult.subtitle}.${topResult.detail ? ` ${topResult.detail}` : ''}`,
    crew: `I found ${countPhrase} in your saved crew-recognition history. The strongest match is ${topResult.title} — ${topResult.subtitle}.`,
    cruises: `I found ${countPhrase} in your saved cruise history and availability records. The strongest match is ${topResult.title} — ${topResult.subtitle}.`,
    offers: `I found ${countPhrase} in your saved offers. The strongest match is ${topResult.title} — ${topResult.subtitle}.`,
    certificates: `I found ${countPhrase} in your downloaded certificate evidence. The strongest match is ${topResult.title} — ${topResult.subtitle}.`,
    calendar: `I found ${countPhrase} on your saved calendar and agenda. The next strongest match is ${topResult.title} — ${topResult.subtitle}.`,
    machines: `I found ${countPhrase} in your saved slot-machine and Machine Atlas records. The strongest match is ${topResult.title} — ${topResult.subtitle}.`,
    system: `I found ${countPhrase} in your saved financial, casino, and app-health evidence. The strongest match is ${topResult.title} — ${topResult.subtitle}.`,
    overview: topResult.detail || `${topResult.title}: ${topResult.subtitle}.`,
  };
  const summary = rankedSources.length === 1 && primarySource
    ? summaryBySource[primarySource] ?? `I found ${countPhrase} in ${primarySource}. The strongest match is ${topResult.title}.`
    : `Your question matched ${countPhrase} across ${sourceLabels}. The strongest answer evidence is ${topResult.title} — ${topResult.subtitle}. ${highConfidenceCount > 0 ? `${highConfidenceCount} visible match${highConfidenceCount === 1 ? '' : 'es'} use high-confidence saved evidence.` : 'The closest matches are ranked by evidence strength.'}`;

  return `${summary}\n\nWhat I checked: ${response.interpretedIntent}.\n\nEvidence Agent SEA used:\n${lines.join('\n\n')}\n\nYou can also ask: ${response.suggestedQueries.slice(0, 3).join(' | ')}`;
}
