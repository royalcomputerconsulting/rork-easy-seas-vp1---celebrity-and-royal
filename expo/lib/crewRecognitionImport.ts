...
function isCsvDocument(text: string): boolean {
  ...
  const hasShipColumn = first.some((header) => ['ship', 'shipname', 'vessel', 'vesselname', 'onboardship'].includes(header));
  const hasCrewColumn = first.some((header) => ['crewmember', 'crewmembername', 'crewname', 'fullname', 'name', 'crewid', 'crewidentifier'].includes(header));
  return hasShipColumn && hasCrewColumn;
}
...
function parseCsv(text: string, userId: string): CrewRecognitionImportResult {
  ...
  matrix.slice(1).forEach((cells, rowIndex) => {
    ...
    const shipName = value(row, 'Ship', 'Ship Name', 'Ship_Name', 'Vessel', 'Vessel Name');
    const fullName = value(row, 'Crew Member', 'Crew Member Name', 'Crew Name', 'Crew_Name', 'Full Name', 'Name');
