...
export function normalizeBrand(value: unknown): 'royal' | 'celebrity' | 'other' {
  const text = normalize(value);
  if (text.includes('celebrity') || text.includes('blue') || text === 'c') return 'celebrity';
  if (text.includes('royal') || text.includes('club') || text === 'r') return 'royal';
  return 'other';
}
...
export function isAvailableOfferCruise(cruise: AdvisorCruise): cruise is Cruise {
  ...
  return Boolean((cruise as Cruise).offerCode) && !('bookingId' in cruise) && (status === 'available' || source === 'royal' || source === 'celebrity' || status === '');
}
...
export function buildOfferRecommendations(cruises: Cruise[], bookedCruises: BookedCruise[], offers: CasinoOffer[]): OfferRecommendation[] {
  ...
  offers.forEach((offer) => {
    ...
    if (code && !offerByCode.has(code)) offerByCode.set(code, offer);
  });

  return cruises
    .filter((cruise) => Boolean(cruise.offerCode))
    .map((cruise) => {
      const offer = offerByCode.get(normalize(cruise.offerCode));
