...
import { getRoyalTierCertificateTradeInValue } from '@/lib/casinoPointTruth';
...
export function buildLedgerFromCruise(cruise: Record<string, unknown>): CruiseValueLedgerItem[] {
  ...
  const tierCertificateTradeInValue = getRoyalTierCertificateTradeInValue(cruise);
  add('club-royale-annual-cruise', 'Club Royale annual cruise value', n(cruise.clubRoyaleAnnualCruiseValue) || n(cruise.annualCruiseValue) || tierCertificateTradeInValue, 'club-royale', 'cruise-fare', 'comp');
  add('crown-anchor-milestone-cruise', 'Crown & Anchor milestone certificate value', n(cruise.crownAnchorCertificateValue) || n(cruise.pinnacleCertificateValue) || (/pinnacle/i.test(String(cruise.crownAnchorLevel ?? cruise.notes ?? '')) ? tierCertificateTradeInValue : 0), 'crown-anchor', 'cruise-fare', 'comp');
