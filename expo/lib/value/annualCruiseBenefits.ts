...
import { TIER_CERTIFICATE_TRADE_IN_VALUE } from '@/lib/casinoPointTruth';
...
export function createAnnualCruiseBenefit(input: Partial<AnnualCruiseBenefit> & { tier: AnnualCruiseBenefit['tier']; benefitYear: string }): AnnualCruiseBenefit {
  ...
  const defaultTierTradeInValue = input.tier === 'prime' || input.tier === 'signature'
    ? TIER_CERTIFICATE_TRADE_IN_VALUE
    : undefined;
  return {
    ...
    estimatedRetailValue: input.estimatedRetailValue ?? defaultTierTradeInValue,
