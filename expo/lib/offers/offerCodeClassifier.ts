...
export function classifyOfferCode(rawCode?: string | null): OfferCodeClassification {
  ...
  // Royal marketing campaigns such as 26TOR603, 26VAR503, 26BAF305,
  // and 26OCT104 retain the certificate ladder in their final two
  // characters. The preceding digit belongs to the campaign/version, not the
  // point level. Treat this as decoded code evidence (medium confidence),
  // while explicit provider/certificate points remain authoritative.
  const marketingLevel = code.match(/(VIP\d+|\d{2}A?|\d{2})$/)?.[1];
  if (marketingLevel && LADDER[marketingLevel]) {
    return {
      code,
      offerType: 'marketing-offer',
      levelCode: marketingLevel,
      pointsRequired: LADDER[marketingLevel],
      confidence: 'medium',
      notes: ['Point level decoded from the Royal campaign-code suffix.'],
    };
  }
