...
import { getCruiseGuestEligibility } from '@/lib/cruiseRecordIntegrity';
...
export function resolveOfferCabinEntitlement(offer: Partial<CasinoOffer> | undefined, sailings: readonly Cruise[]): OfferDisplayFact<string> {
  const rowCabins = Array.from(new Set(sailings.map((sailing) => text(sailing.cabinType)).filter(Boolean)));
  if (rowCabins.length > 1) return { value: null, label: 'Varies by sailing', evidence: 'sailing_rows', source: 'Eligible sailing rows', varies: true };
  ...
  const providerCabin = text(offer?.roomType ?? offer?.cabinType);
  ...
  return { value: null, label: 'Not supplied by Royal', evidence: 'missing', source: 'No cabin entitlement field', varies: false };
}
