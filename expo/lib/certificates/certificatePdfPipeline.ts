...
function decodePdfHex(value: string, unicodeMaps: PdfUnicodeMap[]): string {
  ...
  unicodeMaps.forEach((unicodeMap) => {
    ...
    // Prefer an explicit ToUnicode mapping when it covers the token. A mapped
    // one-character "L" and its raw encoded "$" have equal readability
    // scores; keeping the raw byte on that tie corrupts statement labels and
    // currency values.
    if (decoded && mapped / tokenCount >= 0.8) candidates.unshift(decoded);
  });

  return candidates.sort((left, right) => readableTextScore(right) - readableTextScore(left) || right.length - left.length)[0] ?? '';
}

function decodePdfLiteralWithUnicodeMaps(value: string, unicodeMaps: PdfUnicodeMap[]): string {
  if (!value || unicodeMaps.length === 0) return value;
  const hex = Array.from(value)
    .map((character) => (character.charCodeAt(0) & 0xff).toString(16).padStart(2, '0'))
    .join('');
  return decodePdfHex(hex, unicodeMaps) || value;
}
...
function readPdfTextArray(value: string, start: number, unicodeMaps: PdfUnicodeMap[]): { text: string; end: number } {
  ...
  while (position < value.length && value[position] !== ']') {
    ...
    if (value[position] === '(') {
      ...
      word += decodePdfLiteralWithUnicodeMaps(literal.text, unicodeMaps);
...
function extractTextFromPdfContentStream(stream: string, unicodeMaps: PdfUnicodeMap[], fontUnicodeMaps: Map<string, PdfUnicodeMap>): string {
  const segments: string[] = [];
  let activeFontMap: PdfUnicodeMap | undefined;
  let position = 0;
  while (position < stream.length) {
    while (/\s/.test(stream[position] ?? '')) position += 1;
    if (stream[position] === '/') {
      const font = stream.slice(position).match(/^\/([A-Za-z0-9_-]+)\s+[-\d.]+\s+Tf\b/);
      if (font) {
        activeFontMap = fontUnicodeMaps.get(font[1]);
        position += font[0].length;
        continue;
      }
    }
    const availableMaps = activeFontMap ? [activeFontMap] : unicodeMaps;
    if (stream[position] === '[') {
      const saved = position;
      const array = readPdfTextArray(stream, position + 1, availableMaps);
      position = array.end;
      while (/\s/.test(stream[position] ?? '')) position += 1;
    ...
    if (stream[position] === '(') {
      ...
      if (stream.slice(position, position + 2) === 'Tj' || stream[position] === "'") {
        ...
        const decoded = decodePdfLiteralWithUnicodeMaps(literal.text, availableMaps);
    ...
    if (stream[position] === '<' && stream[position + 1] !== '<') {
      ...
      if (close > position) {
        const decoded = decodePdfHex(stream.slice(position + 1, close), availableMaps);
        position = close + 1;
        while (/\s/.test(stream[position] ?? '')) position += 1;
        if (stream.slice(position, position + 2) === 'Tj' || stream[position] === "'") {
          position += stream[position] === "'" ? 1 : 2;
...
function extractCompressedPdfText(bytes: Uint8Array): string {
  ...
  const decodedStreams: Array<{ objectNumber: number; stream: string; printableRatio: number }> = [];
  ...
  while ((headerMatch = objectHeaderPattern.exec(raw)) !== null) {
    ...
    const objectNumber = Number.parseInt(headerMatch[0], 10);
    decodedStreams.push({ objectNumber, stream: decodedStream, printableRatio });
  }
  ...
  const unicodeMapEntries = decodedStreams
    .map(({ objectNumber, stream }) => ({ objectNumber, map: parsePdfUnicodeMap(stream) }))
    .filter((entry): entry is { objectNumber: number; map: PdfUnicodeMap } => entry.map !== null);
  const unicodeMaps = unicodeMapEntries.map((entry) => entry.map);
  const unicodeMapsByObject = new Map(unicodeMapEntries.map((entry) => [entry.objectNumber, entry.map]));
  const fontUnicodeMaps = new Map<string, PdfUnicodeMap>();
  for (const fontBlock of raw.matchAll(/\/Font\s*<<([\s\S]*?)>>/g)) {
    for (const resource of fontBlock[1].matchAll(/\/([A-Za-z0-9_-]+)\s+(\d+)\s+\d+\s+R/g)) {
      const fontObject = Number(resource[2]);
      const fontBody = raw.match(new RegExp(`(?:^|\\n)${fontObject}\\s+\\d+\\s+obj\\b([\\s\\S]*?)endobj`))?.[1] ?? '';
      const toUnicodeObject = Number(fontBody.match(/\/ToUnicode\s+(\d+)\s+\d+\s+R/)?.[1]);
      const unicodeMap = unicodeMapsByObject.get(toUnicodeObject);
      if (unicodeMap) fontUnicodeMaps.set(resource[1], unicodeMap);
    }
  }
  ...
  decodedStreams.forEach(({ stream, printableRatio }) => {
    ...
    const text = extractTextFromPdfContentStream(stream, unicodeMaps, fontUnicodeMaps);
