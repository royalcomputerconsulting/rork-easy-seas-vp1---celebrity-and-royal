...
export interface CertificateMonthAvailability {
  currentMonthCode: string;
  currentMonthLabel: string;
  nextMonthCode: string;
  nextMonthLabel: string;
  nextMonthAvailable: boolean;
  nextMonthOpensOn: string;
  nextMonthOpensOnLabel: string;
  daysUntilNextMonthAvailable: number;
}
...
/**
 * Royal normally publishes the following month's A/C catalog near month-end.
 * Keep the next-month UI closed until the final ten calendar days while still
 * retaining every previously downloaded certificate document in local storage.
 */
export function getCertificateMonthAvailability(now = new Date(), publicationWindowDays = 10): CertificateMonthAvailability {
  const safeNow = Number.isNaN(now.getTime()) ? new Date() : now;
  const safeWindow = Math.max(1, Math.min(20, Math.trunc(publicationWindowDays) || 10));
  const daysInMonth = new Date(safeNow.getFullYear(), safeNow.getMonth() + 1, 0).getDate();
  const openingDay = Math.max(1, daysInMonth - safeWindow + 1);
  const opensOn = new Date(safeNow.getFullYear(), safeNow.getMonth(), openingDay);
  const todayStart = new Date(safeNow.getFullYear(), safeNow.getMonth(), safeNow.getDate());
  const daysUntil = Math.max(0, Math.ceil((opensOn.getTime() - todayStart.getTime()) / 86_400_000));
  return {
    currentMonthCode: getMonthCodeForTarget('thisMonth', safeNow),
    currentMonthLabel: getMonthLabelForTarget('thisMonth', safeNow),
    nextMonthCode: getMonthCodeForTarget('nextMonth', safeNow),
    nextMonthLabel: getMonthLabelForTarget('nextMonth', safeNow),
    nextMonthAvailable: localDateKey(todayStart) >= localDateKey(opensOn),
    nextMonthOpensOn: localDateKey(opensOn),
    nextMonthOpensOnLabel: opensOn.toLocaleDateString('en-US', { month: 'short', day: 'numeric' }),
    daysUntilNextMonthAvailable: daysUntil,
  };
}
