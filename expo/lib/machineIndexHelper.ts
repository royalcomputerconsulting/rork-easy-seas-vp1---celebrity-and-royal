...
let bundledMachinesCache: any[] | null = null;

/**
 * Keep the full bundled atlas out of ordinary startup/provider evaluation.
 * Metro still packages the JSON for offline use, but it is not parsed until a
 * Slots, machine-detail, export, or Agent SEA machine request explicitly asks
 * the repository for the index/details.
 */
function loadBundledMachines(): any[] {
  if (bundledMachinesCache) return bundledMachinesCache;
  // eslint-disable-next-line @typescript-eslint/no-require-imports
  const moduleValue = require('@/assets/MACHINES_262.json');
  const raw = moduleValue?.default ?? moduleValue;
  bundledMachinesCache = Array.isArray(raw) ? raw : [];
  return bundledMachinesCache;
}
...
async function getMachinesData(): Promise<any[]> {
  const bundledMachines = loadBundledMachines();
  const sharedMachines = await fetchSharedMachines();
  const mergedMachines = mergeMachineData(bundledMachines, sharedMachines);
  console.log(`[MachineIndex] Using ${mergedMachines.length} machines (${bundledMachines.length} bundled + ${sharedMachines.length} shared)`);
