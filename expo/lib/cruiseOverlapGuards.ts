...
/**
 * Royal Caribbean's published itineraries for the user's three confirmed
 * Harmony sailings in September 2026. These are voyage facts, not casino-
 * offer guesses, and intentionally omit port times Royal did not publish in
 * the retained evidence.
 *
 * Sources:
 * - https://www.royalcaribbean.com/itinerary/5-night-bahamas-perfect-day-cruise-from-orlando-port-canaveral-on-harmony-HM5BH353?country=USA&groupId=HM05PCN-3790280278&packageCode=HM5BH353&sailDate=2026-09-10
 * - https://www.royalcaribbean.com/itinerary/4-night-bahamas-perfect-day-from-orlando-on-harmony-HM04PCN-4069340908?country=USA&currencyCode=USD&sailDate=2026-09-15
 * - https://www.royalcaribbean.com/itinerary/7-night-eastern-caribbean-perfect-day-from-orlando-port-canaveral-on-harmony-HM07E503?country=USA&groupId=HM07PCN-4284516355&packageCode=HM07E503&sailDate=2026-09-19
 */
const VERIFIED_HARMONY_SEPTEMBER_2026_ITINERARIES: Record<string, ItineraryDay[]> = {
  '2026-09-10': [
    { day: 1, port: 'Port Canaveral, Florida', departure: '16:00', isSeaDay: false, source: 'public_document', dataConfidence: 'verified' },
    { day: 2, port: 'Nassau, Bahamas', isSeaDay: false, source: 'public_document', dataConfidence: 'verified' },
    { day: 3, port: 'At Sea', isSeaDay: true, source: 'public_document', dataConfidence: 'verified' },
    { day: 4, port: 'Perfect Day at CocoCay, Bahamas', isSeaDay: false, source: 'public_document', dataConfidence: 'verified' },
    { day: 5, port: 'At Sea', isSeaDay: true, source: 'public_document', dataConfidence: 'verified' },
    { day: 6, port: 'Port Canaveral, Florida', isSeaDay: false, source: 'public_document', dataConfidence: 'verified' },
  ],
  '2026-09-15': [
    { day: 1, port: 'Port Canaveral, Florida', departure: '16:00', isSeaDay: false, source: 'public_document', dataConfidence: 'verified' },
    { day: 2, port: 'At Sea', isSeaDay: true, source: 'public_document', dataConfidence: 'verified' },
    { day: 3, port: 'Perfect Day at CocoCay, Bahamas', arrival: '07:00', departure: '17:00', isSeaDay: false, source: 'public_document', dataConfidence: 'verified' },
    { day: 4, port: 'Nassau, Bahamas', isSeaDay: false, source: 'public_document', dataConfidence: 'verified' },
    { day: 5, port: 'Port Canaveral, Florida', isSeaDay: false, source: 'public_document', dataConfidence: 'verified' },
  ],
  '2026-09-19': [
    { day: 1, port: 'Port Canaveral, Florida', departure: '16:00', isSeaDay: false, source: 'public_document', dataConfidence: 'verified' },
    { day: 2, port: 'At Sea', isSeaDay: true, source: 'public_document', dataConfidence: 'verified' },
    { day: 3, port: 'At Sea', isSeaDay: true, source: 'public_document', dataConfidence: 'verified' },
    { day: 4, port: 'Charlotte Amalie, St. Thomas', isSeaDay: false, source: 'public_document', dataConfidence: 'verified' },
    { day: 5, port: 'At Sea', isSeaDay: true, source: 'public_document', dataConfidence: 'verified' },
    { day: 6, port: 'Perfect Day at CocoCay, Bahamas', isSeaDay: false, source: 'public_document', dataConfidence: 'verified' },
    { day: 7, port: 'Nassau, Bahamas', isSeaDay: false, source: 'public_document', dataConfidence: 'verified' },
    { day: 8, port: 'Port Canaveral, Florida', isSeaDay: false, source: 'public_document', dataConfidence: 'verified' },
  ],
};

function verifiedPortsFromItinerary(itinerary: ItineraryDay[]): string[] {
  return itinerary.filter((day) => !day.isSeaDay).map((day) => day.port);
}
...
export function applyKnownBookingCorrectionsToCruise(cruise: BookedCruise): BookedCruise {
  ...
  const verifiedHarmonyItinerary = shipName === 'harmony of the seas'
    ? VERIFIED_HARMONY_SEPTEMBER_2026_ITINERARIES[sailDate]
    : undefined;

  if (verifiedHarmonyItinerary) {
    return {
      ...cruise,
      departurePort: 'Port Canaveral, Florida',
      destination: sailDate === '2026-09-19' ? 'Eastern Caribbean & Perfect Day' : 'Bahamas & Perfect Day',
      itineraryName: sailDate === '2026-09-19'
        ? '7 Night Eastern Caribbean & Perfect Day'
        : sailDate === '2026-09-15'
          ? '4 Night Bahamas & Perfect Day Cruise'
          : '5 Night Bahamas & Perfect Day Cruise',
      ports: verifiedPortsFromItinerary(verifiedHarmonyItinerary),
      itinerary: verifiedHarmonyItinerary.map((day) => ({ ...day })),
      itineraryNeedsManualEntry: false,
      sourceAuthority: 'public_document',
      dataConfidence: 'verified',
      validationStatus: 'valid',
      sourceProvider: cruise.sourceProvider ?? 'Royal Caribbean published itinerary',
      notes: addCorrectionNote(cruise.notes, `Verified Royal Caribbean itinerary for Harmony of the Seas departing ${sailDate}.`),
    };
  }
...
export function applyUserConfirmedBookedCruiseManifest(cruises: BookedCruise[]): BookedCruise[] {
  ...
  // Manifest merging may insert or replace records after the initial correction
  // pass. Reapply verified voyage facts so every distinct reservation on a
  // known sailing receives the same itinerary without losing booking identity.
  return dedupeBookedCruises(
    merged.map(applyKnownBookingCorrectionsToCruise),
    'booked cruises with user-confirmed current Royal manifest',
  );
}
