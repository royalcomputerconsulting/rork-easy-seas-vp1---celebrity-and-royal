...
function shouldLogParserRow(rowIndex: number): boolean {
  return rowIndex <= 10 || rowIndex % 500 === 0;
}
...
export function parseOffersCSV(content: string): { cruises: Cruise[]; offers: CasinoOffer[] } {
  ...
  const colIndices = {
    ...
    priceOceanView: getColumnIndex(headerMap, ['price ocean view', 'priceoceanview', 'ocean view price', 'oceanviewprice', 'oceanview price', 'oceanview', 'price oceanview', 'ov', 'o/v', 'ocean view']),
    ...
    offerType: getColumnIndex(headerMap, ['offer type / category', 'offer type', 'offertype', 'offer category', 'type']),
    ...
    b2bSequence: getColumnIndex(headerMap, ['b2b', 'back to back', 'back-to-back', 'backtoback', 'b2b sequence', 'b2bsequence']),
  };
  ...
  for (let i = 1; i < lines.length; i++) {
    ...
    const logThisRow = shouldLogParserRow(i);
    if (logThisRow) {
    ...
    const b2bSequence = getValue(colIndices.b2bSequence).trim();
    ...
    const sourceRecordId = [parsedSource, offerInstanceId || offerCode, shipName, sailDate, roomType, guestsInfo, b2bSequence || i].filter(Boolean).join('|');
    ...
    const cruise: Cruise = {
      ...
      b2bSequence: b2bSequence || undefined,
      sourceRowIndex: i,
      guestsInfo,
      status: 'available',
...
function formatDateMMDDYYYY(dateStr: string | undefined | null): string {
  ...
  try {
    ...
    const calendarDate = normalized.match(/^(\d{2})-(\d{2})-(\d{4})$/);
