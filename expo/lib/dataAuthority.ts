...
export function isOperationallyAuthoritative(value: unknown): boolean {
  ...
  // Explicit user-entered itinerary corrections are facts in that owner's
  // dataset, not estimates. They may drive local calculations while remaining
  // visibly distinguished from provider/public-document evidence.
  return authority === 'provider' || authority === 'public_document' || authority === 'verified_local' || authority === 'user_entered';
}
