...
function extractNightsFromText(text: string): number | null {
  ...
  // Royal certificate and offer exports use both "8 Night" and compact
  // forms such as "8 NT". Treat those as equivalent provider evidence.
  const nightsMatch = text.match(/(\d+)\s*(?:[-–—]\s*)?(?:nights?|nts?)\b/i);
  if (nightsMatch) {
...
export function transformOfferRowsToCruisesAndOffers(
  ...
  for (const offer of offerRows) {
    ...
    const providerItinerary = (offer.dayByDayItinerary || [])
      .filter((day) => Number(day.day) > 0)
      .map((day) => ({
        day: Number(day.day),
        port: day.portName || (String(day.type || '').toLowerCase().includes('sea') ? 'At Sea' : 'Port not stated'),
        arrival: day.arrivalTime || undefined,
        departure: day.departureTime || undefined,
        isSeaDay: String(day.type || '').toLowerCase().includes('sea') || /at sea|sea day/i.test(day.portName || ''),
        source: 'provider' as const,
        dataConfidence: 'verified' as const,
      }));
    const providerPorts = providerItinerary.map((day) => day.port).filter(Boolean);
    const portsAndTimes = providerItinerary.length > 0
      ? providerItinerary.map((day) => `Day ${day.day}: ${day.port}${day.arrival ? ` ${day.arrival}` : ''}${day.departure ? `-${day.departure}` : ''}`).join('\n')
      : undefined;
    ...
    const cruise: Cruise = {
      ...
      // A marketing itinerary name is not a day-by-day route. Preserve only
      // provider day rows here so casino/sea-day calculations cannot mistake
      // “Bahamas & Perfect Day Cruise” for one factual port day.
      itineraryRaw: providerPorts.length > 0 ? providerPorts : [],
      ports: providerPorts.length > 0 ? providerPorts : ports,
      portsAndTimes,
  ...
  offerMap.forEach((offer, key) => {
    ...
    // CasinoOffer is the offer-instance summary; it is not a sailing row.
    // Retain a material sailing field only when every linked sailing supplies
    // the same value. This prevents the first of 70/1,000 eligible rows from
    // masquerading as the ship, cabin, guest count, or itinerary for all rows.
    const linkedCruises = cruises.filter((cruise) => cruiseIds.includes(cruise.id));
    const uniform = <T,>(
      read: (cruise: Cruise) => T | null | undefined,
      serialize: (value: T) => string = (value) => JSON.stringify(value),
    ): T | undefined => {
      if (linkedCruises.length === 0) return undefined;
      const values = linkedCruises.map(read);
      if (values.some((value) => value === undefined || value === null || value === '')) return undefined;
      const first = values[0] as T;
      return values.every((value) => serialize(value as T) === serialize(first)) ? first : undefined;
    };

    if (linkedCruises.length > 0) {
      offer.shipName = uniform((cruise) => cruise.shipName, (value) => String(value).trim().toLowerCase());
      offer.sailingDate = uniform((cruise) => cruise.sailDate);
      offer.itineraryName = uniform((cruise) => cruise.itineraryName);
      offer.nights = uniform((cruise) => cruise.nights > 0 ? cruise.nights : undefined);
      offer.itinerary = uniform((cruise) => cruise.itinerary?.length ? cruise.itinerary : undefined);
      offer.ports = uniform((cruise) => cruise.ports?.length ? cruise.ports : undefined);
      offer.portsAndTimes = uniform((cruise) => cruise.portsAndTimes || undefined);
      offer.roomType = uniform((cruise) => cruise.cabinType || undefined, (value) => String(value).trim().toLowerCase());
      offer.guestsInfo = uniform((cruise) => cruise.guestsInfo || undefined, (value) => String(value).trim().toLowerCase());
      offer.guests = uniform((cruise) => cruise.guests && cruise.guests > 0 ? cruise.guests : undefined);
    }
  });
