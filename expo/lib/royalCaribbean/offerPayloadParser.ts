...
export function parseCasinoOffersPayload(
  ...
  rawOffers.forEach((entry) => {
    ...
    const offerStateroom = getString(
      offer.roomType ??
      offer.stateroomType ??
      offer.cabinType ??
      offer.stateroomCategory ??
      offer.stateroomClass ??
      offer.awardType ??
      asRecord(offer.marketingUiAttributes)?.stateroomType ??
      asRecord(offer.marketingUiAttributes)?.cabinType
    );
    ...
    sailings.forEach((sailing) => {
      ...
      const cabinType = getString(
        sailing.roomType ??
        sailing.stateroomType ??
        sailing.cabinType ??
        sailing.stateroomCategory ??
        sailing.stateroomClass ??
        sailing.awardType ??
        asRecord(sailing.category)?.name ??
        asRecord(sailing.stateroomClass)?.name ??
        asRecord(sailing.stateroomClass)?.code
      ) || offerStateroom;
