...
function normalizeImportedUsers(value: unknown): FullAppDataBundle['users'] {
  const users = normalizeImportedArray<FullAppDataBundle['users'][number]>(value);
  const seenIds = new Set<string>();
  let ownerAssigned = false;

  return users.map((user, index) => {
    const requestedId = typeof user.id === 'string' ? user.id.trim() : '';
    const baseId = requestedId || `legacy-user-${index + 1}`;
    let id = baseId;
    let suffix = 2;
    while (seenIds.has(id)) {
      id = `${baseId}-imported-${suffix}`;
      suffix += 1;
    }
    seenIds.add(id);

    const isOwner = user.isOwner === true && !ownerAssigned;
...
export function buildExportAllDataSummary(bundle: FullAppDataBundle): ExportAllDataSummary {
  ...
  // Backups made before the shared certificate-document store was introduced
  // legitimately omit this collection. Keep their summaries/load path safe.
  const certificateDocuments = bundle.certificateDocuments ?? [];
  ...
  return {
    ...
    certificateDocuments: certificateDocuments.length,
    certificateSailingRows: certificateDocuments.reduce((total, document) => total + (document.parseHistory.at(-1)?.result.sailings.length ?? 0), 0),
...
export function formatExportAllDataSummary(summary: ExportAllDataSummary, fileName: string): string {
  return [
    ...
    `• ${formatCount(summary.certificateDocuments, 'retained Club Royale certificate PDF')}`,
    `• ${formatCount(summary.certificateSailingRows, 'parsed certificate sailing row')}`,
    '',
    'Casino tracking:',
...
export function normalizeImportedBackup(rawBundle: LegacyFullDataBundle): FullAppDataBundle {
  ...
  const certificateDocuments = normalizeImportedArray<FullAppDataBundle['certificateDocuments'][number]>(rawBundle.certificateDocuments);
  // Backups from early 2.x builds can contain two profiles with the same id
  // (and sometimes both flagged as the account owner). Repair that legacy
  // shape before it reaches profile storage so Load All remains deterministic.
  const users = normalizeImportedUsers(rawBundle.users);
...
export async function importAllDataFromFile(email?: string | null, profileGate?: DataProfileGate): Promise<{
  ...
  try {
    ...
    if (typeof pickDocument !== 'function') {
      return {
        success: false,
        error: 'The iOS file picker is not available in this build. Please reinstall the current Easy Seas build and try again.',
      };
    }
    ...
    if (
      ...
      bundle.certificateDocuments.length === 0 &&
    ...
    console.log('[DataFileIO] Bundle contains:', {
      ...
      certificateDocuments: bundle.certificateDocuments?.length || 0,
    ...
    if (importResult.errors.length > 0) {
      console.error('[DataFileIO] Import did not complete:', importResult.errors);
      return {
        success: false,
