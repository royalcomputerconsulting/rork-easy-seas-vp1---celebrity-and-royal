...
function isZeroLikeCasinoValue(value: unknown): boolean {
...
function mergeRecordPreferIncoming<T extends Record<string, unknown>>(existing: T, incoming: T): T {
  ...
  Object.entries(incoming).forEach(([key, value]) => {
    ...
    const preservesCasinoFact = CASINO_PERFORMANCE_FIELDS.has(key)
      && isMeaningfulValue(existing[key])
      && isZeroLikeCasinoValue(value);
    const preservesLocalCasinoFactFromProvider = CASINO_PERFORMANCE_FIELDS.has(key)
      && incomingAuthority === 'provider'
      && isMeaningfulValue(existing[key])
      && isMeaningfulValue(value);
    if (isMeaningfulValue(value) && !preservesExisting && !preservesCasinoFact && !preservesLocalCasinoFactFromProvider) {
      merged[key] = value;
    }
  });
  return merged as T;
...
const CASINO_PERFORMANCE_FIELDS = new Set([
  // User-entered / completed-cruise-import point truth. Provider sync can lag,
  // rename fields, or omit this data entirely; once the user closes out a
  // sailing, these values must remain attached to that user's cruise record.
  'pointsEarned', 'earnedPoints', 'casinoPoints', 'clubRoyalePoints', 'blueChipPoints',
  'captainsClubPoints', 'tierPoints', 'pointsSource', 'casinoPointsSource',
  'pointsCalculationConfidence', 'casinoCalculationConfidence',

  // Coin-in, actual cash result, and closeout aliases used across old/new tabs,
  // imports, session tooling, and booked-cruise casino summaries.
  'coinIn', 'totalCoinIn', 'estimatedCoinIn', 'winningsBroughtHome', 'winnings',
  'totalWinnings', 'winLoss', 'casinoWinLoss', 'actualWinLoss', 'cashResult',
  'netResult', 'casinoNetResult', 'netGamingResult', 'gamingResult',
  'amountWon', 'amountLost', 'cashIn', 'cashOut', 'cashInAmount', 'cashOutAmount',
  'buyIn', 'startingCash', 'endingCash', 'casinoStartingCash', 'casinoEndingCash',
  'casinoHandpays', 'handpayAmount', 'handpays', 'jackpotAmount',

  // Derived host/casino analytics. These are recalculated by EasySeas from the
  // user's durable closeout facts and should not be zeroed by a provider row.
  'hoursPlayed', 'estimatedPlayHours', 'sessionsPlayed', 'ratedGamingDays',
  'casinoOpenHours', 'actualSessionCount', 'estimatedSessionCount', 'adt',
  'averageDailyTheo', 'theoretical', 'theoreticalWin', 'theoreticalLoss',
  'actualLoss', 'dailyTheo', 'dailyActual',

  // Certificate/reward evidence that ties an earned certificate back to the
  // sailing where it was earned.
  'certificateEarnedCode', 'certificateCode', 'instantCertificateOfferCode',
  'instantCertificateValue', 'instantCertificateWon', 'certificatePointFloor',
  'certificateEvidenceCode', 'certificateIssuedDate', 'certificateExpirationDate',
  'certificateAwardType', 'tradeInValue',

  // Import/closeout provenance.
  'casinoHistoryImportId', 'casinoHistorySource', 'completedCruiseImportId',
  'completedCruiseImportedAt', 'historicalCasinoImportId', 'historicalCasinoImportedAt',
  'sourceImportFile', 'calculationConfidence', 'postCruiseCloseoutNotes',
  'postCruiseCloseoutAt', 'closeoutStatus', 'casinoCloseoutSource', 'closeoutSource',
]);
