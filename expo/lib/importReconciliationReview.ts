...
export function buildOffersImportReviewRows(input: OffersReviewInput): SmartImportReviewRow[] {
  ...
  const maxRows = Math.max(25, input.maxRows ?? 250);
  const hasRoom = () => rows.length < maxRows;

  for (let index = 0; index < input.importedCruises.length && hasRoom(); index += 1) {
    const cruise = input.importedCruises[index];
  ...
  for (let index = 0; index < input.importedOffers.length && hasRoom(); index += 1) {
    const offer = input.importedOffers[index];
  ...
  const reviewRows = [...input.mergedCruises, ...input.mergedOffers].filter(hasReviewStatus);
  reviewRows.slice(0, Math.max(0, maxRows - rows.length)).forEach((record, index) => {
  ...
  const totalSourceRows = input.importedCruises.length + input.importedOffers.length + reviewRows.length;
  if (totalSourceRows > rows.length) {
    rows.push({
      id: `preview-limit:${totalSourceRows}:${rows.length}`,
      kind: 'Cruise',
      action: 'preserve',
      title: 'Preview limited for performance',
      subtitle: `${formatCount(rows.length, 'representative row')} shown from ${formatCount(totalSourceRows, 'import/review row')}.`,
      meta: 'The full import will still be applied; only this review preview is capped to keep the app responsive.',
      after: 'Apply uses the complete merged dataset.',
      changedFields: [],
      fieldDiffs: [],
    });
  }
