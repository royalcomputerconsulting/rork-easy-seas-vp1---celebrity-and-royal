...
function isInactiveNonCompletedStatus(status: string): boolean {
  return ['archived', 'replaced', 'skipped', 'review needed', 'reviewneeded', 'cancelled', 'canceled'].includes(status);
}

export function isCancelledBookedCruise(cruise: BookedCruise): boolean {
  const normalizedStatus = String(cruise.status ?? '').trim().toLowerCase();
...
export function isCompletedBookedCruise(cruise: BookedCruise, today: Date = startOfToday()): boolean {
  ...
  if (cruise.completionState === 'completed' || normalizedStatus === 'completed' || normalizedStatus === 'past') {
    return true;
  }
