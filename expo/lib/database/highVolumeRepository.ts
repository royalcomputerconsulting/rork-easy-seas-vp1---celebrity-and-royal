...
function toRows(
  ...
  const rowsByKey = new Map<string, DomainRecordRow>();
  records.forEach((record, index) => {
    const recordJson = JSON.stringify(record);
    const baseRecordId = highVolumeRecordId(record, index);
    let recordId = baseRecordId;
    const existing = rowsByKey.get(recordId);

    // Older Easy Seas backups can contain two legitimate calendar rows with
    // the same legacy id (for example a cruise span and an itinerary event).
    // SQLite correctly rejects that collision at statement finalization. Keep
    // exact duplicates collapsed, but retain distinct records under a stable
    // content-qualified id so a whole restore cannot fail at finalizeAsync.
    if (existing && existing.recordJson !== recordJson) {
      const contentSuffix = hashText(recordJson);
      recordId = `${baseRecordId}::${contentSuffix}`;
      let collision = 1;
      while (rowsByKey.has(recordId) && rowsByKey.get(recordId)?.recordJson !== recordJson) {
        recordId = `${baseRecordId}::${contentSuffix}-${collision}`;
        collision += 1;
      }
    }

    rowsByKey.set(recordId, {
