...
function hashDomainRecord(value: string): string {
  let hash = 2166136261;
...
export async function replaceDomainRecords(ownerId: string, domain: string, rows: DomainRecordRow[]): Promise<void> {
  const collisionSafeRows = new Map<string, DomainRecordRow>();
  for (const incoming of rows) {
    const row = { ...incoming, ownerId, domain };
    const baseRecordId = String(row.recordId);
    let recordId = baseRecordId;
    const existing = collisionSafeRows.get(recordId);
    if (existing && existing.recordJson !== row.recordJson) {
      const contentSuffix = hashDomainRecord(row.recordJson);
      recordId = `${baseRecordId}::${contentSuffix}`;
      let collision = 1;
      while (collisionSafeRows.has(recordId) && collisionSafeRows.get(recordId)?.recordJson !== row.recordJson) {
        recordId = `${baseRecordId}::${contentSuffix}-${collision}`;
        collision += 1;
      }
    }
    collisionSafeRows.set(recordId, { ...row, recordId });
  }

  await withHealthTrustWriteTransaction(async (transaction) => {
    await transaction.runAsync('DELETE FROM domain_records WHERE owner_id=? AND domain=?', [ownerId, domain]);
    for (const row of collisionSafeRows.values()) {
