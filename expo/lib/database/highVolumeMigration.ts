...
import { getHealthTrustDatabase, upsertDomainRecords, withHealthTrustWriteTransaction } from './HealthTrustDatabase';
...
async function migrateSource(ownerId: string, definition: HighVolumeDomain, sourceKey: string, onProgress?: (processed: number, total: number) => void): Promise<{ migrated: number; skipped: boolean }> {
  ...
  let parsed: unknown; try { parsed = JSON.parse(raw); } catch (error) { await withHealthTrustWriteTransaction((transaction) => transaction.runAsync('INSERT OR REPLACE INTO migration_checkpoints(id,owner_id,domain,source_key,state,last_index,total_rows,source_hash,started_at,updated_at,error) VALUES(?,?,?,?,?,?,?,?,?,?,?)', [checkpointId, ownerId, definition.domain, sourceKey, 'failed', 0, 0, sourceHash, new Date().toISOString(), new Date().toISOString(), error instanceof Error ? error.message : String(error)])); throw error; }
  const rows = records(parsed);
  const recordIds = new Map<string, string>();
  const migrationRecordIds = rows.map((record, index) => {
    const recordJson = JSON.stringify(record);
    const baseId = highVolumeRecordId(record, index);
    const existingJson = recordIds.get(baseId);
    if (existingJson === undefined || existingJson === recordJson) { recordIds.set(baseId, recordJson); return baseId; }
    const qualifiedId = `${baseId}::${hashText(recordJson)}`;
    recordIds.set(qualifiedId, recordJson);
    return qualifiedId;
  });
  ...
  if (!existing || restartingRollback) {
    await withHealthTrustWriteTransaction(async (transaction) => {
      await transaction.runAsync('DELETE FROM domain_records WHERE owner_id=? AND domain IN (?,?)', [effectiveOwner, stagingDomain, rollbackDomain]);
      await transaction.runAsync(`INSERT OR REPLACE INTO domain_records(owner_id,domain,record_id,record_json,source_storage_key,source_hash,updated_at)
  ...
  await withHealthTrustWriteTransaction((transaction) => transaction.runAsync('INSERT OR REPLACE INTO migration_checkpoints(id,owner_id,domain,source_key,state,last_index,total_rows,source_hash,started_at,updated_at,error) VALUES(?,?,?,?,?,?,?,?,?,?,NULL)', [checkpointId, ownerId, definition.domain, sourceKey, 'running', start, rows.length, sourceHash, timestamp, timestamp]));
  for (let offset = start; offset < rows.length; offset += 250) {
    ...
    const batch = rows.slice(offset, offset + 250).map((record, index) => ({ ownerId: effectiveOwner, domain: stagingDomain, recordId: migrationRecordIds[offset + index], recordJson: JSON.stringify(record), sourceStorageKey: sourceKey, sourceHash, updatedAt: new Date().toISOString() }));
    await upsertDomainRecords(batch); const processed = Math.min(rows.length, offset + batch.length);
    await withHealthTrustWriteTransaction((transaction) => transaction.runAsync('UPDATE migration_checkpoints SET last_index=?,updated_at=? WHERE id=?', [processed, new Date().toISOString(), checkpointId])); onProgress?.(processed, rows.length); await yieldToUi();
  }
  await withHealthTrustWriteTransaction(async (transaction) => {
    await transaction.runAsync('DELETE FROM domain_records WHERE owner_id=? AND domain=? AND source_storage_key=?', [effectiveOwner, definition.domain, sourceKey]);
    await transaction.runAsync(`INSERT OR REPLACE INTO domain_records(owner_id,domain,record_id,record_json,source_storage_key,source_hash,updated_at)
      SELECT owner_id,?,record_id,record_json,source_storage_key,source_hash,updated_at FROM domain_records
      WHERE owner_id=? AND domain=?`, [definition.domain, effectiveOwner, stagingDomain]);
    await transaction.runAsync('DELETE FROM domain_records WHERE owner_id=? AND domain=?', [effectiveOwner, stagingDomain]);
    await transaction.runAsync('UPDATE migration_checkpoints SET state=?,last_index=?,updated_at=? WHERE id=?', ['complete', rows.length, new Date().toISOString(), checkpointId]);
  });
...
export async function rollbackHighVolumeMigration(ownerId: string, definition: HighVolumeDomain, sourceKey: string, reason = 'User requested rollback from migration diagnostics'): Promise<MigrationRollbackResult> {
  ...
  await withHealthTrustWriteTransaction(async (transaction) => {
    await transaction.runAsync('DELETE FROM domain_records WHERE owner_id=? AND domain=? AND source_storage_key=? AND source_hash=?', [effectiveOwner, definition.domain, sourceKey, sourceHash]);
    await transaction.runAsync(`INSERT OR REPLACE INTO domain_records(owner_id,domain,record_id,record_json,source_storage_key,source_hash,updated_at)
      SELECT owner_id,?,record_id,record_json,source_storage_key,source_hash,updated_at FROM domain_records
      WHERE owner_id=? AND domain=?`, [definition.domain, effectiveOwner, rollbackDomain]);
    await transaction.runAsync('DELETE FROM domain_records WHERE owner_id=? AND domain IN (?,?)', [effectiveOwner, stagingDomain, rollbackDomain]);
    await transaction.runAsync("UPDATE migration_checkpoints SET state='rolled_back',last_index=0,updated_at=?,error=? WHERE id=?", [new Date().toISOString(), reason, checkpoint.id]);
    await transaction.runAsync('INSERT INTO migration_rollback_history(id,owner_id,domain,source_key,source_hash,removed_rows,rolled_back_at,reason) VALUES(?,?,?,?,?,?,?,?)', [auditId, normalizedOwner, definition.domain, sourceKey, sourceHash || null, liveCount, new Date().toISOString(), reason]);
  });
