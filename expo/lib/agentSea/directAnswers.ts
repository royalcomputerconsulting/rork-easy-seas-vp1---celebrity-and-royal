...
function buildLastCruisePointsAnswer(input: {
  ...
  const text = truth.points.value == null
    ...
    : `You earned ${truth.points.value.toLocaleString()} casino points on your latest matching cruise, ${sailing}. ${truth.points.kind === 'estimated' ? 'That figure is an estimate, not a provider-recorded total.' : 'That is the saved cruise-level points value.'}`;
  return {
    intent: 'last_cruise_points',
    text,
