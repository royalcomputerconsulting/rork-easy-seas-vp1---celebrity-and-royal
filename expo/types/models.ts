...
export interface Cruise extends SharedOwnershipFields {
  ...
  nextCruiseBonus?: string | number | boolean;
  nextCruiseBonusValue?: string | number;
  bookingBonus?: string | number;
  gty?: string | boolean;
  perks?: string[];
  percentOff?: number;
  ...
  /** B2B grouping/position from provider exports. This is part of offer-option identity. */
  b2bSequence?: string | number;
  /** Source CSV/export row index used only to keep exact provider rows auditable. */
...
export interface BookedCruise extends Cruise {
  ...
  certificateAwardType?: string;
  certificateExpirationDate?: string;
  certificateIssueDate?: string;
  certificateSailingDate?: string;
  coinIn?: number;
  coinInCalculationSource?: 'actual' | 'provider' | 'club_royale_slot_points_estimate';
  /** User confirmation that the recorded Club Royale points were earned from
   * eligible slot play, which is required before applying the $5/point model. */
  slotPointsConfirmed?: boolean;
  /** Explicit provider- or user-recorded rated gaming days. Cruise nights are
   * never substituted for this value when calculating ADT. */
