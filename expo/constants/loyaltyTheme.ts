...
export function createLoyaltyCardTheme(accentColor?: string | null): LoyaltyCardTheme {
  ...
  // Loyalty color is semantic evidence, not a full-card paint bucket.  Keeping
  // the main surface neutral makes Crown & Anchor, Club Royale, Blue Chip and
  // the other programs read as one Easy Seas product while the tier color
  // remains visible in badges and progress bars.
  const progressEnd = mixHexColors(resolvedAccent, '#FFFFFF', 0.22);

  return {
    accentColor: resolvedAccent,
    gradientColors: ['#FFFFFF', '#F8FAFB', '#F3F3F2'],
    borderColor: '#D9E1E6',
    surfaceColor: '#FFFFFF',
    surfaceColorMuted: '#F3F3F2',
    topTextColor: '#17324D',
    secondaryTextColor: '#66737F',
    progressBarGradient: [resolvedAccent, progressEnd],
  };
}
