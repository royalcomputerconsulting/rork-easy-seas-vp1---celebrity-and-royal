...
import { SPACING, BORDER_RADIUS, TYPOGRAPHY, SHADOW, COLORS, EASY_SEAS_PHOTO_POLICY } from '@/constants/theme';
...
import { MetricGrid } from '@/components/ui/EasySeasPrimitives';

const OFFER_IMAGE_FALLBACK = require('../assets/images/section-themes/offers-certificates-v1.png');
  ...
  return (
    <View style={summaryStyles.container} testID="offers-overview-card">
      <View style={summaryStyles.header}>
        <View>
          <Text style={summaryStyles.title}>Your offer portfolio</Text>
          <Text style={summaryStyles.subtitle}>Current value and eligible inventory</Text>
        </View>
      </View>

      <MetricGrid
        columns={3}
        testID="offers-summary-metric-grid"
        metrics={[
          { label: 'Estimated value', value: isLoading ? '…' : totalValue > 0 ? `$${totalValue.toLocaleString()}` : '—', icon: <DollarSign size={15} color="#0E7FA7" /> },
          { label: 'Active offers', value: isLoading ? '…' : totalOffers.toLocaleString(), icon: <Tag size={15} color="#0E7FA7" /> },
          { label: 'Eligible sailings', value: isLoading ? '…' : totalCruises.toLocaleString(), icon: <Ship size={15} color="#0E7FA7" /> },
        ]}
      />
      ...
      <View style={summaryStyles.buttonRow}>
        ...
        >
          <Clock size={15} color={activeSortMode === 'soonest' ? '#FFFFFF' : '#167C80'} />
        ...
        >
          <DollarSign size={15} color={activeSortMode === 'highestValue' ? '#FFFFFF' : '#167C80'} />
...
export const CasinoOfferCard = React.memo(function CasinoOfferCard({
  ...
  // FlatList recycles card instances while filters and paging change. Keep the
  // visible photograph tied to the current offer instead of retaining the URI
  // (or fallback) from the card that previously occupied this row.
  useEffect(() => {
    setCardImageUri(offerImageUrl);
    setShowCardImage(true);
  }, [offerImageUrl]);

  const handleCardImageError = useCallback(() => {
    if (cardImageUri !== DEFAULT_CRUISE_IMAGE) {
      setCardImageUri(DEFAULT_CRUISE_IMAGE);
      return;
    }
  ...
  const getStatusBadge = () => {
    if (!isActive) {
      return { text: 'Expired', bg: '#FCE8E8', fg: '#B53A3A', border: '#F0B6B6' };
    }
    if (isBestValue) {
      return { text: 'Best value', bg: '#E7F5EC', fg: '#237A4B', border: '#B9DEC7' };
    }
    return { text: 'Active', bg: '#E7F5EC', fg: '#237A4B', border: '#B9DEC7' };
  };
  ...
  const displayedCabinTruth = useMemo(() => {
    ...
    if (values.length > 1) return { ...cabinTruth, value: null, label: values.join(' · '), evidence: 'sailing_rows' as const, source: `${values.length} Royal stateroom categories`, varies: true };
    if ((sailingSummary?.total ?? 0) > 0) return { ...cabinTruth, value: null, label: 'Categories available by sailing', evidence: 'sailing_rows' as const, source: 'Royal eligible sailing rows', varies: true };
    return cabinTruth;
  }, [cabinTruth, sailingSummary?.cabinTypes, sailingSummary?.total]);
  ...
  const indexedValueRange = sailingSummary?.minimumRoomValue != null && sailingSummary?.maximumRoomValue != null
    ...
    : null;
  const hasDisplayedStateroomValue = displayedStateroomValue > 0 || indexedValueRange != null;
  const displayedStateroomValueLabel = indexedValueRange
    ...
    : displayedStateroomValue > 0
      ? `$${Math.round(displayedStateroomValue).toLocaleString()}`
      : 'Not supplied';
  ...
  return (
    ...
    >
      <View style={styles.offerHeaderSection}>
        ...
        <View style={[styles.statusBadgeLarge, { backgroundColor: statusBadge.bg, borderColor: statusBadge.border }]}> 
          <Text style={[styles.statusBadgeLargeText, { color: statusBadge.fg }]}>{statusBadge.text}</Text>
        </View>
      </View>

      <View style={styles.imageSection}>
        <Image
          source={OFFER_IMAGE_FALLBACK}
      ...
      <View style={styles.contentSection}>
        <View style={styles.offerFactsGrid} testID="casino-offer-card.primary-facts">
          <View style={styles.offerFact}>
            ...
            <Text style={styles.offerFactValue} numberOfLines={2}>{displayedPointTruth.label}</Text>
          </View>
          <View style={styles.offerFact}>
          ...
          <View style={styles.offerFact}>
            ...
            <Text style={styles.offerFactValue} numberOfLines={3}>{displayedCabinTruth.label}</Text>
          </View>
          <View style={[styles.offerFact, !hasDisplayedStateroomValue && styles.offerFactMissing]}>
            <Text style={styles.offerFactLabel}>{displayedStateroomValueIsEstimate ? 'Est. stateroom value' : 'Stateroom value'}</Text>
            <Text style={[styles.offerFactValue, !hasDisplayedStateroomValue && styles.offerFactValueMissing]}>{displayedStateroomValueLabel}</Text>
          </View>
        </View>
        ...
        <TouchableOpacity
          ...
          testID="casino-offer-card.details-disclosure"
        >
          <Text style={styles.detailsDisclosureText}>{showDetails ? 'Hide details and sources' : 'Details and sources'}</Text>
          {showDetails ? <ChevronUp size={15} color="#17324D" /> : <ChevronDown size={15} color="#17324D" />}
        </TouchableOpacity>

        {showDetails && uniqueDestinations.length > 0 && (
        ...
        {showDetails ? (
          <View style={styles.provenanceRow} testID="casino-offer-card.provenance">
            ...
            <Text style={styles.provenanceValue} numberOfLines={5}>
              {offerDetails.sourceLabel} · Points: {displayedPointTruth.source} ({displayedPointTruth.evidence}) · Guests: {displayedGuestTruth.source} · Cabin: {displayedCabinTruth.source} ({displayedCabinTruth.evidence}) · Value: {indexedValueRange ? 'All eligible sailing prices' : normalizedOfferValue.components.cabinRetail.sourceField || 'no eligible sailing price'} ({indexedValueRange ? 'all sailing rows' : normalizedOfferValue.components.cabinRetail.evidence})
            </Text>
          </View>
        ) : null}
