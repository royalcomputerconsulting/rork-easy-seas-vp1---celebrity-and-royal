...
import { COLORS, SPACING, BORDER_RADIUS, TYPOGRAPHY, SHADOW, EASY_SEAS_PHOTO_POLICY, EASY_SEAS_TYPE_STYLES } from '@/constants/theme';
...
export const OfferCard = React.memo(function OfferCard({
  ...
  const getStatusBadge = () => {
    if (isBooked) {
      return { text: 'Booked', fg: '#237A4B', bg: '#E7F5EC', border: '#B9DAC7' };
    }
    if (recommended) {
      return { text: 'Recommended', fg: '#A86B00', bg: '#FFF4D6', border: '#E6B63D' };
    }

    return { text: 'Casino offer', fg: '#2C64A0', bg: '#E8F1FA', border: '#B7CCE2' };
  };
    ...
    >
      ...
      {showImage && (
        <View style={styles.imageSection}>
          ...
          <View style={[styles.saleBadge, { backgroundColor: statusBadge.bg, borderColor: statusBadge.border }]}>
            <Text style={[styles.saleBadgeText, { color: statusBadge.fg }]}>{statusBadge.text}</Text>
          </View>

          {isExpiringSoon && (
            <View style={styles.expiresUrgentBadge}>
              <Clock size={12} color="#A52B34" />
...
const styles = StyleSheet.create({
  ...
  offerNameHeader: {
    ...EASY_SEAS_TYPE_STYLES.cardTitle,
    color: '#17324D',
    marginBottom: 6,
  },
  offerCodeRow: {
