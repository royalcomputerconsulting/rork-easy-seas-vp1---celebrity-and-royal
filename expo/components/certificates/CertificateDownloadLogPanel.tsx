    ...
    <View style={styles.container} testID="certificate-download-log.panel">
      ...
      {snapshot.total > 0 ? (
        <View style={styles.progressWrap} testID="certificate-download-log.progress">
          ...
          <AccessibleProgress progress={snapshot.completed / snapshot.total} label={`${snapshot.completed} of ${snapshot.total} certificates processed`} />
          {!snapshot.isActive && snapshot.completed === snapshot.total ? <PurposefulSuccess label="Certificate download pass complete" /> : null}
...
const styles = StyleSheet.create({
  ...
  progressWrap: { backgroundColor: '#F5F5F4', borderRadius: 12, borderWidth: 1, borderColor: '#D7E2EF', padding: 10, gap: 6 },
  ...
  entriesWrap: { backgroundColor: '#F5F5F4', borderRadius: 12, borderWidth: 1, borderColor: '#EAECF0', padding: 10, gap: 8 },
