...
import { X, Bell, Trash2, Check, AlertTriangle, Clock, Tag, Ship, CheckCircle, Circle, Store, TrendingUp, Zap, CalendarX, Calendar, TrendingDown, ChevronRight, ExternalLink, } from 'lucide-react-native';
...
export function AlertsManagerModal({ visible, onClose }: AlertsManagerModalProps) {
    ...
    const { alerts, activeAlerts, dismissAlert, snoozeAlert, resolveAlert, clearAllAlerts, summary, } = useAlerts();
    ...
    const dismissedCount = useMemo(() => alerts.filter(a => a.status === 'dismissed').length, [alerts]);
    ...
    const renderAlertItem = (alert: Alert) => {
        ...
        return (<View key={alert.id} style={[
                styles.alertItem,
                (isDismissed || isResolved) && styles.alertItemRead,
                isSnoozed && styles.alertItemSnoozed,
                isExpanded && styles.alertItemExpanded,
            ]}>
        <TouchableOpacity style={styles.alertTouchable} onPress={() => handleToggleExpand(alert.id)} activeOpacity={0.8}>
          <View style={styles.alertTopRow}>
            <TouchableOpacity style={styles.readToggle} onPress={() => handleMarkAsRead(alert.id)} activeOpacity={0.7}>
              {isResolved ? (<CheckCircle size={20} color={COLORS.success}/>) : (<Circle size={20} color={COLORS.beigeWarm}/>)}
            </TouchableOpacity>
            ...
            <ChevronRight size={16} color={COLORS.beigeWarm} style={{ transform: [{ rotate: isExpanded ? '90deg' : '0deg' }] }}/>
          </View>
        </TouchableOpacity>
    ...
    return (<Modal visible={visible} animationType="slide" presentationStyle="pageSheet" onRequestClose={onClose}>
      <View style={styles.container}>
        ...
        <View style={styles.filterRow}>
          {(['all', 'active', 'dismissed'] as const).map(f => (<TouchableOpacity key={f} style={[styles.filterChip, filter === f && styles.filterChipActive]} onPress={() => setFilter(f)} activeOpacity={0.7}>
        ...
        <View style={styles.bulkActions}>
          <TouchableOpacity style={styles.bulkActionBtn} onPress={handleMarkAllAsRead} activeOpacity={0.7}>
            <Check size={16} color={COLORS.beigeWarm}/>
            <Text style={styles.bulkActionText}>Mark All Read</Text>
          </TouchableOpacity>
          <TouchableOpacity style={styles.bulkActionBtn} onPress={handleClearAll} activeOpacity={0.7}>
        ...
        <ScrollView style={styles.scrollView} contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
          {filteredAlerts.length === 0 ? (<View style={styles.emptyState}>
            ...
            </View>) : (<View style={styles.alertsList}>
