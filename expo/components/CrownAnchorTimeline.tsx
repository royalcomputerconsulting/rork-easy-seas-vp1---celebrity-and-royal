...
function getLevelAtPoints(pts: number): {
    name: string;
    points: number;
    color: string;
    bgColor: string;
    ...
    for (let i = LEVEL_THRESHOLDS.length - 1; i >= 0; i--) {
        if (pts >= LEVEL_THRESHOLDS[i].points)
...
export function CrownAnchorTimeline({ currentPoints, bookedCruises }: CrownAnchorTimelineProps) {
    const timeline = useMemo((): TimelineEntry[] => {
        ...
        const rciCruises = bookedCruises
            .filter(c => {
            if (!isRoyalCaribbeanShip(c.shipName))
                return false;
        ...
        rciCruises.forEach((cruise, idx) => {
            ...
            const crossesPinnacle = pinnacleThresholdCrossedIdx === -1 &&
                running < 700 &&
                newTotal >= 700;
            if (crossesPinnacle) {
    ...
    return (<View style={styles.container}>
      {/* Header Summary */}
      <LinearGradient colors={['#0F2439', '#1E3A5F']} style={styles.header} start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }}>
      ...
      {timeline.map((entry, idx) => {
            ...
            return (<View key={entry.cruise.id} style={styles.entryRow}>
            ...
            <View style={styles.entryLeft}>
              ...
              <View style={[
                ...
                {entry.crossesPinnacle ? (<Crown size={10} color={COLORS.goldAccent}/>) : entry.levelUp ? (<TrendingUp size={10} color={entry.levelAfter.color}/>) : entry.isCompleted ? (<CheckCircle size={10} color={COLORS.success}/>) : (<Clock size={10} color={COLORS.aquaAccent}/>)}
              </View>
            </View>
            ...
            <View style={[
              ...
              <View style={styles.cardBottom}>
                <View style={styles.cardNightsRow}>
                  ...
                  {entry.cruise.itineraryName ? (<Text style={styles.cardItinerary} numberOfLines={1}>{entry.cruise.itineraryName}</Text>) : null}
                </View>
