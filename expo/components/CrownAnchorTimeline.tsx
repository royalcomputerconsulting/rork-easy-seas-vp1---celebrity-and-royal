...
const LEVEL_THRESHOLDS: { name: string; points: number; color: string; bgColor: string }[] = [
  ...
  { name: 'Emerald',      points: 55,  color: '#4EC0A5', bgColor: 'rgba(16,185,129,0.12)' },
  { name: 'Diamond',      points: 80,  color: '#3E84D9', bgColor: 'rgba(0,188,212,0.12)' },
  { name: 'Diamond Plus', points: 175, color: '#7B68EE', bgColor: 'rgba(123,104,238,0.12)' },
  { name: 'Pinnacle',     points: 700, color: '#A46000', bgColor: 'rgba(184,134,11,0.12)' },
];
...
function getCrownAnchorPointsForCruise(cruise: BookedCruise): number {
  return projectCrownAnchorCruisePoints(cruise).points ?? 0;
}

function getMultiplierLabel(cruise: BookedCruise): string {
  const multiplier = projectCrownAnchorCruisePoints(cruise).multiplier;
