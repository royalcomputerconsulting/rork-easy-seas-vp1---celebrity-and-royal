...
import { Calendar, ChevronDown, ChevronRight, Users, Ship, Heart, Sparkles, Anchor, Ticket, Gauge, AlertTriangle, Clock3, Star } from 'lucide-react-native';
import { LinearGradient } from 'expo-linear-gradient';
import { COLORS, SPACING, BORDER_RADIUS, TYPOGRAPHY, SHADOW, EASY_SEAS_PHOTO_POLICY } from '@/constants/theme';
...
const CRUISE_IMAGE_FALLBACK = require('../assets/images/section-themes/cruises-discovery-v1.png');
...
export const CruiseCard = React.memo(function CruiseCard({ 
  ...
  const shipDisplayName = useMemo(() => {
    const rawName = String(cruise.shipName || '').trim();
    return /^[A-Z]{1,2}\s+of the Seas$/i.test(rawName) ? 'Ship name needs review' : rawName || 'Ship name not stated';
  }, [cruise.shipName]);
  ...
  const [showMiniDetails, setShowMiniDetails] = useState<boolean>(false);
  ...
  const getStatusBadge = () => {
    switch (cruiseStatus) {
      case 'completed':
        return { text: 'Completed', bg: '#EEF1F3', fg: '#66737F', border: '#D9E1E6' };
      case 'active':
        return { text: 'On board', bg: '#E8F1FA', fg: '#2C64A0', border: '#B7CCE2' };
      default:
        return isBooked 
          ? { text: 'Booked', bg: '#E8F1FA', fg: '#2C64A0', border: '#B7CCE2' }
          : { text: 'Available', bg: '#E7F5EC', fg: '#237A4B', border: '#B9DAC7' };
    }
  };

  const getItineraryName = () => {
    const rawName = String(cruise.itineraryName || cruise.destination || '').split(':').pop()?.trim() || '';
    const withoutRepeatedDuration = rawName
      .replace(/^\s*\d+\s*(?:-|–)?\s*(?:nt|night|nights)\b[\s:–-]*/i, '')
      .trim();
    const meaningfulName = withoutRepeatedDuration && !/^\d+$/.test(withoutRepeatedDuration)
      ? withoutRepeatedDuration
      : String(cruise.destination || 'Cruise').trim();
    return `${cruise.nights || 0}-night ${meaningfulName || 'Cruise'}`;
  };

  const brandLabel = useMemo(() => {
    const source = String(cruise.brand || cruise.cruiseSource || '').toLowerCase();
    if (source.includes('celebrity')) return 'Celebrity Cruises';
    if (source.includes('carnival')) return 'Carnival Cruise Line';
    if (source.includes('silversea')) return 'Silversea';
    if (source.includes('royal')) return 'Royal Caribbean';
    return 'Cruise voyage';
  }, [cruise.brand, cruise.cruiseSource]);
  ...
  if (mini) {
    ...
    return (
      ...
      <TouchableOpacity 
        ...
        accessibilityLabel={`Open ${shipDisplayName} cruise details`}
      ...
      >
        ...
        <View style={styles.miniContent}>
          <View style={styles.miniTopRow}>
            <Text style={styles.miniBrandLabel}>{brandLabel}</Text>
            <View style={styles.miniBadgeStack}>
              <View style={[styles.miniStatusBadge, { backgroundColor: statusBadge.bg, borderColor: statusBadge.border }]}>
                <Text style={[styles.miniStatusBadgeText, { color: statusBadge.fg }]}>{statusBadge.text}</Text>
              </View>
              {futureTopTierBadge ? <View style={[styles.miniTopTierBadge, { backgroundColor: futureTopTierBadge.bg }]} testID="cruise-card-top-tier-badge"><Text style={[styles.miniTopTierBadgeText, { color: futureTopTierBadge.textColor }]}>{futureTopTierBadge.text}</Text></View> : null}
            </View>
          </View>
          <View style={styles.miniShipRow}>
            <Ship size={13} color={COLORS.navyDeep} />
            <Text style={styles.miniShipName} numberOfLines={2}>{shipDisplayName}</Text>
          </View>
          ...
          <View style={styles.miniBottomRow}>
            <View style={styles.miniMeta}>
              ...
              <Text style={styles.miniDate}>{formatDateRange(cruise.sailDate, cruise.returnDate, cruise.nights)}</Text>
            </View>
            <Text style={styles.miniNights}>{cruise.nights} night{cruise.nights === 1 ? '' : 's'}</Text>
          </View>
          <Text style={styles.miniDestination} numberOfLines={2}>
          ...
          {showMiniDetails ? <View style={styles.miniPlanningRow} testID="cruise-card-mini-sea-day-score">
          ...
          {showMiniDetails && operationalMetrics ? (
          ...
          <View style={styles.miniValueRow}>
            <View style={styles.miniMeta}>
              ...
              <Text style={styles.miniDate}>{guestCount ? `${guestCount} guest${guestCount === 1 ? '' : 's'}` : 'Guests not stated'}</Text>
            </View>
            ...
            {catalogFacts.isGty ? <Text style={styles.miniCatalogFact}>Guarantee cabin</Text> : null}
          ...
          {showMiniDetails && (financialFacts.actualPaid != null || financialFacts.advertisedPrice != null || financialFacts.suppliedValue != null) ? (
          ...
          {showMiniDetails && (relatedOfferOptionCount > 1 || relatedReservationCount > 1 || Boolean(cruise.offerCode || catalogFacts.certificateCode)) ? (
          ...
          {showMiniDetails && (isBooked || !!(bookedCruise.offerCode || cruise.offerCode)) && (
          ...
          {showMiniDetails && !!((cruise.interiorPrice && cruise.interiorPrice > 0) || (cruise.oceanviewPrice && cruise.oceanviewPrice > 0) || (cruise.balconyPrice && cruise.balconyPrice > 0) || (cruise.suitePrice && cruise.suitePrice > 0)) && (
            <View style={styles.miniPricingRow}>
              {cruise.interiorPrice != null && cruise.interiorPrice > 0 && (
                <View style={styles.miniPricingItem}>
                  <Text style={styles.miniPricingLabel}>Interior</Text>
              ...
              {cruise.oceanviewPrice != null && cruise.oceanviewPrice > 0 && (
                <View style={styles.miniPricingItem}>
                  <Text style={styles.miniPricingLabel}>Ocean view</Text>
              ...
              {cruise.balconyPrice != null && cruise.balconyPrice > 0 && (
                <View style={styles.miniPricingItem}>
                  <Text style={styles.miniPricingLabel}>Balcony</Text>
          ...
          {showMiniDetails && provenanceLabel ? <Text style={styles.miniProvenance} testID="cruise-card-mini-provenance">Source: {provenanceLabel}</Text> : null}
          {showMiniDetails && ((bookedCruise.taxes ?? cruise.taxes ?? 0) > 0) && (
          ...
          {showMiniDetails && isBooked && !!(bookedCruise.musterStation || bookedCruise.bookingStatus || bookedCruise.packageCode || bookedCruise.stateroomNumber || bookedCruise.stateroomCategoryCode) && (
          ...
          <TouchableOpacity
            style={styles.miniDetailsToggle}
            onPress={(event) => {
              event.stopPropagation();
              setShowMiniDetails((current) => !current);
            }}
            accessibilityRole="button"
            accessibilityLabel={`${showMiniDetails ? 'Hide' : 'Show'} voyage details for ${shipDisplayName}`}
            accessibilityState={{ expanded: showMiniDetails }}
            testID="cruise-card-mini-details-toggle"
          >
            <Text style={styles.miniDetailsToggleText}>{showMiniDetails ? 'Hide voyage details' : 'Voyage details'}</Text>
            <ChevronDown size={15} color={COLORS.navyDeep} style={{ transform: [{ rotate: showMiniDetails ? '180deg' : '0deg' }] }} />
          </TouchableOpacity>
        </View>
    ...
    return (
      ...
      >
        ...
        {showCompactImage ? (
          <Image 
            source={{ uri: compactImageUri }} 
            style={styles.compactRemoteImage}
            resizeMode="cover"
            onError={handleCompactImageError}
            accessibilityIgnoresInvertColors
          />
        ...
        <View style={styles.compactContent}>
          <Text style={styles.compactShipName}>{shipDisplayName}</Text>
          <Text style={styles.compactDestination} numberOfLines={2}>{cruise.destination}</Text>
      ...
      <View style={styles.contentSection}>
        <View style={styles.headerRow}>
          <View style={styles.shipInfo}>
            ...
            <Text style={styles.shipName}>{shipDisplayName}</Text>
            <View style={styles.inlineBadgeStack}>
              <View style={[styles.inlineStatusBadge, { backgroundColor: statusBadge.bg, borderColor: statusBadge.border }]}>
                <Text style={[styles.inlineStatusBadgeText, { color: statusBadge.fg }]}>
                  {statusBadge.text}
                </Text>
              </View>
              {futureTopTierBadge ? (
          ...
          {onToggleFavorite ? (
            <View style={styles.actionIcons}>
              <TouchableOpacity
                ...
                accessibilityRole="button"
                accessibilityLabel={isFavorite ? `Remove ${shipDisplayName} from favorites` : `Add ${shipDisplayName} to favorites`}
        ...
        {bookedCruise.itinerary && bookedCruise.itinerary.length > 0 ? (
          <View style={styles.visitingSection}>
            ...
            <TouchableOpacity
              onPress={onPress}
              accessibilityRole="button"
              accessibilityLabel={`View ports and itinerary for ${shipDisplayName}`}
            >
        ...
        <View style={styles.compactActionRow}>
          <TouchableOpacity style={styles.compactPrimaryButton} onPress={onPress} accessibilityRole="button" accessibilityLabel={`View details for ${shipDisplayName}`}>
            <Text style={styles.compactPrimaryButtonText}>Details</Text>
          </TouchableOpacity>
          <TouchableOpacity style={styles.compactSecondaryButton} onPress={onPress} accessibilityRole="button" accessibilityLabel={`View itinerary for ${shipDisplayName}`}>
...
const styles = StyleSheet.create({
  ...
  miniBackgroundImage: {
    width: '100%',
    aspectRatio: EASY_SEAS_PHOTO_POLICY.cardAspectRatio,
    opacity: 1,
  },
  miniRemoteImage: {
    position: 'absolute',
  ...
  imageSection: {
    aspectRatio: EASY_SEAS_PHOTO_POLICY.heroAspectRatio,
    maxHeight: 232,
    position: 'relative',
  },
  heroImage: {
