...
export function AgentXAnalysisCard({ onViewDetails, onRefresh }: AgentXAnalysisCardProps) {
    ...
    const analysis = useMemo(() => {
        ...
        const totalNights = allCompletedCruises.reduce((sum: number, c: BookedCruise) => sum + (c.nights || 0), 0);
        const earnedPointsFromCruises = allCompletedCruises.reduce((sum: number, c: BookedCruise) => sum + (c.earnedPoints || c.casinoPoints || 0), 0);
        const totalPoints = clubRoyalePoints > 0 ? clubRoyalePoints : earnedPointsFromCruises;
        const totalWinnings = allCompletedCruises.reduce((sum: number, c: BookedCruise) => sum + (c.winnings || 0), 0);
    ...
    if (analysis.cruiseCount === 0 && analysis.totalBookedCount === 0) {
        ...
        <ImageBackground source={{ uri: OCEAN_BG }} style={styles.backgroundImage} resizeMode="cover">
          <LinearGradient colors={['rgba(30, 58, 95, 0.85)', 'rgba(123, 45, 142, 0.85)']} start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }} style={styles.gradientOverlay}>
            ...
            <TouchableOpacity style={styles.refreshButton} onPress={handleRefresh} activeOpacity={0.7} disabled={isRefreshing}>
              {isRefreshing ? (<ActivityIndicator size="small" color="#FFFFFF"/>) : (<RefreshCw size={14} color="#FFFFFF"/>)}
    ...
    return (<View style={styles.container}>
      <ImageBackground source={{ uri: OCEAN_BG }} style={styles.backgroundImage} resizeMode="cover">
        <LinearGradient colors={['rgba(30, 58, 95, 0.88)', 'rgba(123, 45, 142, 0.88)']} start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }} style={styles.gradientOverlay}>
          <View style={styles.header}>
            <View style={styles.headerLeft}>
              <View style={styles.iconContainer}>
                <Bot size={18} color="#FFFFFF"/>
