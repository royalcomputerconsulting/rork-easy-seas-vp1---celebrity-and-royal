...
import { Settings, Bell, Ship, Anchor, Tag, CheckCircle2, LogOut, Target, Users, X, ChevronRight, TrendingUp, Star, Moon } from 'lucide-react-native';
...
export const CompactDashboardHeader = React.memo(function CompactDashboardHeader({
  ...
  const cruiseCountText = dataLoading ? '…' : availableCruises.toLocaleString();
  const sailingOptionCountText = dataLoading ? '…' : (availableCruiseOptions ?? availableCruises).toLocaleString();
  const bookedCountText = dataLoading ? '…' : bookedCruises.toLocaleString();
  const offerCountText = dataLoading ? '…' : activeOffers.toLocaleString();
  const {
    clubRoyaleTier,
  ...
  const { colors: experienceColors, isDark, minimumControlSize, preferences, textScale } = useExperience();
  ...
  const playerCardTheme = useMemo<LoyaltyCardTheme>(() => {
    const brandTheme = getPlayerCardTheme({
    brand: activeBrand,
    crownAnchorLevel,
    celebrityLevel,
    silverseaTier,
    ...
    if (!isDark) {
      return {
        ...brandTheme,
        gradientColors: ['#FFFFFF', '#F7F9FA', '#FFFFFF'],
        borderColor: '#D9E1E6',
        surfaceColor: '#FFFFFF',
        surfaceColorMuted: '#F7F9FA',
        topTextColor: '#17212B',
        secondaryTextColor: '#66737F',
      };
    }
    const highContrast = preferences.theme === 'high-contrast';
    return {
      ...brandTheme,
      gradientColors: highContrast
        ? ['#000000', '#000000', '#101010']
        : [experienceColors.surface, experienceColors.surface, experienceColors.background],
      borderColor: experienceColors.border,
      surfaceColor: experienceColors.surface,
      surfaceColorMuted: experienceColors.background,
      topTextColor: experienceColors.text,
      secondaryTextColor: experienceColors.muted,
      progressBarGradient: highContrast
        ? [experienceColors.accent, '#FFFFFF']
        : brandTheme.progressBarGradient,
    };
  }, [activeBrand, carnivalVifpTier, celebrityLevel, crownAnchorLevel, experienceColors, isDark, preferences.theme, silverseaTier]);
  ...
  const loyaltyHeadline = activeBrand === 'royal'
    ? crownAnchorLevel
    : activeBrand === 'celebrity'
      ? celebrityLevel
      : activeBrand === 'silversea'
        ? silverseaTier
        : `VIFP ${carnivalVifpTier}`;
  ...
  return (
    ...
    <LinearGradient
      ...
      style={[styles.container, compact && styles.containerCompact, { borderColor: playerCardTheme.borderColor }]}
    >
      <View style={[styles.topRow, compact && styles.topRowCompact]}>
        <View style={styles.memberInfoInline}>
          <View style={styles.memberTextInfo}>
            {!compact ? <Text style={[styles.memberEyebrow, { color: playerCardTheme.accentColor }]}>Loyalty profile</Text> : null}
            <Text style={[styles.memberGreeting, compact && styles.memberGreetingCompact, { color: playerCardTheme.topTextColor, fontSize: Math.min(compact ? 30 : 36, (compact ? 24 : 28) * textScale), lineHeight: Math.min(compact ? 35 : 41, (compact ? 29 : 33) * textScale) }]}>{compact ? displayName.toUpperCase() : loyaltyHeadline}</Text>
            <TouchableOpacity onPress={rawNumber ? toggleShowNumber : undefined} activeOpacity={rawNumber ? 0.7 : 1}>
              <Text style={[styles.memberSubtitle, { color: playerCardTheme.secondaryTextColor }]}>{compact ? `${displayNumberLabel} ${displayNumber}` : `${displayName} · ${displayNumberLabel} ${displayNumber}`}</Text>
            </TouchableOpacity>
          </View>
        </View>
        ...
        <View style={styles.actionsSection}>
          {onAlertsPress && (
            <TouchableOpacity 
              style={[styles.iconBtn, { backgroundColor: playerCardTheme.surfaceColor, minWidth: minimumControlSize, minHeight: minimumControlSize }]} 
              onPress={onAlertsPress}
              activeOpacity={0.7}
              accessibilityRole="button"
              accessibilityLabel={`Open alerts${alertCount > 0 ? `, ${alertCount} unread` : ''}`}
            >
      ...
      <BrandToggle activeBrand={activeBrand} onToggle={handleBrandToggle} compact={compact} />

      {activeBrand === 'royal' ? (
        <>
      {/* === ROYAL CARIBBEAN === */}
      <View style={[styles.royalBody, compact && styles.royalBodyCompact]}>
      {compact ? (
        <>
          <View pointerEvents="none" style={styles.royalLineworkOuter} />
          <View pointerEvents="none" style={styles.royalLineworkInner} />
          <View pointerEvents="none" style={styles.royalCrest}>
            <Text style={styles.royalCrestStar}>✦</Text>
            <Text style={styles.royalCrestLetter}>P</Text>
            <Text style={styles.royalCrestLaurel}>❧   ❧</Text>
          </View>
        </>
      ) : null}
      <View style={[styles.tierRow, compact && styles.tierRowCompact]}>
      ...
      {clubRoyaleTierIsProtected && clubRoyaleTierValidThrough ? (
        <View style={[styles.retainedStatusRow, compact && styles.retainedStatusRowCompact, { backgroundColor: compact ? 'transparent' : playerCardTheme.surfaceColor, borderColor: compact ? 'transparent' : playerCardTheme.borderColor }]}> 
      ...
      <View style={[styles.progressGrid, compact && styles.progressGridCompact]}>
        ...
        {(() => {
          ...
          const nextTierDescription = compact
            ? isPinnacle
              ? `Confirmed at 700 · ${crownAnchorPoints.toLocaleString()} current points`
              : `${nightsToNext.toLocaleString()} points to ${nextLevel} · 1 per shared night, 2 solo, +1 suite`
            : isPinnacle
              ? `Confirmed at 700 C&A points • Current balance ${crownAnchorPoints.toLocaleString()}`
              : `${nightsToNext.toLocaleString()} C&A points to ${nextLevel} • Standard double-occupancy nights earn 1 point; solo nights earn 2; suite-or-higher nights add 1`;

          return (
            <View style={[styles.progressCard, compact && styles.progressCardCompact, progressCardStyle]}>
              <Text style={[styles.progressKicker, compact && styles.progressKickerCompact, { color: compact ? '#B37A12' : playerCardTheme.accentColor }]}>{compact ? (isPinnacle ? 'PINNACLE' : `${crownAnchorLevel.toUpperCase()} → ${String(nextLevel).toUpperCase()}`) : 'CROWN & ANCHOR'}</Text>
              <View style={styles.progressHeadlineRow}>
                <View style={styles.progressHeadlineCopy}>
                  {!compact ? <Text style={[styles.progressLabel, styles.progressLabelCompact, progressLabelStyle]}>
                    {isPinnacle ? 'Pinnacle' : `${crownAnchorLevel} → ${nextLevel}`}
                  </Text> : null}
                  <Text style={[styles.progressValue, progressLabelStyle]}>
                    {crownAnchorPoints.toLocaleString()}
                    <Text style={[styles.progressDenominator, progressMetaStyle]}> / {nextThreshold.toLocaleString()}</Text>
                  </Text>
                </View>
                {isPinnacle ? (
              ...
              <View style={[styles.progressBarBg, progressBarTrackStyle]}>
                <LinearGradient
                  colors={compact ? ['#A96F0E', '#E6B63D', '#F2CC68'] : playerCardTheme.progressBarGradient}
                  start={{ x: 0, y: 0 }}
                  end={{ x: 1, y: 0 }}
                  style={[styles.progressBarFill, { width: `${Math.min(100, percentComplete)}%` }]}
                />
              </View>
              {compact ? <View style={styles.progressRangeRow}><Text style={styles.progressRangeText}>0</Text><Text style={styles.progressRangeText}>{nextThreshold.toLocaleString()}</Text></View> : null}
              <Text style={[styles.progressEta, compact && styles.progressEtaCompact, progressMetaStyle]}>{nextTierDescription}</Text>
        ...
        {(() => {
          ...
          return (
            <View style={[styles.progressCard, compact && styles.progressCardCompact, progressCardStyle]}>
              <Text style={[styles.progressKicker, compact && styles.progressKickerCompact, { color: compact ? '#0E7FA7' : playerCardTheme.accentColor }]}>{compact ? (isMasters ? 'MASTERS' : `${currentSeasonTier.toUpperCase()} → ${String(nextTier).toUpperCase()}`) : 'CLUB ROYALE · CURRENT SEASON'}</Text>
              <View style={styles.progressHeadlineRow}>
                <View style={styles.progressHeadlineCopy}>
                  {!compact ? <Text style={[styles.progressLabel, styles.progressLabelCompact, progressLabelStyle]}>
                    {isMasters ? 'Masters' : `${currentSeasonTier} → ${nextTier}`}
                  </Text> : null}
                  <Text style={[styles.progressValue, progressLabelStyle]}>
                    {clubRoyaleCurrentYearPoints.toLocaleString()}
                    {!isMasters ? <Text style={[styles.progressDenominator, progressMetaStyle]}> / {nextThreshold.toLocaleString()}</Text> : null}
                  </Text>
                </View>
                {isMasters ? (
                  <View style={styles.achievedBadge}>
              ...
              <View style={[styles.progressBarBg, progressBarTrackStyle]}>
                <LinearGradient
                  colors={compact ? ['#123D73', '#0E7FA7', '#20C4D4'] : playerCardTheme.progressBarGradient}
                  start={{ x: 0, y: 0 }}
                  end={{ x: 1, y: 0 }}
                  style={[styles.progressBarFill, { width: `${Math.min(100, percentComplete)}%` }]}
                />
              ...
              {compact ? <View style={styles.progressRangeRow}><Text style={styles.progressRangeText}>0</Text><Text style={styles.progressRangeText}>{nextThreshold.toLocaleString()}</Text></View> : null}
              <Text style={[styles.progressEta, compact && styles.progressEtaCompact, progressMetaStyle]}>
                {isMasters
        ...
        {(() => {
          ...
          return (
            <View style={[styles.progressCard, styles.achievedCard, compact && styles.compactHidden, progressCardStyle]}>
              <View style={styles.progressHeader}>
                <View style={styles.achievedLabelRow}>
                  <CheckCircle2 size={14} color={CLEAN_THEME.badge.achieved.text} />
                  <Text style={[styles.progressLabel, compact && styles.progressLabelCompact, progressLabelStyle]}>
      ...
      <View style={[styles.statsRow, compact && styles.statsRowCompact, progressCardStyle]}>
        <View style={styles.statItem}>
          {compact ? <View style={styles.statIcon}><TrendingUp size={13} color="#17324D" /></View> : null}
        ...
        <View style={styles.statItem}>
          {compact ? <View style={styles.statIcon}><Star size={13} color="#17324D" fill="#17324D" /></View> : null}
        ...
        <View style={[styles.statDivider, progressDividerStyle]} />
        <View style={styles.statItem}>
          {compact ? <View style={styles.statIcon}><Moon size={13} color="#17324D" fill="#DDE9EE" /></View> : null}
      ...
      <View style={[styles.quickStatsPillRow, compact && styles.compactHidden]}>
        <TouchableOpacity 
        ...
        >
          ...
          <Text style={[styles.quickStatPillValue, progressLabelStyle]}>{sailingOptionCountText}</Text>
          ...
          {availableCruiseOptions != null && availableCruiseOptions !== availableCruises ? (
            <Text style={[styles.quickStatPillSubLabel, progressMetaStyle]}>{cruiseCountText} unique departures</Text>
          ) : null}
        </TouchableOpacity>
        ...
        >
          ...
          <Text style={[styles.quickStatPillValue, progressLabelStyle]}>{bookedCountText}</Text>
        ...
        >
          ...
          <Text style={[styles.quickStatPillValue, progressLabelStyle]}>{offerCountText}</Text>
      ...
      <View style={[styles.quickStatsPillRow, compact && styles.compactHidden]}>
        ...
        >
          <Anchor size={14} color={COLORS.points} />
          <Text style={[styles.quickStatPillValue, progressLabelStyle]}>{cruiseCountText}</Text>
...
const styles = StyleSheet.create({
  ...
  royalLineworkOuter: {
    ...
    borderRadius: 90,
    borderWidth: 1,
    borderColor: 'rgba(230,182,61,0.14)',
    right: -62,
    top: -70,
  },
  royalLineworkInner: {
    ...
    borderRadius: 63,
    borderWidth: 1,
    borderColor: 'rgba(230,182,61,0.18)',
    right: -35,
    top: -44,
  },
  royalCrest: {
  ...
  royalCrestLetter: {
    ...
    fontSize: 44,
    lineHeight: 50,
    textShadowColor: 'rgba(154,101,13,0.18)',
  ...
  statsRowCompact: {
    ...
    marginBottom: 0,
    flexWrap: 'nowrap',
    backgroundColor: 'rgba(247,250,250,0.86)',
  },
  statItem: {
    flex: 1,
    minWidth: 92,
