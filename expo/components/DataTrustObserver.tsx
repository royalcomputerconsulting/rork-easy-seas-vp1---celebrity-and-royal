...
export function DataTrustObserver() {
  ...
  useEffect(() => {
    ...
    const casino = (core.bookedCruises ?? []).map((row) => ({ ...(row as any), id: row.id, ownerId: (row as any).ownerProfileId ?? ownerId, points: (row as any).casinoPoints ?? (row as any).pointsEarned ?? 0, coinIn: (row as any).coinIn ?? 0 }));
    ...
    return { ownerId, cruises: allCruises as any[], offers: (core.casinoOffers ?? []) as any[], offerSailings, certificates: searchableCertificates as any[], casinoTotals: casino, loyalty: loyaltyRecords as any[] };
    });
