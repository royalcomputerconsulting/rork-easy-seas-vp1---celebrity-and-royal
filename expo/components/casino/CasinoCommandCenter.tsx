...
type CasinoTab = 'intelligence' | 'ship' | 'session' | 'charts' | 'calcs' | 'slots';
type SessionPeriod = 'today' | 'week' | 'month' | 'all';

const TABS: Array<{ id: CasinoTab; label: string; testID: string }> = [
  { id: 'intelligence', label: 'Overview', testID: 'casino-overview-tab' },
  { id: 'ship', label: 'Cruises', testID: 'casino-ship-intelligence-tab' },
  { id: 'session', label: 'Play', testID: 'casino-play-tab' },
  { id: 'charts', label: 'Analytics', testID: 'casino-analytics-tab' },
  { id: 'calcs', label: 'Calculator', testID: 'casino-calculator-tab' },
  { id: 'slots', label: 'Slots', testID: 'casino-open-slots-tab' },
];
...
function ColorMetric({ icon: Icon, label, value, detail, color, confidence }: { icon: React.ComponentType<{ size?: number; color?: string }>; label: string; value: string; detail: string; color: string; confidence?: string }) {
  ...
  return <TouchableOpacity style={[styles.colorMetric, isMissing && styles.colorMetricMissing, { borderTopColor: color }]} onPress={() => router.push({ pathname: '/casino/metric-evidence' as never, params: { label, value: displayValue, evidence, detail } } as never)} accessibilityRole="button" accessibilityLabel={`${label}. ${displayValue === '—' ? 'Data not available' : displayValue}. View contributing records`}>
    ...
    <View style={styles.colorMetricContent}>
      ...
      <Text style={styles.colorMetricDetail} numberOfLines={2}>{isMissing ? 'Add or reconcile a contributing cruise record.' : detail}</Text>
      <Text style={styles.metricDrill}>View contributing records ›</Text>
    </View>
  </TouchableOpacity>;
}
...
export default function CasinoCommandCenter() {
  ...
  useEffect(() => {
    ...
    if (requested === 'slots' || requested === 'machines') {
      router.replace('/machines' as never);
      return;
    }
    const mapped: Record<string, CasinoTab> = {
      ...
      cruises: 'ship', cruise: 'ship', ship: 'ship',
      charts: 'charts', chart: 'charts', analytics: 'charts',
      sessions: 'session', session: 'session',
      tools: 'calcs', action: 'calcs', simulator: 'calcs', calcs: 'calcs', calculations: 'calcs', calculator: 'calcs',
    };
  ...
  return <SafeAreaView style={[styles.safe, { backgroundColor: colors.background }]} edges={['top']} testID="casino-relationship-intelligence">
    <ScrollView style={styles.scroll} contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled">
      <View style={[styles.identityShell, { backgroundColor: colors.background }]}>
        <View style={[styles.authorIdentity, { backgroundColor: colors.surface, borderColor: colors.border }]} testID="casino-scott-astin-identity">
          <Image source={require('../../assets/images/easyseas-scott-astin-logo.jpeg')} style={styles.authorMark} contentFit="cover" contentPosition="center" accessibilityLabel="Easy Seas by Scott Astin" />
          <View style={styles.authorCopy}><Text style={[styles.authorName, { color: colors.heroText }]}>Scott Astin</Text><Text style={[styles.authorSection, { color: colors.muted }]}>Casino intelligence</Text></View>
          <View style={styles.authorSignatureBadge}><Text style={styles.authorSignatureText}>{tier || 'Signature'}</Text></View>
        </View>
      </View>
      <View style={[styles.programRow, { backgroundColor: colors.surface }]}> 
        ...
        <TouchableOpacity style={[styles.askButton, { backgroundColor: colors.accent }]} onPress={() => router.push('/ask-my-data')} accessibilityLabel="Ask Agent SEA"><Database size={17} color={colors.inverseText} /><Text style={[styles.askText, { color: colors.inverseText }]} numberOfLines={1}>Ask SEA</Text></TouchableOpacity>
      </View>

      <View
        accessibilityRole="tablist"
        accessibilityLabel="Casino sections"
        style={[styles.tabsScroller, { backgroundColor: colors.surface, borderBottomColor: colors.border }]}
      >
        <View style={styles.tabs}>
          {TABS.map((tab) => <TouchableOpacity key={tab.id} style={[styles.tab, activeTab === tab.id && styles.tabActive]} onPress={() => tab.id === 'slots' ? router.push('/machines' as never) : setActiveTab(tab.id)} accessibilityRole="tab" accessibilityLabel={`${tab.label} casino page`} accessibilityState={{ selected: activeTab === tab.id }} testID={tab.testID}><Text numberOfLines={1} style={[styles.tabText, { color: colors.muted }, activeTab === tab.id && { color: colors.accent }]}>{tab.label}</Text></TouchableOpacity>)}
        </View>
      </View>
      ...
      {casinoDataReady && activeTab === 'ship' ? <>
        <ThemedSectionHeader tab="casino" emoji="🚢" title="Casino cruises" subtitle="Cruise results share one card order and retain points, gaming, certificate, value, and evidence details." compact testID="casino-cruise-portfolio-section" />
        <View style={styles.cruiseActions}>
          <TouchableOpacity style={styles.compactAction} onPress={() => router.push('/casino/ship-performance')} accessibilityRole="button"><Ship size={17} color="#0b766d" /><Text style={styles.compactActionText}>Ship intelligence</Text></TouchableOpacity>
          <TouchableOpacity style={styles.compactAction} onPress={() => router.push('/casino/ship-observations')} accessibilityRole="button"><Ticket size={17} color="#0b766d" /><Text style={styles.compactActionText}>Machine reports</Text></TouchableOpacity>
        </View>
...
const styles = StyleSheet.create({
  ...
  authorIdentity: { minHeight: 72, borderRadius: 16, borderWidth: 1, padding: 9, flexDirection: 'row', alignItems: 'center', gap: 10, shadowColor: '#0F2247', shadowOpacity: 0.05, shadowRadius: 8, shadowOffset: { width: 0, height: 3 } },
  authorMark: { width: 54, height: 54, borderRadius: 12, backgroundColor: '#DFF2EF' },
  authorCopy: { flex: 1, minWidth: 0 },
  authorName: { fontFamily: TYPOGRAPHY.fontFamilyEditorialSemibold, fontSize: 21, fontWeight: '600' },
  authorSection: { marginTop: 2, fontSize: 12, fontWeight: '700' },
  authorSignatureBadge: { borderRadius: 999, borderWidth: 1, borderColor: '#D8AA32', backgroundColor: '#FFF9E8', paddingHorizontal: 9, paddingVertical: 6, maxWidth: 104 },
  authorSignatureText: { color: '#6D4B00', fontSize: 11, fontWeight: '900' },
  ...
  askButton: { minHeight: 44, flexShrink: 0, flexDirection: 'row', gap: 5, alignItems: 'center', justifyContent: 'center', backgroundColor: '#123D73', paddingHorizontal: 12, paddingVertical: 9, borderRadius: 12 },
  askText: { color: '#fff', fontWeight: '800', fontSize: 12 },
  programRow: { backgroundColor: '#FFFFFF', flexDirection: 'row', alignItems: 'center', paddingHorizontal: 14, paddingBottom: 12, gap: 8 },
  programControls: { flex: 1, minWidth: 0, flexDirection: 'row', gap: 8 },
  ...
  tabsScroller: { flexGrow: 0, borderBottomWidth: StyleSheet.hairlineWidth, paddingHorizontal: 8, paddingBottom: 6 },
  tabs: { flexDirection: 'row', flexWrap: 'wrap' },
  tab: { width: '33.3333%', minHeight: 44, paddingHorizontal: 4, paddingVertical: 10, alignItems: 'center', justifyContent: 'center', borderBottomWidth: 3, borderBottomColor: 'transparent' },
  ...
  colorMetric: { width: '48%', minHeight: 136, backgroundColor: '#fff', borderRadius: 15, padding: 11, borderWidth: 1, borderColor: '#d8e4e8', borderTopWidth: 4, gap: 7 },
  colorMetricMissing: { minHeight: 108, backgroundColor: '#F8F7F3' },
  colorMetricIcon: { width: 34, height: 34, borderRadius: 17, alignItems: 'center', justifyContent: 'center' },
  colorMetricContent: { flex: 1 },
  colorMetricLabel: { color: '#4a5e6d', fontSize: 12, fontWeight: '900', flex: 1 },
  colorMetricValue: { fontSize: 22, fontWeight: '900', marginTop: 2 },
  colorMetricDetail: { color: '#667987', fontSize: 11, lineHeight: 15, marginTop: 2 },
  confidencePill: { fontSize: 9, fontWeight: '900', paddingHorizontal: 5, paddingVertical: 3, borderRadius: 5, overflow: 'hidden', maxWidth: 86 },
  ...
  metric: { backgroundColor: '#fff', borderRadius: 14, padding: 11, width: '48%', minHeight: 104, borderWidth: 1, borderColor: '#d8e4e8' },
  ...
  primaryAction: { alignSelf: 'flex-start', minHeight: 44, backgroundColor: '#0b766d', borderRadius: 13, paddingHorizontal: 14, paddingVertical: 10, flexDirection: 'row', gap: 8, alignItems: 'center', justifyContent: 'center', marginTop: 12 }, primaryActionText: { color: '#fff', fontWeight: '900', fontSize: 13 },
  ...
  cruiseActions: { flexDirection: 'row', gap: 8, marginBottom: 12 },
  compactAction: { flex: 1, minHeight: 44, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 7, paddingHorizontal: 10, borderRadius: 12, borderWidth: 1, borderColor: '#b8cbd3', backgroundColor: '#FFFFFF' },
  compactActionText: { color: '#0d355a', fontSize: 11, fontWeight: '900', textAlign: 'center', flexShrink: 1 },
