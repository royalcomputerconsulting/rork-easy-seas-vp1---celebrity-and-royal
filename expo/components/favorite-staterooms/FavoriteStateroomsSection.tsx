...
function FavoriteStateroomsSectionInner({ shipOptions = [], showHeader = true }: FavoriteStateroomsSectionProps) {
  const {
    entries,
    allEntries,
    entriesTotal,
  ...
  const handleDelete = useCallback((entry: FavoriteStateroom) => {
    Alert.alert(
      'Delete Favorite Stateroom',
      `Remove stateroom ${entry.stateroomNumber} on ${entry.shipName} from your favorites?`,
      [
  ...
  const handleClearAll = useCallback(() => {
    ...
    Alert.alert(
      'Clear Favorite Staterooms',
      'Delete every favorite stateroom in this list?',
      [
        { text: 'Cancel', style: 'cancel' },
        {
          text: 'Clear All',
  ...
  const renderInput = useCallback((label: string, field: keyof FavoriteStateroomDraft, placeholder: string, multiline?: boolean) => {
    ...
    return (
      <View style={styles.fieldGroup} key={field}>
        ...
        <TextInput
          style={[styles.input, isLongField && styles.textArea]}
          value={draft[field] ?? ''}
          onChangeText={(value) => updateDraftValue(field, value)}
          placeholder={placeholder}
          placeholderTextColor={COLORS.textTertiary}
          multiline={isLongField}
          textAlignVertical={isLongField ? 'top' : 'center'}
          testID={`favoriteStaterooms.input.${field}`}
        />
      </View>
    );
  ...
  return (
    <View style={styles.sectionContainer} testID="favoriteStaterooms.section">
      {showHeader ? <LinearGradient colors={['#0F3D63', '#126A82', '#1C4F8C']} style={styles.headerCard}>
      ...
      <View style={styles.actionsRow}>
        ...
        <TouchableOpacity
          style={[styles.secondaryAction, entriesTotal === 0 && styles.secondaryActionDisabled]}
          onPress={handleClearAll}
          disabled={entriesTotal === 0}
          activeOpacity={0.85}
          testID="favoriteStaterooms.clearButton"
        >
      ...
      <View style={styles.searchRow}>
        <View style={styles.searchBox}>
          ...
          <TextInput
            ...
            placeholder="Search ship, room, notes, nearby rooms..."
            placeholderTextColor={COLORS.textTertiary}
            testID="favoriteStaterooms.searchInput"
          />
          {filters.search.length > 0 && (
            <TouchableOpacity onPress={() => updateFilters({ search: '' })} testID="favoriteStaterooms.clearSearchButton">
        ...
        <TouchableOpacity
          ...
          activeOpacity={0.85}
          testID="favoriteStaterooms.filterButton"
        >
      ...
      {showFilters && (
        <View style={styles.filtersPanel}>
          <View style={styles.filtersHeaderRow}>
            <Text style={styles.filtersTitle}>Ships</Text>
            {activeFilterCount > 0 && (
              <TouchableOpacity onPress={resetFilters} testID="favoriteStaterooms.resetFiltersButton">
          ...
          <View style={styles.shipChipRow}>
            <TouchableOpacity
              style={[styles.shipChip, filters.shipNames.length === 0 && styles.shipChipActive]}
              onPress={() => updateFilters({ shipNames: [] })}
              activeOpacity={0.85}
              testID="favoriteStaterooms.filterShip.all"
            >
              <Text style={[styles.shipChipText, filters.shipNames.length === 0 && styles.shipChipTextActive]}>All Ships</Text>
            </TouchableOpacity>
            {uniqueShips.map((shipName) => {
              const isSelected = filters.shipNames.includes(shipName);
              return (
                <TouchableOpacity
                  key={shipName}
                  style={[styles.shipChip, isSelected && styles.shipChipActive]}
                  onPress={() => handleToggleShipFilter(shipName)}
                  activeOpacity={0.85}
                  testID={`favoriteStaterooms.filterShip.${shipName}`}
                >
                  <Text style={[styles.shipChipText, isSelected && styles.shipChipTextActive]}>{shipName}</Text>
                </TouchableOpacity>
              );
            })}
          </View>
      ...
      {isLoading ? (
        ...
        <View style={styles.emptyCard} testID="favoriteStaterooms.emptyState">
        ...
        <View style={styles.cardsList}>
          {entries.map((entry) => (
            <View key={entry.id} style={styles.entryCard} testID={`favoriteStaterooms.card.${entry.id}`}>
              <View style={styles.entryHeader}>
                ...
                <View style={styles.entryActions}>
                  <TouchableOpacity
                    ...
                    activeOpacity={0.85}
                    testID={`favoriteStaterooms.editButton.${entry.id}`}
                  >
                  ...
                  <TouchableOpacity
                    ...
                    activeOpacity={0.85}
                    testID={`favoriteStaterooms.deleteButton.${entry.id}`}
                  >
              ...
              <View style={styles.metaRow}>
                {!!entry.deckNumber && (
                  <View style={styles.metaPill}>
                    <Check size={12} color={COLORS.navyDeep} />
                    <Text style={styles.metaPillText}>Deck {entry.deckNumber}</Text>
                  </View>
                )}
                {!!entry.category && (
                  <View style={styles.metaPill}>
                    <Tag size={12} color={COLORS.navyDeep} />
                    <Text style={styles.metaPillText}>{entry.category}</Text>
                  </View>
                )}
              </View>
              ...
              {!!entry.locationNotes && (
                <View style={styles.detailBlock}>
                  <Text style={styles.detailLabel}>Location notes</Text>
                  <Text style={styles.detailText}>{entry.locationNotes}</Text>
                </View>
              )}

              {!!entry.nearbyAlternatives && (
                <View style={styles.detailBlock}>
                  <Text style={styles.detailLabel}>Nearby alternatives</Text>
                  <Text style={styles.detailText}>{entry.nearbyAlternatives}</Text>
                </View>
              )}

              {!!entry.notes && (
                <View style={styles.detailBlock}>
                  <Text style={styles.detailLabel}>Booking notes</Text>
        ...
        <View style={styles.modalOverlay}>
          <View style={styles.modalCard}>
            ...
            <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.modalScrollContent}>
              {uniqueShips.length > 0 && (
                <View style={styles.quickPickSection}>
                  <Text style={styles.quickPickLabel}>Quick pick ship</Text>
                  <View style={styles.quickPickRow}>
                    {uniqueShips.slice(0, 8).map((shipName) => {
                      const isSelected = draft.shipName.trim() === shipName;
                      return (
                        <TouchableOpacity
                          key={shipName}
                          style={[styles.quickPickChip, isSelected && styles.quickPickChipActive]}
                          onPress={() => updateDraftValue('shipName', shipName)}
                          activeOpacity={0.85}
                          testID={`favoriteStaterooms.quickPick.${shipName}`}
                        >
                          <Text style={[styles.quickPickChipText, isSelected && styles.quickPickChipTextActive]}>{shipName}</Text>
                        </TouchableOpacity>
                      );
                    })}
                  </View>
            ...
            <View style={styles.modalActions}>
              ...
              <TouchableOpacity
                style={[styles.modalPrimaryButton, isSubmitting && styles.modalPrimaryButtonDisabled]}
                onPress={() => {
                  void handleSave();
                }}
                disabled={isSubmitting}
                activeOpacity={0.85}
                testID="favoriteStaterooms.saveButton"
              >
                {isSubmitting ? (
                  <ActivityIndicator size="small" color={COLORS.white} />
                ) : (
                  <Text style={styles.modalPrimaryButtonText}>{isEditing ? 'Save Changes' : 'Save Favorite'}</Text>
                )}
              </TouchableOpacity>
            </View>
          </View>
