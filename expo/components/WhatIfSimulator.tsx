...
import { Zap, Ship, Plus, Minus, TrendingUp, ArrowRight, ChevronDown, ChevronUp, Sparkles, Target, RefreshCw, Home, } from 'lucide-react-native';
import { COLORS, SPACING, BORDER_RADIUS, TYPOGRAPHY, SHADOW, CLEAN_THEME } from '@/constants/theme';
import { formatCurrency, formatNumber } from '@/lib/format';
import { ScenarioType, ScenarioInput, SimulationResult, runComparisonSimulation, PlayerContext, } from '@/lib/whatIfSimulator';
...
export function WhatIfSimulator({ playerContext, bookedCruises, offers = [], onSimulationComplete, }: WhatIfSimulatorProps) {
    ...
    const [selectedCruiseId, setSelectedCruiseId] = useState<string | null>(bookedCruises[0]?.id || null);
    ...
    const runSimulation = useCallback(() => {
        ...
        setTimeout(() => {
            const simulationResult = runComparisonSimulation(playerContext, bookedCruises, scenario, offers);
        ...
        switch (selectedScenario) {
            ...
            <View style={styles.inputRow}>
              <View style={styles.inputGroup}>
                ...
                <TextInput style={styles.input} value={newNights} onChangeText={setNewNights} keyboardType="numeric" placeholder="7" placeholderTextColor={COLORS.textSecondary}/>
              </View>
              <View style={styles.inputGroup}>
                <Text style={styles.inputLabel}>Est. Spend</Text>
                <TextInput style={styles.input} value={newSpend} onChangeText={setNewSpend} keyboardType="numeric" placeholder="2000" placeholderTextColor={COLORS.textSecondary}/>
              </View>
            </View>
          </View>);
            case 'remove_cruise':
            ...
            <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.cruiseScroll}>
              {bookedCruises.map((cruise) => (<TouchableOpacity key={cruise.id} style={[
                            styles.cruiseOption,
                            selectedCruiseId === cruise.id && styles.cruiseOptionSelected,
                        ]} onPress={() => setSelectedCruiseId(cruise.id)}>
                  <Ship size={14} color={selectedCruiseId === cruise.id ? COLORS.navyDeep : COLORS.textSecondary}/>
                  <Text style={[
            ...
            <View style={styles.cabinOptions}>
              {CABIN_OPTIONS.map((cabin) => (<TouchableOpacity key={cabin} style={[
                            styles.cabinOption,
            ...
            <View style={styles.inputRow}>
              <View style={styles.inputGroup}>
                ...
                <TextInput style={styles.input} value={customPoints} onChangeText={setCustomPoints} keyboardType="numeric" placeholder="0" placeholderTextColor={COLORS.textSecondary}/>
              </View>
              <View style={styles.inputGroup}>
                <Text style={styles.inputLabel}>Add Nights</Text>
                <TextInput style={styles.input} value={customNights} onChangeText={setCustomNights} keyboardType="numeric" placeholder="0" placeholderTextColor={COLORS.textSecondary}/>
              </View>
            </View>
            <View style={styles.inputGroupFull}>
              <Text style={styles.inputLabel}>Additional Spend</Text>
              <TextInput style={styles.input} value={newSpend} onChangeText={setNewSpend} keyboardType="numeric" placeholder="0" placeholderTextColor={COLORS.textSecondary}/>
            </View>
          </View>);
            default:
                return null;
    ...
    const renderResults = () => {
        ...
        <LinearGradient colors={['rgba(77, 208, 225, 0.15)', 'rgba(77, 208, 225, 0.05)']} style={styles.resultsGradient}>
    ...
    return (<View style={styles.container}>
      <TouchableOpacity style={styles.header} onPress={() => setExpanded(!expanded)} activeOpacity={0.8}>
        <LinearGradient colors={['rgba(139, 92, 246, 0.25)', 'rgba(139, 92, 246, 0.1)']} style={styles.headerGradient}>
