...
import { Flame, Trophy, Target, Star, Award, Zap, Crown, Sun, Moon, Ship, Coins, Medal, ChevronRight, Clock, CheckCircle, Lock, Sparkles, Calendar, } from 'lucide-react-native';
          ...
          <View style={styles.streakRow}>
            <View style={[styles.streakIconContainer, { backgroundColor: 'rgba(249, 115, 22, 0.15)' }]}>
              <Animated.View style={{ transform: [{ scale: flameScale }] }}>
                <Flame size={16} color={isStreakAlive ? '#F97316' : '#9CA3AF'} fill={isStreakAlive ? '#F97316' : 'transparent'}/>
              </Animated.View>
            </View>
...
function WeeklyGoalItem({ goal }: {
    ...
    return (<View style={[styles.goalItem, isCompleted && styles.goalItemCompleted]}>
      <View style={styles.goalIconContainer}>
        {isCompleted ? (<CheckCircle size={18} color={COLORS.success} fill={COLORS.success}/>) : (getGoalIcon())}
      </View>
...
function AchievementBadge({ achievement, onPress }: {
    ...
    return (<TouchableOpacity style={[
      ...
      <View style={[
        ...
        {isUnlocked ? (getAchievementIcon(achievement.icon, 16, '#FFFFFF')) : (<Lock size={12} color="#FFFFFF"/>)}
      </View>
      <Text style={[
...
function LevelDisplay({ level, xp, nextLevelXP }: {
    level: number;
    xp: number;
    nextLevelXP: number;
}) {
    ...
    return (<LinearGradient colors={marbleConfig.gradientColors as unknown as [
        ...
        string,
        ...string[]
    ]} locations={marbleConfig.gradientLocations} start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }} style={styles.levelContainer}>
      ...
      <View style={styles.levelInfo}>
        ...
        <View style={styles.xpProgressContainer}>
          <View style={styles.xpProgressBar}>
            <LinearGradient colors={['#8B5CF6', '#A78BFA']} start={{ x: 0, y: 0 }} end={{ x: 1, y: 0 }} style={[styles.xpProgressFill, { width: `${Math.min(progress, 100)}%` }]}/>
          </View>
...
export function GamificationCard({ showStreaks = true, showAchievements = true, showWeeklyGoals = true, showLevel = true, compact = false, maxAchievements = 6, }: GamificationCardProps) {
    const { achievements, streak, weeklyGoals, stats, getUnlockedAchievements, getLockedAchievements, } = useGamification();
