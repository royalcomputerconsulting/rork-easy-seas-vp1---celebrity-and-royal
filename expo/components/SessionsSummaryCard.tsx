...
export const SessionsSummaryCard = React.memo(function SessionsSummaryCard({
  ...
  const pphStatus = useMemo(() => {
    if (targetPPH === 0) return { status: 'N/A', color: '#676A70' };
    const percentage = (analytics.pointsPerHour / targetPPH) * 100;
    if (percentage >= 120) return { status: 'Exceptional', color: '#4EC0A5' };
    if (percentage >= 100) return { status: 'On Target', color: '#3B82F6' };
    if (percentage >= 70) return { status: 'Close', color: '#E6B63D' };
    return { status: 'Below', color: '#A52B34' };
    ...
    <View style={styles.container}>
      ...
      <View style={styles.content}>
            ...
            {recentTrend.direction !== 'stable' && (
              <View style={styles.trendBadge}>
                {recentTrend.direction === 'up' ? (
                  <TrendingUp size={10} color="#4EC0A5" />
                ) : (
                  <TrendingDown size={10} color="#A52B34" />
                )}
                <Text style={[
                  styles.trendText,
                  { color: recentTrend.direction === 'up' ? '#4EC0A5' : '#A52B34' }
                ]}>
        ...
        <View style={styles.metricRow}>
          ...
          <Text style={[styles.metricValue, { color: '#4EC0A5' }]}>
        ...
        <View style={styles.metricRow}>
          ...
          <Text style={[styles.metricValue, { color: '#E6B63D' }]}>
        ...
        <View style={styles.metricRow}>
          <View style={[styles.metricIcon, { backgroundColor: analytics.netWinLoss >= 0 ? 'rgba(16, 185, 129, 0.15)' : 'rgba(239, 68, 68, 0.15)' }]}>
            <TrendingUp size={16} color={analytics.netWinLoss >= 0 ? '#4EC0A5' : '#A52B34'} />
          </View>
          <View style={styles.metricInfo}>
            <Text style={styles.metricLabel}>Net Win/Loss</Text>
            <Text style={styles.metricSubtext}>Total gambling result</Text>
          </View>
          <Text style={[styles.metricValue, { color: analytics.netWinLoss >= 0 ? '#4EC0A5' : '#A52B34' }]}>
            ...
            <View style={styles.highlightSection}>
              ...
              <View style={styles.highlightStats}>
                <View style={styles.highlightStat}>
                  ...
                  <Text style={[styles.highlightStatValue, { color: '#E6B63D' }]}>
