...
import { Trophy, Medal, Award, Clock, Calendar, Zap, Crown, TrendingUp, Flame, } from 'lucide-react-native';
...
function AnimatedRankBadge({ rank, delay, }: {
    rank: number;
    delay: number;
}) {
...
function LeaderboardRow({ session, rank, delay, isTop3, onPress, }: {
    ...
    return (<Animated.View style={[
      ...
      <TouchableOpacity style={styles.rowTouchable} onPress={handlePress} activeOpacity={0.7} disabled={!onPress}>
...
export function PPHLeaderboard({ sessions, maxEntries = 3, onSessionPress, }: PPHLeaderboardProps) {
    ...
    return (<View style={styles.container}>
      ...
      <View style={styles.content}>
        ...
        <View style={styles.leaderboardList}>
          {rankedSessions.map((session, index) => (<LeaderboardRow key={session.id} session={session} rank={session.rank} delay={index * 80} isTop3={index < 3} onPress={onSessionPress ? () => onSessionPress(session) : undefined}/>))}
        </View>
      </View>
    </View>);
}
