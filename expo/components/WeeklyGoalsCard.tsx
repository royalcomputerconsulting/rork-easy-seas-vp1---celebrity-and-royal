...
function CircularProgress({
  ...
  const { preferences, motionDuration } = useExperience();
  const animatedProgress = useRef(new Animated.Value(preferences.reducedMotion ? Math.min(progress, 100) : 0)).current;
  ...
  }, [progress, animatedProgress, motionDuration, preferences.reducedMotion]);
...
function GoalProgressRing({
  ...
  useEffect(() => {
    if (isCompleted) {
      ...
      const completionAnimation = Animated.parallel([
  ...
  }, [isCompleted, checkScaleAnim, motionDuration, preferences.reducedMotion, pulseAnim]);
    ...
    <Animated.View style={[styles.goalRingContainer, { transform: [{ scale: pulseAnim }] }]}>
      <TouchableOpacity onPress={handlePress} activeOpacity={0.7}>
        <CircularProgress
          ...
          size={90}
          strokeWidth={8}
          color={isCompleted ? '#4EC0A5' : config.color}
          backgroundColor={config.lightColor}
        >
          {isCompleted ? (
            <Animated.View style={{ transform: [{ scale: checkScaleAnim }] }}>
              <CheckCircle size={32} color="#4EC0A5" fill="#4EC0A5" />
            </Animated.View>
          ) : (
    ...
    <View style={styles.weekProgressContainer}>
      ...
      <View style={styles.progressLegend}>
        <View style={styles.legendItem}>
          <View style={[styles.legendDot, { backgroundColor: '#4EC0A5' }]} />
          <Text style={styles.legendText}>{completedCount} Completed</Text>
        </View>
        <View style={styles.legendItem}>
          <View style={[styles.legendDot, { backgroundColor: '#E6B63D' }]} />
            ...
            return (
              <View key={goal.id} style={[
                ...
                <View style={[styles.goalListIcon, { backgroundColor: config.lightColor }]}>
                  {goal.completed ? (
                    <CheckCircle size={16} color="#4EC0A5" fill="#4EC0A5" />
                  ) : (
                ...
                <View style={styles.goalListContent}>
                  <View style={styles.goalListHeader}>
                    ...
                    <Text style={[
                      ...
                      { color: goal.completed ? '#4EC0A5' : config.color },
                    ]}>
                  ...
                  <View style={styles.goalListProgressBar}>
                    <View 
                      style={[
                        ...
                        { 
                          width: `${progress}%`,
                          backgroundColor: goal.completed ? '#4EC0A5' : config.color,
                        },
                      ]} 
                    />
                  </View>
                </View>
                <ChevronRight size={14} color="#D5D5D0" />
              </View>
            );
          })}
        </View>
