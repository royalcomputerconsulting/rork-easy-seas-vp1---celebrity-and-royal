...
export function UserProfileCard({ currentValues, enrichmentData, onSave, isSaving = false, }: UserProfileCardProps) {
    const entitlement = useEntitlement();
    const [formData, setFormData] = useState<UserProfileData>(currentValues);
    const [activeBrand, setActiveBrand] = useState<BrandType>((currentValues.preferredBrand as BrandType) || 'royal');
    ...
    const renderEnrichmentBadge = (hasData: boolean) => (<View style={[styles.enrichmentBadge, hasData ? styles.enrichmentBadgeActive : styles.enrichmentBadgeInactive]}>
      {hasData ? (<CheckCircle size={10} color={COLORS.white}/>) : (<AlertCircle size={10} color="rgba(255,255,255,0.7)"/>)}
      <Text style={styles.enrichmentBadgeText}>
        {hasData ? 'Synced' : 'Manual'}
      </Text>
    </View>);
    const renderValueCard = (label: string, value: string | number | undefined, color?: string, wide?: boolean) => (<View style={[styles.valueCard, wide && styles.valueCardWide]}>
    ...
    const getBrandGradient = () => {
        switch (activeBrand) {
            case 'royal':
                return ['#0369A1', '#0284C7'] as [
                    string,
                    string
                ];
            case 'celebrity':
                return ['#1E40AF', '#2563EB'] as [
                    string,
                    string
                ];
            case 'silversea':
                return ['#78350F', '#92400E'] as [
                    string,
                    string
                ];
            case 'carnival':
                return ['#CC2232', '#E02040'] as [
                    string,
                    string
                ];
            default:
    ...
    const renderEditForm = () => {
        if (activeBrand === 'royal') {
          ...
          <View style={styles.inputGroup}>
            ...
            <TextInput style={styles.input} value={formData.name} onChangeText={(text) => setFormData(prev => ({ ...prev, name: text }))} placeholder="Enter your name" placeholderTextColor="#9CA3AF"/>
          </View>
          <View style={styles.inputGroup}>
            <Text style={styles.inputLabel}>Email Address</Text>
            <TextInput style={styles.input} value={formData.email} onChangeText={(text) => setFormData(prev => ({ ...prev, email: text }))} placeholder="Enter your email" placeholderTextColor="#9CA3AF" keyboardType="email-address" autoCapitalize="none" autoCorrect={false}/>
          </View>
          <View style={styles.inputGroup}>
            <Text style={styles.inputLabel}>Crown & Anchor #</Text>
            <TextInput style={styles.input} value={formData.crownAnchorNumber} onChangeText={(text) => setFormData(prev => ({ ...prev, crownAnchorNumber: text }))} placeholder="Enter Crown & Anchor number" placeholderTextColor="#9CA3AF" keyboardType="numeric"/>
          </View>
          <View style={styles.inputGroup}>
            <Text style={styles.inputLabel}>Loyalty Points (Nights)</Text>
            <TextInput style={styles.input} value={formData.loyaltyPoints.toString()} onChangeText={(text) => setFormData(prev => ({ ...prev, loyaltyPoints: parseInt(text) || 0 }))} placeholder="Enter loyalty nights" placeholderTextColor="#9CA3AF" keyboardType="numeric"/>
          ...
          <View style={styles.inputGroup}>
            ...
            <TextInput style={styles.input} value={formData.clubRoyalePoints.toString()} onChangeText={(text) => setFormData(prev => ({ ...prev, clubRoyalePoints: parseInt(text) || 0 }))} placeholder="Enter current points" placeholderTextColor="#9CA3AF" keyboardType="numeric"/>
            <View style={styles.levelHint}>
          ...
          <View style={styles.inputGroup}>
            ...
            <TextInput style={styles.input} value={formData.celebrityEmail || ''} onChangeText={(text) => setFormData(prev => ({ ...prev, celebrityEmail: text }))} placeholder="Enter Celebrity email" placeholderTextColor="#9CA3AF" keyboardType="email-address" autoCapitalize="none" autoCorrect={false}/>
          </View>
          <View style={styles.inputGroup}>
            <Text style={styles.inputLabel}>Captain&apos;s Club #</Text>
            <TextInput style={styles.input} value={formData.celebrityCaptainsClubNumber || ''} onChangeText={(text) => setFormData(prev => ({ ...prev, celebrityCaptainsClubNumber: text }))} placeholder="Enter Captain's Club number" placeholderTextColor="#9CA3AF" keyboardType="numeric"/>
          </View>
          <View style={styles.inputGroup}>
            <Text style={styles.inputLabel}>Captain&apos;s Club Points</Text>
            <TextInput style={styles.input} value={(formData.celebrityCaptainsClubPoints || 0).toString()} onChangeText={(text) => setFormData(prev => ({ ...prev, celebrityCaptainsClubPoints: parseInt(text) || 0 }))} placeholder="Enter Captain's Club points" placeholderTextColor="#9CA3AF" keyboardType="numeric"/>
            <View style={styles.levelHint}>
          ...
          <View style={styles.inputGroup}>
            ...
            <TextInput style={styles.input} value={(formData.celebrityBlueChipPoints || 0).toString()} onChangeText={(text) => setFormData(prev => ({ ...prev, celebrityBlueChipPoints: parseInt(text) || 0 }))} placeholder="Enter Blue Chip points" placeholderTextColor="#9CA3AF" keyboardType="numeric"/>
            <View style={styles.levelHint}>
          ...
          <View style={styles.inputGroup}>
            ...
            <TextInput style={styles.input} value={formData.carnivalVifpNumber || ''} onChangeText={(text) => setFormData(prev => ({ ...prev, carnivalVifpNumber: text }))} placeholder="Enter VIFP number" placeholderTextColor="#9CA3AF" keyboardType="numeric"/>
          </View>
          <View style={styles.inputGroup}>
            <Text style={styles.inputLabel}>VIFP Tier</Text>
            <TextInput style={styles.input} value={formData.carnivalVifpTier || ''} onChangeText={(text) => setFormData(prev => ({ ...prev, carnivalVifpTier: text }))} placeholder="e.g., Red, Gold, Platinum, Diamond" placeholderTextColor="#9CA3AF"/>
          </View>
          <View style={styles.inputGroup}>
            <Text style={styles.inputLabel}>Players Club Tier</Text>
            <TextInput style={styles.input} value={formData.carnivalPlayersClubTier || ''} onChangeText={(text) => setFormData(prev => ({ ...prev, carnivalPlayersClubTier: text }))} placeholder="e.g., Red, Gold, Platinum" placeholderTextColor="#9CA3AF"/>
          </View>
          <View style={styles.inputGroup}>
            <Text style={styles.inputLabel}>Players Club Points</Text>
            <TextInput style={styles.input} value={(formData.carnivalPlayersClubPoints || 0).toString()} onChangeText={(text) => setFormData(prev => ({ ...prev, carnivalPlayersClubPoints: parseInt(text) || 0 }))} placeholder="Enter points" placeholderTextColor="#9CA3AF" keyboardType="numeric"/>
          </View>
        </>);
        }
        else {
          ...
          <View style={styles.inputGroup}>
            ...
            <TextInput style={styles.input} value={formData.silverseaEmail || ''} onChangeText={(text) => setFormData(prev => ({ ...prev, silverseaEmail: text }))} placeholder="Enter Silversea email" placeholderTextColor="#9CA3AF" keyboardType="email-address" autoCapitalize="none" autoCorrect={false}/>
          </View>
          <View style={styles.inputGroup}>
            <Text style={styles.inputLabel}>Venetian Society Member #</Text>
            <TextInput style={styles.input} value={formData.silverseaVenetianNumber || ''} onChangeText={(text) => setFormData(prev => ({ ...prev, silverseaVenetianNumber: text }))} placeholder="Enter member number" placeholderTextColor="#9CA3AF" keyboardType="numeric"/>
          </View>
          <View style={styles.inputGroup}>
            <Text style={styles.inputLabel}>Venetian Tier</Text>
            <TextInput style={styles.input} value={formData.silverseaVenetianTier || ''} onChangeText={(text) => setFormData(prev => ({ ...prev, silverseaVenetianTier: text }))} placeholder="e.g., Silver, Gold, Platinum" placeholderTextColor="#9CA3AF"/>
          </View>
          <View style={styles.inputGroup}>
            <Text style={styles.inputLabel}>Venetian Points</Text>
            <TextInput style={styles.input} value={(formData.silverseaVenetianPoints || 0).toString()} onChangeText={(text) => setFormData(prev => ({ ...prev, silverseaVenetianPoints: parseInt(text) || 0 }))} placeholder="Enter points" placeholderTextColor="#9CA3AF" keyboardType="numeric"/>
          </View>
        </>);
        }
    };
    return (<View style={styles.container}>
      <LinearGradient colors={getBrandGradient()} style={styles.header}>
      ...
      <View style={styles.editButtonContainer}>
        <TouchableOpacity style={styles.editButton} onPress={() => setIsModalVisible(true)} activeOpacity={0.7}>
          ...
          <View style={styles.modalContainer}>
            <ScrollView showsVerticalScrollIndicator={false}>
              <View style={styles.modalHeader}>
                ...
                <TouchableOpacity style={styles.modalCloseButton} onPress={() => setIsModalVisible(false)} activeOpacity={0.7}>
                  <X size={20} color="#6B7280"/>
                </TouchableOpacity>
              </View>

              <View style={styles.modalActions}>
                <TouchableOpacity style={styles.modalCancelButton} onPress={() => setIsModalVisible(false)} activeOpacity={0.7} testID="profile-modal-cancel-top-button">
                  <Text style={styles.modalCancelText}>Cancel</Text>
                </TouchableOpacity>
                <TouchableOpacity style={styles.modalSaveButton} onPress={handleSave} activeOpacity={0.7} disabled={isSaving} testID="profile-modal-save-top-button">
                  <LinearGradient colors={getBrandGradient()} start={{ x: 0, y: 0 }} end={{ x: 1, y: 0 }} style={styles.modalSaveButtonGradient}>
