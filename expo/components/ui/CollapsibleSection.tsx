...
export function CollapsibleSection({ title, subtitle, icon, children, defaultExpanded = true, headerStyle = 'default', showBorder = true, onToggle, }: CollapsibleSectionProps) {
    ...
    return (<View style={[styles.container, showBorder && styles.containerBorder]}>
      <LinearGradient colors={marbleConfig.gradientColors as unknown as [
        string,
        string,
        ...string[]
    ]} locations={marbleConfig.gradientLocations} start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }} style={styles.headerBackground}>
        <TouchableOpacity style={[
            styles.header,
            headerStyle === 'compact' && styles.headerCompact,
        ]} onPress={toggleExpanded} activeOpacity={0.7}>
          <View style={styles.headerLeft}>
            {icon ? <View style={styles.iconContainer}>{renderNodeInView(icon)}</View> : null}
      ...
      {expanded ? <View style={styles.content}>{renderNodeInView(children)}</View> : null}
    </View>);
}
