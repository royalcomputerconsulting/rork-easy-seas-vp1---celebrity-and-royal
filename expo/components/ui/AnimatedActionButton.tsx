...
export function AnimatedActionButton({
  ...
  const { preferences, minimumControlSize, motionDuration } = useExperience();
  ...
  }, [motionDuration, opacityAnim, preferences.reducedMotion, scaleAnim]);

  const handlePressOut = useCallback(() => {
    if (preferences.reducedMotion) { scaleAnim.setValue(1); opacityAnim.setValue(1); return; }
    Animated.parallel([
      Animated.spring(scaleAnim, {
        toValue: 1,
        useNativeDriver: true,
  ...
  const sizeConfig = {
    ...
    large: {
      ...
      gap: 8,
      minHeight: Math.max(48, minimumControlSize),
    },
  };
  ...
  const variantConfig = {
    ...
    danger: {
      gradient: ['#A52B34', '#A52B34'] as [string, string],
