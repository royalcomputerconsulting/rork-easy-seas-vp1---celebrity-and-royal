...
export const MinimalistFilterBar = React.memo(function MinimalistFilterBar({
  ...
  const balancedWidth = (count: number): `${number}%` => {
    if (count <= 1) return '100%';
    if (count === 2) return '49%';
    if (count === 3) return '31.8%';
    if (count === 4) return '23.5%';
    if (count === 5) return '18.4%';
    return '31.8%';
  };
        ...
        <View style={[styles.tabsContainer, { backgroundColor: colors.surfaceMuted, borderColor: colors.border }, !hasVisibleActions && styles.tabsContainerFull]}>
          {tabs.map((tab) => {
            ...
            return (
              <TouchableOpacity
                key={tab.key}
                style={[styles.tab, { width: balancedWidth(tabs.length), minHeight: minimumControlSize, backgroundColor: colors.surface, borderColor: colors.border }, isActive && { backgroundColor: colors.accent, borderColor: colors.accent }]}
        ...
        {hasVisibleActions ? <View style={styles.actionsContainer}>
          {visibleActions.map((action) => {
            ...
            return (
              <TouchableOpacity
                key={action.key}
                style={[styles.actionPill, { width: balancedWidth(visibleActions.length), minHeight: minimumControlSize, backgroundColor: colors.surface, borderColor: colors.border }, action.active && { backgroundColor: colors.accent, borderColor: colors.accent }]}
                onPress={action.onPress}
                activeOpacity={0.7}
                testID={`filter-action-${action.key}`}
              >
                <ActionIcon size={14} color={action.active ? colors.inverseText : colors.text} />
                <Text numberOfLines={1} adjustsFontSizeToFit minimumFontScale={0.72} style={[styles.actionPillText, { color: colors.text }, action.active && styles.actionPillTextActive]}>{action.label}</Text>
