...
interface TabIdentityBandProps {
  ...
  /** Lets a nested task inherit its parent tab's palette while keeping its own story artwork. */
  artworkTab?: EasySeasTabThemeKey;
  label?: string;
  title?: string;
  subtitle?: string;
  detail?: string;
  compact?: boolean;
  /** A short identity band for utility screens where controls must remain above the fold. */
  dense?: boolean;
  textOnly?: boolean;
  testID?: string;
}
 ...
 */
export function TabIdentityBand({ tab, artworkTab, label, title, subtitle, detail, compact = false, dense = false, textOnly = false, testID }: TabIdentityBandProps) {
  const theme = TAB_VISUAL_THEMES[tab];
  const artworkKey = artworkTab ?? tab;
  const { width } = useWindowDimensions();
  const { colors, isDark, preferences, textScale } = useExperience();
  const [artworkFailed, setArtworkFailed] = useState(false);
  const bandHeight = textOnly ? 104 : dense ? (width >= 768 ? 138 : 116) : compact ? (width >= 768 ? 190 : 164) : (width >= 768 ? 232 : 204);

  useEffect(() => { setArtworkFailed(false); }, [artworkKey]);
  ...
  return (
    <View
      ...
      accessibilityLabel={`${label ?? theme.label}. ${title ?? theme.title}. ${subtitle ?? theme.subtitle}`}
    >
      {!textOnly && !artworkFailed ? (
        <Image
          source={TAB_ARTWORK[artworkKey]}
      ...
      >
        ...
        <Text style={[styles.eyebrow, { color: isDark ? colors.accentSecondary : '#0E7FA7' }, preferences.theme === 'high-contrast' && styles.highContrastText]}>{label ?? theme.label}</Text>
        <Text style={[styles.title, { color: colors.heroText }, preferences.theme === 'high-contrast' && styles.highContrastText, { fontSize: Math.min(33, 27 * textScale), lineHeight: Math.min(39, 33 * textScale) }]}>{title ?? theme.title}</Text>
        <Text style={[styles.subtitle, { color: colors.muted }, preferences.theme === 'high-contrast' && styles.highContrastSubtitle, { fontSize: Math.min(18, 14 * textScale), lineHeight: Math.min(23, 19 * textScale), maxWidth: width >= 768 ? '50%' : '68%' }]}>{subtitle ?? theme.subtitle}</Text>
        {detail ? (
...
const styles = StyleSheet.create({
  ...
  subtitle: {
    ...EASY_SEAS_TYPE_STYLES.narrative,
    color: '#58585B',
    marginTop: 8,
    maxWidth: '68%',
  },
