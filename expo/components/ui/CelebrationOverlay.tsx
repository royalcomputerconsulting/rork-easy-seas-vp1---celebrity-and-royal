...
import { View, Text, StyleSheet, Animated, Dimensions, Platform, Modal, } from 'react-native';
...
export function CelebrationOverlay({ visible, onDismiss, type = 'achievement', title, subtitle, iconType = 'trophy', autoHideDuration = 3000, }: CelebrationOverlayProps) {
    const scaleAnim = useRef(new Animated.Value(0)).current;
    const opacityAnim = useRef(new Animated.Value(0)).current;
    const iconScaleAnim = useRef(new Animated.Value(0)).current;
    const iconRotateAnim = useRef(new Animated.Value(0)).current;
    const shineAnim = useRef(new Animated.Value(0)).current;
    const particleAnims = useRef(Array(12).fill(0).map(() => ({
    ...
    useEffect(() => {
        if (visible) {
            ...
            Animated.parallel([
                ...
                Animated.loop(Animated.timing(shineAnim, {
                    toValue: 1,
                    duration: 1500,
                    useNativeDriver: true,
                })),
      ...
      <Animated.View style={[
        ...
        <Animated.View style={[
          ...
          <LinearGradient colors={getGradientColors()} start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }} style={styles.card}>
            ...
            {particleAnims.map((anim, index) => (<Animated.View key={index} style={[
                styles.particle,
                {
            ...
            <View style={styles.starsContainer}>
              {[0, 1, 2].map((i) => (<Animated.View key={i} style={{
                ...
                opacity: iconScaleAnim,
                  ...
                  <Star size={16} color="#FFD700" fill="#FFD700" style={{ marginHorizontal: 4 }}/>
                </Animated.View>))}
            </View>
          </LinearGradient>
        </Animated.View>
...
export function TierUpCelebration({ visible, onDismiss, newTier, previousTier, }: TierUpCelebrationProps) {
    return (<CelebrationOverlay visible={visible} onDismiss={onDismiss} type="tier" title={`Welcome to ${newTier}!`} subtitle={previousTier ? `Upgraded from ${previousTier}` : 'You earned a new tier!'} iconType="crown" autoHideDuration={4000}/>);
}
...
export function JackpotCelebration({ visible, onDismiss, amount, }: JackpotCelebrationProps) {
    return (<CelebrationOverlay visible={visible} onDismiss={onDismiss} type="jackpot" title="JACKPOT!" subtitle={`You won ${amount}!`} iconType="sparkles" autoHideDuration={5000}/>);
}
