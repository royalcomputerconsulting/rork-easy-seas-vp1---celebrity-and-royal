...
export function QuickActionsFAB({
  ...
  if (inline) {
    return (
      <View style={styles.inlineShell} testID="offers-quick-actions">
        <TouchableOpacity
          style={styles.inlineToggle}
          onPress={toggleExpanded}
          activeOpacity={0.8}
          accessibilityRole="button"
          accessibilityLabel={`${isExpanded ? 'Close' : 'Open'} quick actions`}
          accessibilityState={{ expanded: isExpanded }}
        >
          <View style={styles.inlineTitleGroup}>
            <Text style={styles.inlineEyebrow}>QUICK ACTIONS</Text>
            <Text style={styles.inlineTitle}>Common tasks</Text>
          </View>
          <View style={styles.inlineToggleIcon}>
            {isExpanded ? <X size={20} color={COLORS.white} /> : <Plus size={20} color={COLORS.white} />}
          </View>
        </TouchableOpacity>
        {isExpanded ? (
          <View style={styles.inlineActionGrid}>
            {actions.map((action) => (
              <TouchableOpacity
                key={action.id}
              ...
              >
                <View style={[styles.inlineActionIcon, { backgroundColor: action.color }]}>{action.icon}</View>
  ...
  return (
    <View style={styles.container} pointerEvents="box-none">
      ...
      >
        <LinearGradient
          colors={isExpanded ? ['#A52B34', '#A52B34'] : [COLORS.textNavy, COLORS.navy]}
