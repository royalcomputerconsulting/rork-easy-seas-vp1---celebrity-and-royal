...
export function QuickActionsFAB({ onBrowseCruises, onImportData, onViewCalendar, onAddCrewmember, onAddSession, }: QuickActionsFABProps) {
    ...
    return (<View style={styles.container} pointerEvents="box-none">
      ...
      <Animated.View style={[
        ...
        {actions.map((action) => (<Animated.View key={action.id} style={[
                styles.actionItem,
                {
                    transform: [
                        {
            ...
            <TouchableOpacity style={styles.actionLabelContainer} onPress={action.onPress} activeOpacity={0.8}>
              <Text style={styles.actionLabel}>{action.label}</Text>
            </TouchableOpacity>
            
            <TouchableOpacity style={[styles.actionButton, { backgroundColor: action.color }]} onPress={action.onPress} activeOpacity={0.8}>
              {renderNodeInView(action.icon)}
            </TouchableOpacity>
          </Animated.View>))}
      </Animated.View>

      <TouchableOpacity style={styles.fabButton} onPress={toggleExpanded} activeOpacity={0.9}>
        <LinearGradient colors={isExpanded ? ['#EF4444', '#DC2626'] : [COLORS.textNavy, COLORS.navy]} style={styles.fabGradient}>
          <Animated.View style={{ transform: [{ rotate: iconRotation }] }}>
            {isExpanded ? (<X size={24} color={COLORS.white}/>) : (<Plus size={24} color={COLORS.white}/>)}
          </Animated.View>
        </LinearGradient>
      </TouchableOpacity>
    </View>);
