...
function AnimatedBar({
  ...
  const animatedHeight = useRef(new Animated.Value(preferences.reducedMotion ? height : 0)).current;

  useEffect(() => {
    const animation = Animated.timing(animatedHeight, {
      toValue: height,
      duration: motionDuration(420),
      delay: preferences.reducedMotion ? 0 : delay,
      useNativeDriver: false,
    });
    animation.start();
    return () => animation.stop();
  }, [height, delay, animatedHeight, motionDuration, preferences.reducedMotion]);
...
export function PPHHistoryChart({
  ...
  const getTrendIcon = () => {
    if (trend > 5) return <TrendingUp size={14} color="#4EC0A5" />;
    if (trend < -5) return <TrendingDown size={14} color="#A52B34" />;
    return <Minus size={14} color="#676A70" />;
  };

  const getTrendColor = () => {
    if (trend > 5) return '#4EC0A5';
    if (trend < -5) return '#A52B34';
    return '#676A70';
  };

  const getBarColor = (pph: number): string => {
    if (pph >= avgPPH * 1.2) return '#4EC0A5';
    if (pph >= avgPPH) return '#3B82F6';
    if (pph >= avgPPH * 0.7) return '#E6B63D';
    return '#A52B34';
  };
    ...
    <View style={styles.container}>
      ...
      <View style={styles.content}>
        ...
        <View style={styles.legend}>
          ...
          <View style={styles.legendItem}>
            <View style={[styles.legendDot, { backgroundColor: '#A52B34' }]} />
