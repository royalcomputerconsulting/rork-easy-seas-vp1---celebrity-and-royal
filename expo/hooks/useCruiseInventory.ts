...
import {
  ...
  type CruiseInventoryIdentityQuery,
...
export function useCruiseInventory() {
  ...
  useEffect(() => {
    ...
    // Never retain another account's counts while the new owner scope opens.
    setCounts(EMPTY_COUNTS);
    setFacets(EMPTY_FACETS);
    setIsInventoryReady(false);
  ...
  const getCruiseByIdentity = useCallback(async (query: CruiseInventoryIdentityQuery) => {
    if (!ownerScopeId) return null;
    return cruiseInventoryRepository.getByIdentity(query, ownerScopeId);
  }, [ownerScopeId]);
  ...
  const queryOfferSailingSummary = useCallback(async (query: CruiseOfferSailingQuery) => {
    if (!ownerScopeId) return { total: 0, cabinTypes: [], guestCounts: [], pointRequirements: [], minimumRoomValue: null, maximumRoomValue: null, minimumNights: null, maximumNights: null };
    return cruiseInventoryRepository.queryOfferSailingSummary({ ...query, ownerScopeId });
  }, [ownerScopeId]);
  ...
  return {
    ...
    totalPhysicalSailings: counts.physicalSailings ?? 0,
    isInventoryReady,
    inventoryError,
    queryCruises,
    getCruiseById,
