...
async function resolveDataDirectory(): Promise<string> {
  const configuredDir = process.env.EASY_SEAS_DATA_DIR?.trim() || process.env.DATA_DIR?.trim() || './data';
  const resolvedDir = path.resolve(configuredDir);
  console.log('[Store] Resolving data directory:', { configuredDir, resolvedDir });
