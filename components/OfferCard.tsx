...
import { CheckCircle, ChevronRight, Clock, Ship, } from 'lucide-react-native';
...
function getOfferImage(offer: Cruise): string {
    ...
    return getUniqueImageForCruise(offer.id, offer.destination, offer.sailDate, offer.shipName);
}
export const OfferCard = React.memo(function OfferCard({ offer, allCruises = [], offerNameOverride, onPress, isBooked = false, recommended = false, showImage = true, compact = false, showValueBreakdown = false, }: OfferCardProps) {
    ...
    if (compact) {
        return (<TouchableOpacity style={[styles.compactContainer, isBooked && styles.compactBooked]} onPress={onPress} activeOpacity={0.7} testID="offer-card-compact">
    ...
    return (<TouchableOpacity style={styles.container} onPress={onPress} activeOpacity={0.9} testID="offer-card">
      <LinearGradient colors={MARBLE_TEXTURES.lightBlue.gradientColors} locations={MARBLE_TEXTURES.lightBlue.gradientLocations} start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }} style={styles.marbleBackground}>
