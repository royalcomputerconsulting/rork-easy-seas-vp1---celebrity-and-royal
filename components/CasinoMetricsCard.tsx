...
import { Clock, Calculator, TrendingUp, Target, ChevronDown, ChevronUp, AlertTriangle, Star, X, Settings, Info, DollarSign, } from 'lucide-react-native';
...
import { estimateCasinoHours, calculateTheoreticalFromPoints, calculatePredictiveScore, GAME_HOUSE_EDGES, type CasinoHoursEstimate, type TheoreticalLoss, type PredictiveScore, } from '@/lib/casinoCalculator';
...
export const CasinoMetricsCard = React.memo(function CasinoMetricsCard({ completedCruises, currentPoints, currentTier, alwaysExpanded = false, }: CasinoMetricsCardProps) {
    ...
    return (<LinearGradient colors={marbleConfig.gradientColors as unknown as [
        string,
        string,
        ...string[]
    ]} locations={marbleConfig.gradientLocations} start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }} style={styles.container}>
      <TouchableOpacity style={styles.header} onPress={alwaysExpanded ? undefined : toggleExpanded} activeOpacity={alwaysExpanded ? 1 : 0.7} disabled={alwaysExpanded}>
        <View style={styles.headerLeft}>
          <Calculator size={18} color={COLORS.navyDeep}/>
          <Text style={styles.headerTitle}>Casino Analytics</Text>
        </View>
        <View style={styles.headerRight}>
          <TouchableOpacity style={styles.settingsButton} onPress={() => setShowSettings(true)} hitSlop={{ top: 10, bottom: 10, left: 10, right: 10 }}>
      ...
      <View style={styles.metricsRow}>
        ...
        <TouchableOpacity style={styles.metricItem} onPress={() => setShowTheoDetails(true)} activeOpacity={0.7}>
          <View style={styles.metricIconRow}>
        ...
        <TouchableOpacity style={styles.metricItem} onPress={() => setShowScoreDetails(true)} activeOpacity={0.7}>
          <View style={styles.metricIconRow}>
            ...
            <ScrollView style={styles.modalScroll}>
              <View style={styles.settingSection}>
                ...
                <View style={styles.stepperRow}>
                  <TouchableOpacity style={styles.stepperButton} onPress={() => setAvgSessionHours(Math.max(1, avgSessionHours - 0.5))}>
                    <Text style={styles.stepperButtonText}>-</Text>
                  </TouchableOpacity>
                  <Text style={styles.stepperValue}>{avgSessionHours}</Text>
                  <TouchableOpacity style={styles.stepperButton} onPress={() => setAvgSessionHours(Math.min(12, avgSessionHours + 0.5))}>
                    <Text style={styles.stepperButtonText}>+</Text>
                  </TouchableOpacity>
                </View>
              </View>
              ...
              <View style={styles.settingSection}>
                ...
                <View style={styles.toggleRow}>
                  <TouchableOpacity style={[styles.toggleOption, playOnPortDays && styles.toggleOptionActive]} onPress={() => setPlayOnPortDays(true)}>
                    <Text style={[styles.toggleText, playOnPortDays && styles.toggleTextActive]}>Yes</Text>
                  </TouchableOpacity>
                  <TouchableOpacity style={[styles.toggleOption, !playOnPortDays && styles.toggleOptionActive]} onPress={() => setPlayOnPortDays(false)}>
              ...
              <View style={styles.settingSection}>
                ...
                <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.gameSelector}>
                  {Object.keys(GAME_HOUSE_EDGES).map((game) => (<TouchableOpacity key={game} style={[styles.gameOption, primaryGame === game && styles.gameOptionActive]} onPress={() => setPrimaryGame(game)}>
              ...
              <View style={styles.settingSection}>
                ...
                <View style={styles.stepperRow}>
                  <TouchableOpacity style={styles.stepperButton} onPress={() => setAvgBetSize(Math.max(5, avgBetSize - 5))}>
                    <Text style={styles.stepperButtonText}>-</Text>
                  </TouchableOpacity>
                  <TextInput style={styles.betInput} value={avgBetSize.toString()} onChangeText={(text) => setAvgBetSize(Math.max(1, parseInt(text) || 1))} keyboardType="numeric"/>
                  <TouchableOpacity style={styles.stepperButton} onPress={() => setAvgBetSize(avgBetSize + 5)}>
                    <Text style={styles.stepperButtonText}>+</Text>
                  </TouchableOpacity>
                </View>
              </View>
      ...
      <Modal visible={showTheoDetails} animationType="fade" transparent={true} onRequestClose={() => setShowTheoDetails(false)}>
        <TouchableOpacity style={styles.modalOverlay} activeOpacity={1} onPress={() => setShowTheoDetails(false)}>
      ...
      <Modal visible={showScoreDetails} animationType="fade" transparent={true} onRequestClose={() => setShowScoreDetails(false)}>
        <TouchableOpacity style={styles.modalOverlay} activeOpacity={1} onPress={() => setShowScoreDetails(false)}>
          <View style={styles.popoverContent}>
