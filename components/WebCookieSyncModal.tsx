import { useState } from 'react';
import { View, Text, StyleSheet, Pressable, Modal, TextInput, ActivityIndicator, ScrollView } from 'react-native';
import { X, Cookie, AlertCircle, HelpCircle } from 'lucide-react-native';
interface WebCookieSyncModalProps {
    visible: boolean;
    onClose: () => void;
    onSubmit: (cookies: string) => Promise<void>;
    cruiseLine: 'royal_caribbean' | 'celebrity' | 'carnival';
    isLoading: boolean;
    error: string | null;
}
export function WebCookieSyncModal({ visible, onClose, onSubmit, cruiseLine, isLoading, error }: WebCookieSyncModalProps) {
    const [cookies, setCookies] = useState('');
    const [showInstructions, setShowInstructions] = useState(false);
    const isCelebrity = cruiseLine === 'celebrity';
    const isCarnival = cruiseLine === 'carnival';
    const brandName = isCelebrity ? 'Celebrity Cruises' : isCarnival ? 'Carnival Cruise Line' : 'Royal Caribbean';
    const websiteUrl = isCelebrity
        ? 'https://www.celebritycruises.com/blue-chip-club/offers'
        : isCarnival
            ? 'https://www.carnival.com/cruise-deals'
            : 'https://www.royalcaribbean.com/club-royale/offers';
    const handleSubmit = async () => {
        if (!cookies.trim())
            return;
        await onSubmit(cookies.trim());
    };
    const handleClose = () => {
        setCookies('');
        setShowInstructions(false);
        onClose();
    };
    return (<Modal visible={visible} transparent animationType="fade" onRequestClose={handleClose}>
      <View style={styles.overlay}>
        <View style={styles.modal}>
          <Pressable style={styles.closeButton} onPress={handleClose}>
            <X size={24} color="#94a3b8"/>
          </Pressable>

          <ScrollView style={styles.scrollView} contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={true}>
            <View style={styles.header}>
              <View style={[styles.iconContainer, isCelebrity && styles.iconContainerCelebrity, isCarnival && styles.iconContainerCarnival]}>
                <Cookie size={32} color={isCelebrity ? '#10b981' : isCarnival ? '#CC2232' : '#3b82f6'}/>
              </View>
              <Text style={styles.title}>Cookie-Based Sync</Text>
              <Text style={styles.subtitle}>
                Paste your {brandName} cookies to sync your data
              </Text>
            </View>

            <Pressable style={styles.instructionsToggle} onPress={() => setShowInstructions(!showInstructions)}>
              <HelpCircle size={16} color="#60a5fa"/>
              <Text style={styles.instructionsToggleText}>
                {showInstructions ? 'Hide' : 'Show'} Instructions
              </Text>
            </Pressable>

            

            <View style={styles.form}>
              <View style={styles.inputGroup}>
                <Text style={styles.label}>Cookies</Text>
                <TextInput style={styles.textArea} value={cookies} onChangeText={setCookies} placeholder="Paste your cookies here (cookie1=value1; cookie2=value2; ...)" placeholderTextColor="#475569" multiline numberOfLines={6} editable={!isLoading} textAlignVertical="top"/>
              </View>

              

              <View style={styles.securityNote}>
                <Cookie size={14} color="#64748b"/>
                <Text style={styles.securityText}>
                  Your cookies are used only for this sync session and are not stored on our servers.
                </Text>
              </View>
            </View>

            <View style={styles.actions}>
              <Pressable style={styles.cancelButton} onPress={handleClose} disabled={isLoading}>
                <Text style={styles.cancelButtonText}>Cancel</Text>
              </Pressable>

              <Pressable style={[
            styles.submitButton,
            isCelebrity && styles.submitButtonCelebrity,
            isCarnival && styles.submitButtonCarnival,
            (!cookies.trim() || isLoading) && styles.submitButtonDisabled
        ]} onPress={handleSubmit} disabled={!cookies.trim() || isLoading}>
                {isLoading ? (<ActivityIndicator size="small" color="#fff"/>) : (<Text style={styles.submitButtonText}>Sync with Cookies</Text>)}
              </Pressable>
            </View>
          </ScrollView>
        </View>
      </View>
    </Modal>);
}
const styles = StyleSheet.create({
    overlay: {
        flex: 1,
        backgroundColor: 'rgba(0, 0, 0, 0.85)',
        justifyContent: 'center',
        alignItems: 'center',
        padding: 20
    },
    modal: {
        backgroundColor: '#1e293b',
        borderRadius: 16,
        width: '100%',
        maxWidth: 600,
        maxHeight: '90%',
        borderWidth: 1,
        borderColor: '#334155'
    },
    scrollView: {
        maxHeight: '100%'
    },
    scrollContent: {
        padding: 24
    },
    closeButton: {
        position: 'absolute',
        top: 16,
        right: 16,
        zIndex: 1,
        padding: 4
    },
    header: {
        alignItems: 'center',
        marginBottom: 20
    },
    iconContainer: {
        width: 64,
        height: 64,
        borderRadius: 32,
        backgroundColor: '#1e40af20',
        alignItems: 'center',
        justifyContent: 'center',
        marginBottom: 16
    },
    iconContainerCelebrity: {
        backgroundColor: '#05966920'
    },
    iconContainerCarnival: {
        backgroundColor: '#CC223220'
    },
    submitButtonCarnival: {
        backgroundColor: '#CC2232'
    },
    title: {
        color: '#fff',
        fontSize: 22,
        fontWeight: '700' as const,
        marginBottom: 8
    },
    subtitle: {
        color: '#94a3b8',
        fontSize: 14,
        textAlign: 'center',
        lineHeight: 20
    },
    instructionsToggle: {
        flexDirection: 'row',
        alignItems: 'center',
        justifyContent: 'center',
        gap: 6,
        paddingVertical: 10,
        marginBottom: 16
    },
    instructionsToggleText: {
        color: '#60a5fa',
        fontSize: 14,
        fontWeight: '500' as const
    },
    instructionsContainer: {
        backgroundColor: '#0f172a',
        borderRadius: 12,
        padding: 16,
        marginBottom: 20,
        borderWidth: 1,
        borderColor: '#334155'
    },
    instructionsTitle: {
        color: '#e2e8f0',
        fontSize: 15,
        fontWeight: '600' as const,
        marginBottom: 12
    },
    instructionsList: {
        gap: 8
    },
    instructionStep: {
        color: '#cbd5e1',
        fontSize: 13,
        lineHeight: 18
    },
    instructionExample: {
        color: '#94a3b8',
        fontSize: 12,
        fontFamily: 'monospace',
        backgroundColor: '#1e293b',
        padding: 8,
        borderRadius: 6,
        marginTop: 4
    },
    warningBox: {
        flexDirection: 'row',
        alignItems: 'flex-start',
        gap: 8,
        backgroundColor: '#78350f40',
        padding: 10,
        borderRadius: 8,
        marginTop: 12,
        borderWidth: 1,
        borderColor: '#f59e0b'
    },
    warningText: {
        flex: 1,
        color: '#fbbf24',
        fontSize: 12,
        lineHeight: 16
    },
    form: {
        gap: 16,
        marginBottom: 24
    },
    inputGroup: {
        gap: 8
    },
    label: {
        color: '#cbd5e1',
        fontSize: 14,
        fontWeight: '500' as const
    },
    textArea: {
        backgroundColor: '#0f172a',
        borderRadius: 10,
        borderWidth: 1,
        borderColor: '#334155',
        padding: 12,
        color: '#fff',
        fontSize: 13,
        fontFamily: 'monospace',
        minHeight: 120
    },
    errorContainer: {
        flexDirection: 'row',
        alignItems: 'flex-start',
        gap: 8,
        backgroundColor: '#7f1d1d40',
        padding: 12,
        borderRadius: 8,
        borderWidth: 1,
        borderColor: '#ef4444'
    },
    errorText: {
        flex: 1,
        color: '#fca5a5',
        fontSize: 13,
        lineHeight: 18
    },
    securityNote: {
        flexDirection: 'row',
        alignItems: 'flex-start',
        gap: 8,
        paddingTop: 8
    },
    securityText: {
        flex: 1,
        color: '#64748b',
        fontSize: 12,
        lineHeight: 16
    },
    actions: {
        flexDirection: 'row',
        gap: 12
    },
    cancelButton: {
        flex: 1,
        paddingVertical: 14,
        borderRadius: 10,
        backgroundColor: '#334155',
        alignItems: 'center',
        justifyContent: 'center'
    },
    cancelButtonText: {
        color: '#cbd5e1',
        fontSize: 15,
        fontWeight: '600' as const
    },
    submitButton: {
        flex: 1.5,
        paddingVertical: 14,
        borderRadius: 10,
        backgroundColor: '#3b82f6',
        alignItems: 'center',
        justifyContent: 'center'
    },
    submitButtonCelebrity: {
        backgroundColor: '#059669'
    },
    submitButtonDisabled: {
        opacity: 0.5
    },
    submitButtonText: {
        color: '#fff',
        fontSize: 15,
        fontWeight: '600' as const
    }
});
