...
import { TrendingUp, TrendingDown, Zap, Clock, Award, Target, Trophy, Dices, } from 'lucide-react-native';
...
export const SessionsSummaryCard = React.memo(function SessionsSummaryCard({ analytics, sessions, targetPPH, }: SessionsSummaryCardProps) {
    ...
    const pphStatus = useMemo(() => {
        ...
        if (percentage >= 120)
            return { status: 'Exceptional', color: '#10B981' };
        if (percentage >= 100)
            return { status: 'On Target', color: '#3B82F6' };
        if (percentage >= 70)
            return { status: 'Close', color: '#F59E0B' };
