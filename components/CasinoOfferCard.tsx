...
import { X, ExternalLink, ChevronRight, Clock, Sparkles, Ship, DollarSign, Tag, } from 'lucide-react-native';
...
export const OfferSummaryCard = React.memo(function OfferSummaryCard({ totalValue, totalCruises, totalOffers, onSoonestPress, onHighestValuePress, activeSortMode = 'soonest', }: OfferSummaryCardProps) {
    return (<View style={summaryStyles.container}>
      <ImageBackground source={{ uri: JACKPOT_BG }} style={summaryStyles.backgroundImage} resizeMode="cover">
        <LinearGradient colors={['rgba(0, 151, 167, 0.9)', 'rgba(30, 58, 95, 0.92)']} start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }} style={summaryStyles.gradientOverlay}>
        ...
        ]} onPress={onSoonestPress} activeOpacity={0.8}>
              <Text style={[
        ...
        ]} onPress={onHighestValuePress} activeOpacity={0.8}>
              <Text style={[
            summaryStyles.filterButtonText,
...
export const CasinoOfferCard = React.memo(function CasinoOfferCard({ offerCode, offerName, expiryDate, tradeInValue, freePlay, obc, cruises, onPress, onCruisePress, bookedCruiseIds = new Set(), isActive = true, isBestValue = false, offerSource, }: CasinoOfferCardProps) {
    ...
    const offerImageUrl = useMemo(() => {
        if (cruises.length > 0) {
            ...
            return getUniqueImageForCruise(firstCruise.id, firstCruise.destination, firstCruise.sailDate, firstCruise.shipName);
        }
        return DEFAULT_CRUISE_IMAGE;
    }, [cruises]);
    const [cardImageUri, setCardImageUri] = useState<string>(offerImageUrl);
    const offerDetails = useMemo(() => {
        const offer = (localData.offers || []).find((o: CasinoOffer) => o.offerCode === offerCode);
    ...
    return (<TouchableOpacity style={styles.container} onPress={onPress} activeOpacity={0.9} testID="casino-offer-card">
      ...
      <Modal visible={showOfferImage} transparent={true} animationType="fade" onRequestClose={() => setShowOfferImage(false)}>
        <View style={styles.modalOverlay}>
          <View style={styles.modalContent}>
            <View style={styles.modalHeader}>
              <Text style={styles.modalTitle}>Offer: {offerCode}</Text>
              <TouchableOpacity style={styles.modalCloseButton} onPress={() => setShowOfferImage(false)}>
                <X size={24} color="#6B7280"/>
              </TouchableOpacity>
            </View>
            
            <View style={styles.modalImageContainer}>
              {imageError ? (<View style={styles.imageErrorContainer}>
                ...
                </View>) : (<Image source={{ uri: getActualOfferImageUrl(offerCode) }} style={styles.modalImage} resizeMode="contain" onError={() => setImageError(true)}/>)}
            </View>
            
            <View style={styles.modalActions}>
              <TouchableOpacity style={styles.openInBrowserButton} onPress={handleOpenInBrowser}>
                <ExternalLink size={16} color={COLORS.navyDeep}/>
                <Text style={styles.openInBrowserText}>Open in Browser</Text>
              </TouchableOpacity>
              
              <TouchableOpacity style={styles.closeModalButton} onPress={() => setShowOfferImage(false)}>
