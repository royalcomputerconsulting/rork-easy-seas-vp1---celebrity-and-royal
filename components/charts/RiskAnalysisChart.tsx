...
import { Shield, AlertTriangle, CheckCircle, XCircle, Minus, Lightbulb, Activity, TrendingUp, TrendingDown, DollarSign, Ship, Coins } from 'lucide-react-native';
...
export function RiskAnalysisChart({ riskAnalysis, title = 'Financial Health Analysis', totalSpent = 0, totalRetailValue = 0, totalSavings = 0, casinoNetResult = 0, totalCruises = 0, pointsEarned = 0, }: RiskAnalysisChartProps) {
    const healthScore = useMemo(() => {
        let score = 50;
        if (totalSavings > 0)
            score += Math.min(25, (totalSavings / 1000) * 5);
        if (totalSavings < 0)
            score -= Math.min(25, (Math.abs(totalSavings) / 1000) * 5);
        if (casinoNetResult > 0)
            score += Math.min(15, (casinoNetResult / 500) * 5);
        if (casinoNetResult < 0)
            score -= Math.min(15, (Math.abs(casinoNetResult) / 500) * 2);
        if (totalCruises >= 5)
            score += 10;
        if (totalCruises >= 10)
            score += 5;
    ...
    return (<View style={styles.container}>
      ...
      <View style={styles.content}>
        ...
        <View style={styles.metricsRow}>
          {financialMetrics.map((metric, index) => (<View key={index} style={styles.metricCard}>
        ...
        <View style={styles.factorsSection}>
          ...
          <View style={styles.factorsList}>
            ...
            return (<View key={index} style={styles.factorItem}>
