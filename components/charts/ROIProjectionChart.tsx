...
export function ROIProjectionChart({ roiProjection, comparisonROI, title = 'Spending vs Savings Analysis', totalSpent, totalRetailValue, totalPointsEarned, }: ROIProjectionChartProps) {
    ...
    const roiTrend = useMemo(() => {
        if (comparisonROI === undefined)
            return 'neutral';
        if (roiProjection.projectedROI > comparisonROI)
            return 'up';
        if (roiProjection.projectedROI < comparisonROI)
            return 'down';
        return 'neutral';
    ...
    const roiColor = useMemo(() => {
        if (roiProjection.projectedROI >= 50)
            return COLORS.success;
        if (roiProjection.projectedROI >= 20)
            return COLORS.warning;
        if (roiProjection.projectedROI >= 0)
            return COLORS.aquaAccent;
        return COLORS.error;
    }, [roiProjection.projectedROI]);
    const barData = useMemo(() => {
        const maxValue = Math.max(actualSpent, totalValueWithPoints, 1);
        return {
    ...
    const valueBreakdown = useMemo(() => {
        ...
        if (total === 0)
            return { spent: 50, savings: 30, points: 20 };
        return {
        ...
        <View style={styles.chartSection}>
          <View style={styles.barChartContainer}>
            ...
            <View style={styles.barRow}>
              ...
              <View style={styles.barWrapper}>
                <LinearGradient colors={[COLORS.success, COLORS.aquaAccent]} start={{ x: 0, y: 0 }} end={{ x: 1, y: 0 }} style={[
            styles.bar,
