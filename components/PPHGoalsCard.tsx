...
import { Target, TrendingUp, TrendingDown, Calendar, Clock, Award, ChevronRight, Edit3, Check, BarChart3, Zap, } from 'lucide-react-native';
...
export const PPHGoalsCard = React.memo(function PPHGoalsCard({ analytics, sessions, targetPPH, onTargetChange, }: PPHGoalsCardProps) {
    ...
    const periodStats = useMemo((): Record<TimePeriod, PeriodStats> => {
        ...
        const calculateStats = (filtered: CasinoSession[], compareSessions: CasinoSession[], label: string, period: TimePeriod): PeriodStats => {
    ...
    return (<View style={styles.container}>
      ...
      <View style={styles.content}>
        <View style={styles.periodTabs}>
          {(['today', 'week', 'month', 'all'] as TimePeriod[]).map((period) => (<TouchableOpacity key={period} style={[
                styles.periodTab,
                selectedPeriod === period && styles.periodTabActive,
            ]} onPress={() => handleSelectPeriod(period)} activeOpacity={0.7}>
              <Text style={[
        ...
        <View style={styles.mainStatsContainer}>
          <Animated.View style={[
            ...
            <View style={[styles.statusBadge, { backgroundColor: `${getStatusColor(currentStats.pointsPerHour)}15` }]}>
              {currentStats.pointsPerHour >= targetPPH ? (<TrendingUp size={14} color={getStatusColor(currentStats.pointsPerHour)}/>) : (<TrendingDown size={14} color={getStatusColor(currentStats.pointsPerHour)}/>)}
          ...
          <View style={styles.targetCard}>
            <View style={styles.targetHeader}>
              ...
              {isEditingTarget ? (<TouchableOpacity onPress={handleSaveTarget} activeOpacity={0.7}>
                  <Check size={16} color="#10B981"/>
                </TouchableOpacity>) : (<TouchableOpacity onPress={handleEditTarget} activeOpacity={0.7}>
                  <Edit3 size={14} color="#6B7280"/>
                </TouchableOpacity>)}
            </View>
            {isEditingTarget ? (<TextInput style={styles.targetInput} value={tempTarget} onChangeText={setTempTarget} keyboardType="number-pad" autoFocus selectTextOnFocus/>) : (<Text style={styles.targetValue}>{targetPPH}</Text>)}
        ...
        <View style={styles.comparisonSection}>
          ...
          {(['today', 'week', 'month', 'all'] as TimePeriod[]).map((period) => {
            ...
            return (<TouchableOpacity key={period} style={[
