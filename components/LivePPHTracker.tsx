...
import { Zap, Play, Pause, RotateCcw, Target, TrendingUp, TrendingDown, Clock, Award, AlertTriangle, ChevronUp, ChevronDown, } from 'lucide-react-native';
...
export function LivePPHTracker({ targetPPH = 100, onSessionComplete, historicalAvgPPH = 0, compact = false, }: LivePPHTrackerProps) {
    ...
    useEffect(() => {
        if (!isTracking || elapsedMinutes < 5)
            return;
    ...
    const getPPHColor = useCallback((pph: number): string => {
        if (pph >= targetPPH * 1.2)
            return '#10B981';
        if (pph >= targetPPH)
            return '#3B82F6';
        if (pph >= targetPPH * 0.7)
            return '#F59E0B';
        return '#EF4444';
    }, [targetPPH]);
    const getPPHStatus = useCallback((pph: number): string => {
        if (pph >= targetPPH * 1.5)
            return 'Exceptional';
        if (pph >= targetPPH * 1.2)
            return 'Excellent';
        if (pph >= targetPPH)
    ...
    if (compact) {
        ...
        <TouchableOpacity style={[styles.compactButton, isTracking && styles.compactButtonActive]} onPress={isTracking ? handlePause : handleStart} activeOpacity={0.7}>
          {isTracking ? (<Pause size={14} color="#FFFFFF"/>) : (<Play size={14} color="#10B981"/>)}
        </TouchableOpacity>
      </View>);
    }
    return (<View style={styles.container}>
      <LinearGradient colors={['#F3E8FF', '#F3E8FF']} style={styles.header}>
      ...
      <View style={styles.content}>
        ...
        <Animated.View style={[
          ...
          <View style={[styles.statusBadge, { backgroundColor: `${getPPHColor(currentPPH)}15` }]}>
            {currentPPH >= targetPPH ? (<TrendingUp size={14} color={getPPHColor(currentPPH)}/>) : (<TrendingDown size={14} color={getPPHColor(currentPPH)}/>)}
        ...
        <View style={styles.pointsSection}>
          ...
          <View style={styles.pointsRow}>
            <TouchableOpacity style={styles.pointsButton} onPress={() => handleIncrementPoints(-10)} activeOpacity={0.7}>
          ...
          <View style={styles.quickPointsRow}>
            {[25, 50, 100, 250].map(amount => (<TouchableOpacity key={amount} style={styles.quickPointsButton} onPress={() => handleIncrementPoints(amount)} activeOpacity={0.7}>
        ...
        <View style={styles.controlsRow}>
          {!isTracking && elapsedSeconds === 0 ? (<TouchableOpacity style={styles.startButton} onPress={handleStart} activeOpacity={0.8}>
            ...
            ]} onPress={isTracking ? handlePause : handleStart} activeOpacity={0.8}>
                {isTracking ? (<>
