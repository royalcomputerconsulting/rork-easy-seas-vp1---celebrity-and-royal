...
import { Brain, TrendingUp, TrendingDown, Target, Flame, Snowflake, Award, Clock, BarChart3, Zap, AlertTriangle, CheckCircle, Activity, } from 'lucide-react-native';
...
function HouseEdgeSection({ analytics, cruiseMetrics }: {
    analytics: SessionAnalytics;
    cruiseMetrics?: CruiseBasedMetrics;
}) {
...
function VarianceSection({ analytics }: {
    analytics: SessionAnalytics;
}) {
...
function WinRateSection({ analytics }: {
    analytics: SessionAnalytics;
}) {
...
function PointsEfficiencySection({ analytics, cruiseMetrics }: {
    analytics: SessionAnalytics;
    cruiseMetrics?: CruiseBasedMetrics;
}) {
...
export function CasinoIntelligenceCard({ analytics, onViewDetails, completedCruises }: CasinoIntelligenceCardProps) {
    const cruiseMetrics = useMemo((): CruiseBasedMetrics | undefined => {
        if (!completedCruises || completedCruises.length === 0)
            return undefined;
        ...
        const totalTheoreticalLoss = cruisesWithData.reduce((sum, c) => {
            if (c.theoreticalLoss)
                return sum + c.theoreticalLoss;
    ...
    if (analytics.totalSessions === 0 && !cruiseMetrics) {
        ...
        <LinearGradient colors={['#8B5CF6', '#6366F1']} style={styles.emptyHeader}>
