...
import { X, Plus, Trash2, Edit3, Save, CircleDollarSign, Ticket, Gift, Award, Check, Sparkles, } from 'lucide-react-native';
...
export function CertificateManagerModal({ visible, onClose, certificates, onAddCertificate, onUpdateCertificate, onDeleteCertificate, }: CertificateManagerModalProps) {
    ...
    const renderCertificateItem = (cert: Certificate) => {
        ...
        if (isEditing) {
            return (<View key={cert.id} style={styles.editForm}>
          <View style={styles.typeSelector}>
            {CERTIFICATE_TYPES.map((typeOption) => {
                    const TypeIcon = typeOption.icon;
                    return (<TouchableOpacity key={typeOption.type} style={[
                            styles.typeOption,
                            newType === typeOption.type && styles.typeOptionSelected,
                        ]} onPress={() => setNewType(typeOption.type)} activeOpacity={0.7}>
                  <TypeIcon size={16} color={newType === typeOption.type ? COLORS.navyDeep : COLORS.beigeWarm}/>
                </TouchableOpacity>);
                })}
          </View>

          <TextInput style={styles.input} placeholder="Label (optional)" placeholderTextColor="rgba(255,255,255,0.4)" value={newLabel} onChangeText={setNewLabel}/>

          <TextInput style={styles.input} placeholder="Value ($)" placeholderTextColor="rgba(255,255,255,0.4)" value={newValue} onChangeText={setNewValue} keyboardType="numeric"/>

          <TextInput style={styles.input} placeholder="Description (optional)" placeholderTextColor="rgba(255,255,255,0.4)" value={newDescription} onChangeText={setNewDescription}/>

          <TextInput style={styles.input} placeholder="Expiry Date (MM-DD-YYYY)" placeholderTextColor="rgba(255,255,255,0.4)" value={newExpiry} onChangeText={setNewExpiry}/>

          <View style={styles.editActions}>
            <TouchableOpacity style={styles.cancelButton} onPress={resetForm} activeOpacity={0.7}>
        ...
        return (<View key={cert.id} style={[
                styles.certItem,
                cert.status === 'used' && styles.certItemUsed
            ]}>
        <TouchableOpacity style={styles.statusToggle} onPress={() => handleToggleStatus(cert)} activeOpacity={0.7}>
          <View style={[
        ...
        <View style={styles.certActions}>
          <TouchableOpacity style={styles.actionButton} onPress={() => handleEdit(cert)} activeOpacity={0.7}>
            <Edit3 size={16} color={COLORS.beigeWarm}/>
          </TouchableOpacity>
          <TouchableOpacity style={styles.actionButton} onPress={() => handleDelete(cert.id)} activeOpacity={0.7}>
    ...
    return (<Modal visible={visible} animationType="slide" presentationStyle="pageSheet" onRequestClose={onClose}>
      <View style={styles.container}>
        <LinearGradient colors={['rgba(0, 31, 63, 0.98)', 'rgba(0, 61, 92, 0.95)']} style={StyleSheet.absoluteFill}/>
        ...
        <ScrollView style={styles.scrollView} contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
          ...
          {certificates.length === 0 ? (<View style={styles.emptyState}>
            ...
            </View>) : (<View style={styles.certList}>
