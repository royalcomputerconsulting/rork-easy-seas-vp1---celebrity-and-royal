...
export function StatCard({ title, value, subtitle, icon: Icon, iconColor, trend, trendValue, onPress, compact = false, highlight = false, statType = 'default', }: StatCardProps) {
    const getStatColor = () => {
        switch (statType) {
            case 'money':
                return COLORS.money;
    ...
    const content = (<LinearGradient colors={marbleConfig.gradientColors as unknown as [
        string,
        string,
        ...string[]
    ]} locations={marbleConfig.gradientLocations} start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }} style={[styles.container, compact && styles.containerCompact, highlight && styles.containerHighlight]}>
