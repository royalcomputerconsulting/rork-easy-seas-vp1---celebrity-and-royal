...
export function SurveyListModal({ visible, onClose, sailings }: SurveyListModalProps) {
    ...
    const surveyListQuery = trpc.crewRecognition.getSurveyList.useQuery({ sailingId: selectedSailingId, userId }, {
    ...
    const localSurveyList = useMemo<SurveyListItem[]>(() => {
        if (!isOfflineMode || !selectedSailingId || !selectedSailing)
            return [];
        const sailingEntries = allEntries.filter(e => {
            if (e.sailingId === selectedSailingId)
                return true;
            if (e.shipName === selectedSailing.shipName && e.sailStartDate === selectedSailing.sailStartDate)
                return true;
            return false;
        });
    ...
    const handleExport = () => {
        ...
        exportSurveyToText(selectedSailing.shipName, selectedSailing.sailStartDate, surveyList, filename);
    };
        ...
        <View style={styles.modal}>
          ...
          <View style={styles.content}>
            <View style={styles.selector}>
              ...
              <TouchableOpacity style={styles.selectorPicker} onPress={() => setShowSailingPicker(!showSailingPicker)}>
                <Text style={selectedSailingId
