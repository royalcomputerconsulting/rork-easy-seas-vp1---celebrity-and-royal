...
export const CrewRecognitionSection = React.memo(function CrewRecognitionSection() {
    const { userId, stats, statsLoading, entries, entriesTotal, entriesLoading, sailings, filters, updateFilters, resetFilters, createCrewMember, updateRecognitionEntry, deleteRecognitionEntry, syncFromCSVLocally, } = useCrewRecognition();
    ...
    const uniqueShips = useMemo(() => Array.from(new Set([...allRoyalShips, ...sailingShips])).sort(), [allRoyalShips, sailingShips]);
    ...
    return (<View style={styles.container}>
      ...
      <View style={styles.actionButtons}>
        ...
        <TouchableOpacity style={[styles.syncButton, isSyncing && styles.syncButtonDisabled]} onPress={handleSync} disabled={isSyncing}>
          {isSyncing ? (<ActivityIndicator size="small" color="#0369A1"/>) : (<RefreshCcw size={18} color="#0369A1"/>)}
          <Text style={styles.syncButtonText}>{syncButtonLabel}</Text>
        </TouchableOpacity>
      </View>

      {syncProgress ? (<View style={styles.syncProgressContainer}>
      ...
      <View style={styles.filterBar}>
        <View style={styles.searchContainer}>
          ...
          <TextInput style={styles.searchInput} value={filters.search} onChangeText={text => updateFilters({ search: text })} placeholder="Search crew name..." placeholderTextColor={COLORS.textTertiary}/>
          {filters.search !== '' ? (<TouchableOpacity onPress={() => updateFilters({ search: '' })}>
      ...
      <View style={styles.resultsContainer}>
        ...
        {entriesLoading ? (<View style={styles.loadingContainer}>
            <ActivityIndicator size="large" color={COLORS.primary}/>
          </View>) : showMockData ? (<View style={styles.mockDataContainer}>
          ...
          </View>) : displayEntries.length === 0 ? (<View style={styles.emptyContainer}>
            ...
            <Text style={styles.emptySubtext}>
              ...
              {displayEntries.map((entry) => (<TouchableOpacity key={entry.id} style={styles.crewCard} onPress={() => setSelectedEntry(entry)} activeOpacity={0.7}>
                  ...
                  <View style={styles.crewCardContent}>
                    <View style={styles.crewCardRow1}>
                      <View style={styles.crewCardNameBlock}>
                        ...
                        <View style={styles.crewCardMetaInline}>
                          {entry.roleTitle ? (<Text style={styles.crewCardRole} numberOfLines={1}>{entry.roleTitle}</Text>) : null}
                          {entry.roleTitle && entry.department ? (<Text style={styles.crewCardMetaDot}>{"·"}</Text>) : null}
                          {entry.department ? (<Text style={styles.crewCardDept} numberOfLines={1}>{entry.department}</Text>) : null}
                        </View>
                      </View>
                    ...
                    {entry.crewNotes && String(entry.crewNotes).trim() ? (<Text style={styles.crewCardNotes} numberOfLines={1}>{String(entry.crewNotes)}</Text>) : null}
                  </View>
                </TouchableOpacity>))}
            </View>)}
      </View>

      <AddCrewMemberModal visible={showAddModal} onClose={() => setShowAddModal(false)} onSubmit={async (data) => {
            await createCrewMember({ ...data, department: data.department as Department, userId });
        }} sailings={sailings} bookedCruises={bookedCruises}/>

      <RecognitionEntryDetailModal visible={!!selectedEntry} entry={selectedEntry} sailings={sailings} onClose={() => setSelectedEntry(null)} onUpdate={async (id, data) => {
            await updateRecognitionEntry({ id, ...data });
        }} onDelete={async (id) => {
            await deleteRecognitionEntry({ id });
        }}/>

      <SurveyListModal visible={showSurveyModal} onClose={() => setShowSurveyModal(false)} sailings={sailings}/>
    </View>);
});
