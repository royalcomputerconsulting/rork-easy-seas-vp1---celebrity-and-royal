...
import { Send, Bot, User, Sparkles, Ship, TrendingUp, Award, Gift, X, Maximize2, Minimize2, } from 'lucide-react-native';
...
export const AgentXChat = React.memo(function AgentXChat({ messages, onSendMessage, isLoading = false, isExpanded = false, onToggleExpand, onClose, showHeader = true, placeholder = 'Ask about cruises, tier progress, offers...', }: AgentXChatProps) {
    ...
    const renderMessage = useCallback((message: ChatMessage, index: number) => {
        ...
        return (<Animated.View key={message.id} style={[
        ...
        <View style={[styles.messageBubble, isUser ? styles.userBubble : styles.assistantBubble]}>
          ...
          <View style={[styles.messageContent, isUser && styles.userMessageContent]}>
            {message.isLoading ? (<View style={styles.loadingContainer}>
                <ActivityIndicator size="small" color={COLORS.navyDeep}/>
                <Text style={styles.loadingText}>
                  {message.toolName ? `Running ${message.toolName}...` : 'Thinking...'}
                </Text>
              </View>) : (<ScrollView style={styles.messageScrollView} nestedScrollEnabled={true} showsVerticalScrollIndicator={true}>
    ...
    const renderWelcome = () => (<View style={styles.welcomeContainer}>
      <View style={styles.welcomeIconContainer}>
        <LinearGradient colors={[COLORS.goldAccent, COLORS.beigeWarm]} style={styles.welcomeIconGradient}>
      ...
      <View style={styles.quickActionsContainer}>
        ...
        <View style={styles.quickActionsGrid}>
          {QUICK_ACTIONS.map((action) => (<TouchableOpacity key={action.id} style={styles.quickActionButton} onPress={() => handleQuickAction(action.prompt)} activeOpacity={0.7}>
    ...
    return (<KeyboardAvoidingView style={[styles.container, isExpanded && styles.containerExpanded]} behavior={Platform.OS === 'ios' ? 'padding' : 'height'} keyboardVerticalOffset={Platform.OS === 'ios' ? 0 : 0}>
      <LinearGradient colors={['#E0F2FE', '#DBEAFE', '#E0F7FA']} start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }} style={StyleSheet.absoluteFill}/>
      ...
      <View style={[styles.inputContainer, { paddingBottom: insets.bottom > 0 ? insets.bottom : SPACING.md }]}>
        <View style={styles.inputWrapper}>
          <TextInput ref={inputRef} style={styles.textInput} value={inputText} onChangeText={setInputText} placeholder={placeholder} placeholderTextColor={COLORS.textSecondary} multiline maxLength={1000} editable={!isLoading} onSubmitEditing={handleSend} returnKeyType="send"/>
          
          <TouchableOpacity style={[
            styles.sendButton,
            (!inputText.trim() || isLoading) && styles.sendButtonDisabled,
        ]} onPress={handleSend} disabled={!inputText.trim() || isLoading} activeOpacity={0.7}>
            {isLoading ? (<ActivityIndicator size="small" color={COLORS.white}/>) : (<Send size={20} color={inputText.trim() ? COLORS.white : 'rgba(255,255,255,0.5)'}/>)}
          </TouchableOpacity>
        </View>
