...
import { View, Text, StyleSheet, TouchableOpacity, Animated, Platform, TextInput, ScrollView, } from 'react-native';
import { Target, Clock, Zap, Award, CheckCircle, Check, Calendar, TrendingUp, Flame, Coins, ChevronRight, Star, Trophy, } from 'lucide-react-native';
...
function CircularProgress({ progress, size = 80, strokeWidth = 8, color, backgroundColor = 'rgba(0,0,0,0.08)', children, }: {
    progress: number;
    size?: number;
    strokeWidth?: number;
    color: string;
    ...
    return (<View style={{ width: size, height: size, justifyContent: 'center', alignItems: 'center' }}>
...
function GoalProgressRing({ goal, isEditing, onEdit, onSave, editValue, onEditChange, }: {
    goal: WeeklyGoal;
    isEditing: boolean;
    onEdit: () => void;
    onSave: () => void;
    ...
    return (<Animated.View style={[styles.goalRingContainer, { transform: [{ scale: pulseAnim }] }]}>
      <TouchableOpacity onPress={handlePress} activeOpacity={0.7}>
        <CircularProgress progress={progress} size={90} strokeWidth={8} color={isCompleted ? '#10B981' : config.color} backgroundColor={config.lightColor}>
          {isCompleted ? (<Animated.View style={{ transform: [{ scale: checkScaleAnim }] }}>
              <CheckCircle size={32} color="#10B981" fill="#10B981"/>
            </Animated.View>) : (<View style={styles.goalRingContent}>
      ...
      {isEditing ? (<View style={styles.editContainer}>
          <TextInput style={[styles.editInput, { borderColor: config.color }]} value={editValue} onChangeText={onEditChange} keyboardType="number-pad" autoFocus selectTextOnFocus/>
          <TouchableOpacity onPress={onSave} style={styles.editButton}>
            <Check size={14} color="#10B981"/>
          </TouchableOpacity>
        </View>) : (<View style={styles.goalValueContainer}>
...
function WeekProgressBar({ weeklyGoals }: {
    ...
    return (<View style={styles.weekProgressContainer}>
...
export const WeeklyGoalsCard = React.memo(function WeeklyGoalsCard({ onGoalComplete, compact = false }: WeeklyGoalsCardProps) {
    ...
    if (compact) {
        const completedCount = weeklyGoals.filter(g => g.completed).length;
        const totalProgress = weeklyGoals.reduce((sum, g) => sum + Math.min((g.current / g.target) * 100, 100), 0) / weeklyGoals.length;
    ...
    return (<View style={styles.container}>
      ...
      <View style={styles.content}>
        ...
        <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.goalsScrollContent}>
          {weeklyGoals.map((goal) => (<GoalProgressRing key={goal.id} goal={goal} isEditing={editingGoal === goal.type} onEdit={() => handleEditGoal(goal.type)} onSave={() => handleSaveGoal(goal.type)} editValue={editValues[goal.type] || ''} onEditChange={(value) => handleEditChange(goal.type, value)}/>))}
        </ScrollView>
        ...
        <View style={styles.goalsListContainer}>
          {weeklyGoals.map((goal) => {
            ...
            const progress = Math.min((goal.current / goal.target) * 100, 100);
            const IconComponent = config.icon;
            return (<View key={goal.id} style={[
                    styles.goalListItem,
                    goal.completed && styles.goalListItemCompleted,
                ]}>
                <View style={[styles.goalListIcon, { backgroundColor: config.lightColor }]}>
                  {goal.completed ? (<CheckCircle size={16} color="#10B981" fill="#10B981"/>) : (<IconComponent size={16} color={config.color}/>)}
                </View>
