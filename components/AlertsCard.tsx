...
import { AlertTriangle, AlertCircle, Bell, BellOff, X, Clock, ChevronRight, TrendingUp, TrendingDown, Tag, Target, Calendar, DollarSign, Sparkles, } from 'lucide-react-native';
...
function getPriorityGradient(priority: AlertPriority): [
    string,
    string
] {
    switch (priority) {
...
export const AlertItem = React.memo(function AlertItem({ alert, onDismiss, onSnooze, onAction, compact = false }: AlertItemProps) {
    ...
    if (compact) {
        return (<TouchableOpacity style={styles.compactItem} onPress={handleAction} activeOpacity={0.8}>
    ...
    return (<View style={styles.alertItem}>
      ...
      <View style={styles.alertContent}>
        ...
        <View style={styles.alertActions}>
          ...
          <View style={styles.secondaryActions}>
            <TouchableOpacity style={styles.iconButton} onPress={handleSnooze} activeOpacity={0.7}>
              <Clock size={16} color={COLORS.textSecondary}/>
            </TouchableOpacity>
            <TouchableOpacity style={styles.iconButton} onPress={() => onDismiss(alert.id)} activeOpacity={0.7}>
...
export const InsightItem = React.memo(function InsightItem({ insight }: InsightItemProps) {
    ...
    return (<View style={styles.insightItem}>
      <LinearGradient colors={['rgba(212, 165, 116, 0.1)', 'rgba(212, 165, 116, 0.02)']} style={StyleSheet.absoluteFill}/>
...
export const AlertsCard = React.memo(function AlertsCard({ alerts, insights = [], onDismiss, onSnooze, onClearAll, maxAlerts = 5, showInsights = true, title = 'Alerts & Insights', }: AlertsCardProps) {
...
export const CompactAlertsList = React.memo(function CompactAlertsList({ alerts, maxItems = 3, onPress }: CompactAlertsListProps) {
    ...
    if (alerts.length === 0)
        return null;
    return (<View style={styles.compactList}>
      {displayAlerts.map((alert) => (<TouchableOpacity key={alert.id} style={styles.compactListItem} onPress={() => onPress?.(alert)} activeOpacity={0.8}>
