...
import { DollarSign, Coffee, Utensils, Wine, Ship, Wifi, Plus, X, Calculator, } from 'lucide-react-native';
...
export function CompValueCalculator({ onCompValueChange, initialItems = [] }: CompValueCalculatorProps) {
    ...
    const handleAddItem = () => {
        ...
        if (!newItemName.trim() || isNaN(value) || value <= 0)
            return;
    ...
    return (<LinearGradient colors={MARBLE_TEXTURES.lightBlue.gradientColors} locations={MARBLE_TEXTURES.lightBlue.gradientLocations} style={styles.container}>
      ...
      {!showAddForm ? (<TouchableOpacity style={styles.addButton} onPress={() => setShowAddForm(true)} activeOpacity={0.8}>
          <Plus size={18} color={COLORS.white}/>
          <Text style={styles.addButtonText}>Add Comp Item</Text>
        </TouchableOpacity>) : (<View style={styles.addForm}>
          <Text style={styles.formLabel}>Category</Text>
          <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.categoryScroll}>
            {COMP_CATEGORIES.map((category) => {
                const isSelected = newItemCategory === category.id;
                const IconComponent = category.icon;
                return (<TouchableOpacity key={category.id} style={[
                        styles.categoryChip,
                        isSelected && [styles.categoryChipActive, { backgroundColor: category.color }],
                    ]} onPress={() => setNewItemCategory(category.id as CompItem['category'])} activeOpacity={0.7}>
          ...
          <TextInput style={styles.input} value={newItemName} onChangeText={setNewItemName} placeholder="e.g., Deluxe Beverage Package" placeholderTextColor={COLORS.textSecondary}/>

          <Text style={styles.formLabel}>Retail Value</Text>
          <View style={styles.valueInputContainer}>
            <Text style={styles.currencySymbol}>$</Text>
            <TextInput style={styles.valueInput} value={newItemValue} onChangeText={setNewItemValue} placeholder="0.00" placeholderTextColor={COLORS.textSecondary} keyboardType="decimal-pad"/>
          </View>
          ...
          <View style={styles.formActions}>
            ...
            ]} onPress={handleAddItem} disabled={!newItemName.trim() || !newItemValue || parseFloat(newItemValue) <= 0} activeOpacity={0.7}>
